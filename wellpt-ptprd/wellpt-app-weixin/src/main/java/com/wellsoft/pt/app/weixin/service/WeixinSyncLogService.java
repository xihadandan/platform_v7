/*
 * @(#)5/23/25 V1.0
 *
 * Copyright 2025 WELL-SOFT, Inc. All rights reserved.
 */
package com.wellsoft.pt.app.weixin.service;

import com.wellsoft.pt.app.weixin.dao.WeixinSyncLogDao;
import com.wellsoft.pt.app.weixin.entity.WeixinSyncLogDetailEntity;
import com.wellsoft.pt.app.weixin.entity.WeixinSyncLogEntity;
import com.wellsoft.pt.jpa.service.JpaService;

import java.util.List;

/**
 * Description:
 *
 * @author zhulh
 * @version 1.0
 *
 * <pre>
 * 修改记录:
 * 修改后版本		修改人		修改日期			修改内容
 * 5/23/25.1	    zhulh		5/23/25		    Create
 * </pre>
 * @date 5/23/25
 */
public interface WeixinSyncLogService extends JpaService<WeixinSyncLogEntity, WeixinSyncLogDao, Long> {
    void saveLog(WeixinSyncLogEntity weixinSyncLog, List<WeixinSyncLogDetailEntity> details);
}
