 ...
 * @date 5/26/25
 */
public class WeixinGroupChatUtils {
    public static boolean isGroupChat(List<WeixinWorkRecordEntity> workRecordEntities) {
        List<WeixinWorkRecordEntity> entities = workRecordEntities.stream().filter(entity -> entity.getType() == null || WeixinWorkRecordEntity.Type.SYSTEM.equals(entity.getType())).collect(Collectors.toList());
        return CollectionUtils.size(entities) == 1 && BooleanUtils.isTrue(entities.get(0).getGroupChat());
    }

    public static WeixinWorkRecordEntity getGroupChat(List<WeixinWorkRecordEntity> workRecordEntities) {
        List<WeixinWorkRecordEntity> entities = workRecordEntities.stream().filter(entity -> entity.getType() == null || WeixinWorkRecordEntity.Type.SYSTEM.equals(entity.getType())).collect(Collectors.toList());
        return CollectionUtils.size(entities) == 1 ? entities.get(0) : null;
    }

    public static boolean isMatchGroupChat(List<String> todoUserIds, WorkTodoEvent event, WeixinConfigVo.WeixinConfiguration weixinConfiguration) {
        WeixinConfigVo.WeixinGroupChat groupChat = weixinConfiguration.getGroupChat();
     ...
     * @return
     */
    private static boolean isMatchFlow(WeixinConfigVo.WeixinGroupChat groupChat, FlowDelegate flowDelegate, WorkTodoEvent event) {
     ...
     * @return
     */
    private static boolean isMatchMultiUserTask(WeixinConfigVo.WeixinGroupChat groupChat, TaskElement taskElement, List<String> todoUserIds) {
     ...
     * @return
     */
    private static boolean isMatchCollaborationTask(WeixinConfigVo.WeixinGroupChat groupChat, TaskElement taskElement, List<String> todoUserIds) {
        List<String> startModes = groupChat.getStartModes();
        if (CollectionUtils.isEmpty(startModes)) {
            return false;
        }
