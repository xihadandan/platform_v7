 ...
 * @date 3/28/25
 */
public class FeishuGroupChatUtils {

    public static boolean isGroupChat(List<FeishuWorkRecordEntity> workRecordEntities) {
        List<FeishuWorkRecordEntity> entities = workRecordEntities.stream().filter(entity -> entity.getType() == null || FeishuWorkRecordEntity.Type.SYSTEM.equals(entity.getType())).collect(Collectors.toList());
        return CollectionUtils.size(entities) == 1 && BooleanUtils.isTrue(entities.get(0).getGroupChat());
    }

    public static FeishuWorkRecordEntity getGroupChat(List<FeishuWorkRecordEntity> workRecordEntities) {
        List<FeishuWorkRecordEntity> entities = workRecordEntities.stream().filter(entity -> entity.getType() == null || FeishuWorkRecordEntity.Type.SYSTEM.equals(entity.getType())).collect(Collectors.toList());
        return CollectionUtils.size(entities) == 1 ? entities.get(0) : null;
    }

    public static boolean isMatchGroupChat(List<String> todoUserIds, WorkTodoEvent event, FeishuConfigVo.FeishuConfiguration feishuConfiguration) {
     ...
     * @return
     */
    private static boolean isMatchFlow(FeishuConfigVo.FeishuGroupChat groupChat, FlowDelegate flowDelegate, WorkTodoEvent event) {
     ...
     * @return
     */
    private static boolean isMatchMultiUserTask(FeishuConfigVo.FeishuGroupChat groupChat, TaskElement taskElement, List<String> todoUserIds) {
     ...
     * @return
     */
    private static boolean isMatchCollaborationTask(FeishuConfigVo.FeishuGroupChat groupChat, TaskElement taskElement, List<String> todoUserIds) {
        List<String> startModes = groupChat.getStartModes();
        if (CollectionUtils.isEmpty(startModes)) {
            return false;
        }
    ...
    public static boolean isSaveContent2Flow(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return !StringUtils.equals("none", groupChat.getSaveContentMode());
    }

    public static boolean isSaveTextContent2Flow(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return StringUtils.equals("text", groupChat.getSaveContentMode()) || StringUtils.equals("textAndFile", groupChat.getSaveContentMode());
    }

    public static boolean isSaveTextAndFileContent2Flow(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return StringUtils.equals("textAndFile", groupChat.getSaveContentMode());
    }

    public static boolean isDeleteChatOnTaskEnd(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return StringUtils.equals("taskEnd", groupChat.getDeleteMode());
    }

    public static boolean isDeleteChatOnFlowEnd(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return StringUtils.equals("flowEnd", groupChat.getDeleteMode());
    }

    public static boolean isAutoSubmit(FeishuConfigVo feishuConfigVo) {
        FeishuConfigVo.FeishuConfiguration feishuConfiguration = feishuConfigVo.getConfiguration();
        FeishuConfigVo.FeishuGroupChat groupChat = feishuConfiguration.getGroupChat();
        return StringUtils.equals("@_all", groupChat.getAutoSubmitMode());
    }
