 ...
 */
@Component
public class FlowHandleEfficiencyByDeptAndFlowDataStore extends FlowHandleEfficiencyByDeptDataStore {

    @Override
    public String getQueryName() {
        return "流程统计分析_办理效率分析按部门及流程统计";
    }
    ...
    public List<QueryItem> query(QueryContext context) {
        String deptId = Objects.toString(context.getQueryParams().get("deptId"), StringUtils.EMPTY);
        if (StringUtils.isBlank(deptId)) {
            return Lists.newArrayList();
        }

        context.getQueryParams().put("deptIds", Lists.newArrayList(deptId));
        Map<String, Object> queryParams = getQueryParams(context);
        List<QueryItem> queryItems = context.getNativeDao().namedQuery("flowHandleEfficiencyAverageHandleTimeGroupByDeptAndFlowQuery",
