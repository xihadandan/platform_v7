 ...
 */
@Component
public class MyAttentionGroupTreeDataProvider implements TreeComponentDataProvider {
    ...
    public List<TreeType> getNodeTypes() {
        return Lists.newArrayList(TreeType.createTreeType("Group", "群组"));
    }
    ...
    public List<TreeNode> loadTreeData(TreeComponentRequestParam arg0) {
        List<MyCalendarGroupEntity> list = attentionFacade.queryMyAttentionGroupList();
        List<TreeNode> children = Lists.newArrayList();
        if (CollectionUtils.isNotEmpty(list)) {
            for (MyCalendarGroupEntity entity : list) {
                TreeNode child = new TreeNode(entity.getUuid(), entity.getGroupName(), null);
                child.setType("group");
