define(["constant", "commons", "server", "HtmlWidgetDevelopment"],
    function (constant, commons, server, HtmlWidgetDevelopment) {
        ...
        commons.inherit(AppDingtalkRegisterWidgetDevelopment, HtmlWidgetDevelopment, {
            init: function () {
                ...
                var len = 0;
                var events = {
                    user_add_org: ["通讯录用户增加", "用户变更"],
                    user_modify_org: ["通讯录用户更改", "用户变更"],
                    user_leave_org: ["通讯录用户离职", "用户变更"],
                    user_active_org: ["加入企业后用户激活", "用户变更"],
                    org_admin_add: ["通讯录用户被设为管理员", "用户变更"],
                    org_admin_remove: ["通讯录用户被取消设置管理员", "用户变更"],
                    org_dept_create: ["通讯录企业部门创建", "部门变更"],
                    org_dept_modify: ["通讯录企业部门修改", "部门变更"],
                    org_dept_remove: ["通讯录企业部门删除", "部门变更"],
                                ...
                                if (data.call_back_tag && data.call_back_tag.length > 0) {
                                    ...
                                    for (var i = 0; i < callbackTag.length; i++) {
                                        $("#eventList").bootstrapTable("insertRow", {
                                            index: 0,
                                            row: {
                                                event: events[callbackTag[i]][0],
                                                eventType: callbackTag[i],
                                                eventCate: events[callbackTag[i]][1]
                                            }
                                        });
                                    }
                                } else {
                ...
                function initTable() {
                    $("#eventList").bootstrapTable("destroy").bootstrapTable({
                        data: [],
                        idField: "uuid",
                        pageStyle: "default",
                        pageStyleName: "默认分页",
                ...
                // 新增注册事件
                $("#btn_register").off().on("click", function () {
                    var html = "<form>" +
                ...
                $("#btn_deleteAll").off().on("click", function () {
                    ...
                    appModal.confirm("确定删除选择的已注册业务事件吗？<br>删除后，业务事件对应的数据，平台将不会处理，<br>从而造成钉钉和平台的数据不一致，请谨慎操作！", function (result) {
                        if (result) {
                            $.ajax({
                                type: "GET",
