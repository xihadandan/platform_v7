...
CREATE index category_idx on wf_flow_definition(category);
 CREATE index forbidden_sys_unit_id_type_idx on multi_org_user_account(is_forbidden,system_unit_id,type);
...
CREATE INDEX user_id_idx ON wf_task_instance_topping (user_id);
 CREATE index flow_i_uuid_task_i_uuid_idx on wf_task_activity(flow_inst_uuid,task_inst_uuid);
CREATE index obj_id_iden_gran_mask_idx on acl_entry(object_id_identity,granting,mask);
