...
(function($, document) {
	...
	var ImageViewer = $.ImageViewer = $.Class.extend({
		...
		bindEvent: function() {
			...
			document.addEventListener(enterEventName, function(event) {
				...
				self.swipe = self.images.length > 1;
				self.enableSwipe(self.swipe);
			...
			self.viewer.addEventListener('swipeleft', function(event) {
				if (self.scaleValue == 1 && self.swipe) self.next();
				event.preventDefault();
				event.cancelBubble = true;
			}, false);
			self.viewer.addEventListener('swiperight', function(event) {
				if (self.scaleValue == 1 && self.swipe) self.prev();
				event.preventDefault();
				event.cancelBubble = true;
			}, false);
			//处理缩放开始
		...
		enableSwipe : function(enable) {
			var self = this;
			if(self.leftButton){
				self.leftButton.style.display = enable ? "inline-block" : "none";
			}
			if(self.rightButton){
				self.rightButton.style.display = enable ? "inline-block" : "none";
			}
		},
		//释放不显示的图片
	...
	$.ready(function() {
		$.defaultImageViewer = $.imageViewer('.' + imageClassName);
	});
