 ...
 */
/**
 * 标识忽略多android 多webview处理方式
 */
var disabledMultiWebview = true;
/**
 * MUI核心JS
 * @type _L4.$|Function
 */
    ...
    document.addEventListener('focusin', function (e) {
      ...
      if (
        ...
        (target.tagName === 'TEXTAREA' ||
          (target.tagName === 'INPUT' && (target.type === 'text' || target.type === 'search' || target.type === 'number')))
      ) {
...
(function ($, window) {
  ...
  window.addEventListener(
    ...
    function (e) {
      ...
      if (
        ($.os.android || $.os.ios) &&
        (($.targets.popover && e.target === $.targets.popover) || $.targets.tab || $.targets.offcanvas || $.targets.modal)
      ) {
        e.preventDefault();
      }
    },
      ...
      if (hasPulldown || hasPullup) {
        ...
        if (container) {
          ...
          if ($container.length === 1) {
            if ($.os.plus && $.os.android && !disabledMultiWebview) {
              //android 5+
              $.plusReady(function () {
...
(function ($, window, document, undefined) {
  ...
  var Scroll = $.Class.extend({
    ...
    _initEvent: function (detach) {
      ...
      this.wrapper[action]('mousewheel', this);
    },
    ...
    _drag: function (e) {
      ...
      if (this.moved || isPreventDefault) {
        ...
        detail.gesture && detail.gesture.preventDefault && detail.gesture.preventDefault();
      }
      if (isReturn) {
        //禁止非法方向滚动
        return;
...
(function ($, window, document) {
  ...
  var createButtons = function (btnArray) {
    ...
    for (var i = 0; i < length; i++) {
      btns.push(
        '<span class="' + CLASS_POPUP_BUTTON + (i === length - 1 ? ' ' + CLASS_POPUP_BUTTON_BOLD : '') + '">' + btnArray[i] + '</span>'
      );
    }
  ...
  var createPrompt = function (message, placeholder, title, btnArray, callback, type) {
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(
        createInner(message, title || '提示', createInput(placeholder)) + createButtons(btnArray || ['取消', '确认']),
        callback
      );
    }
    ...
    if (container.classList.contains(CLASS_PROGRESSBAR)) {
      ...
      if (!progressbar) {
        ...
        progressbar.className =
          ...
          (typeof progress !== 'undefined' ? '' : ' ' + CLASS_PROGRESSBAR_INFINITE) +
  ...
  Input.prototype.createAction = function (row, actionClass, actionSelector) {
    ...
    if (!action) {
      ...
      if (actionClass === this.searchActionClass) {
        action.innerHTML =
          '<span class="' + CLASS_ICON + ' ' + CLASS_ICON_SEARCH + '"></span><span>' + this.element.getAttribute('placeholder') + '</span>';
...
(function ($, window) {
  ...
  Transparent.prototype.handleScroll = function () {
    this._style.backgroundColor =
      'rgba(' + this._R + ',' + this._G + ',' + this._B + ',' + (window.scrollY - this.options.top) / this.options.offset + ')';
  };
