...
module.exports = class Command {
  constructor(options) {
    options = options || {};
    this.name = options.name || 'well-init';
    this.configName = options.configName || 'well-init-config';
  ...
  * run(cwd, args) {
    ...
    // detect registry url
    this.registryUrl = this.getRegistryByType(argv.registry || 'wellsoft');
  ...
  * processFiles(targetDir, templateDir) {
  ...
  getParserOptions() {
    return {
      type: {
        type: 'string',
        description: '模板类型，支持: simple/plugin',
      },
      dir: {
        type: 'string',
        description: '目录',
      ...
      registry: {
        type: 'string',
        description: 'npm仓库，支持: wellsoft/china/npm/custom',
        alias: 'r',
      },
      silent: {
        type: 'boolean',
        description: '静默初始化，项目选项使用默认值',
      },
    };
  }
  ...
  getRegistryByType(key) {
    switch (key) {
      ...
      case 'wellsoft':
        return 'http://192.168.0.215:4873';
      default: {
        if (/^https?:/.test(key)) {
          return key.replace(/\/$/, '');
        } else {
  ...
  * getTemplateDir() {
    let templateDir;
    // when use `well-init --template=PATH`
  ...
  /**
   * fetch boilerplate mapping from `well-init-config`
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'well-init-boilerplate');
