<template>
  <a-table
    ...
    :locale="locale"
    :data-source="formConfig.materialFieldMappings"
    ...
    <template v-if="!disabled" slot="operationSlot" slot-scope="text, record, index">
      ...
      <ProcessDesignDrawer :id="'field-mapping-edit_' + record.id" :closeOpenDrawer="closeOpenDrawer" :width="800" title="编辑字段回填">
        ...
        <template slot="content">
          <MaterialFieldMappingConfiguration
            :fieldMapping="record"
...
import MaterialFieldMappingConfiguration from './item-form-material-field-mapping-configuration.vue';
...
export default {
  ...
  components: { ProcessDesignDrawer, MaterialFieldMappingConfiguration },
  inject: ['pageContext'],
  data() {
    if (!this.formConfig.materialFieldMappings) {
      this.$set(this.formConfig, 'materialFieldMappings', []);
    }
    return {
      locale: {
  ...
  mounted() {
    this.tableDraggable(
      this.formConfig.materialFieldMappings,
  ...
  methods: {
    deleteFieldMapping(index) {
      this.formConfig.materialFieldMappings.splice(index, 1);
    },
    onConfirmOk() {
      this.$refs.addFieldMapping.collect().then(data => {
        if (data) {
          this.formConfig.materialFieldMappings.push(data);
