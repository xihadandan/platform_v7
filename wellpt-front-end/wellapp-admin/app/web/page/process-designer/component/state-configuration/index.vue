<template>
  <a-table
    ...
    :pagination="false"
    :bordered="false"
    :columns="stateConfigTableColumn"
    :locale="locale"
    :data-source="definition.states"
  ...
  >
    ...
    <template v-if="!disabled" slot="operationSlot" slot-scope="text, record, index">
      ...
      <ProcessDesignDrawer :id="'state-config-edit_' + record.id" :closeOpenDrawer="closeOpenDrawer" :width="800" title="编辑状态配置">
        ...
        <template slot="content">
          <StateDefinition :stateDefinition="record" :formUuid="definition.formUuid"></StateDefinition>
        </template>
      </ProcessDesignDrawer>
    </template>
    <template v-if="!disabled" slot="footer">
      <ProcessDesignDrawer :id="drawerAddId" :closeOpenDrawer="closeOpenDrawer" :width="800" title="添加状态配置">
        ...
        <template slot="content">
          <StateDefinition ref="addStateDefinition" :formUuid="definition.formUuid"></StateDefinition>
        </template>
...
import StateDefinition from './state-configuration.vue';
...
export default {
  ...
  components: { ProcessDesignDrawer, StateDefinition },
  inject: ['pageContext'],
  data() {
    if (!this.definition.states) {
      this.$set(this.definition, 'states', []);
    }
    return {
      locale: {
        emptyText: <span>暂无数据</span>
      },
      stateConfigTableColumn: [
  ...
  created() {},
  mounted() {
    this.tableDraggable(this.definition.states, this.$el.querySelector('.widget-table-state-config-table tbody'), '.drag-btn-handler');
  },
  methods: {
    ...
    onConfirmOk() {
      this.$refs.addStateDefinition.collect().then(data => {
        if (data) {
          this.definition.states.push(data);
