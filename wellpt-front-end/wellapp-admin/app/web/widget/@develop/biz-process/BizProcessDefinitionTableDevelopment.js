...
class BizProcessDefinitionTableDevelopment extends WidgetTableDevelopment {

  get META() {
    return {
      name: '业务流程定义表格二开',
      hook: {
        deleteProcessDefinition: '删除业务流程定义',
        defExport: '定义导出',
        defImport: '定义导入'
      }
    };
  ...
  deleteProcessDefinition(evt) {
    ...
    evt.$evtWidget.$confirm({
      ...
      onOk() {
        $axios
          .post(`/proxy/api/biz/process/definition/deleteAll?uuids=${uuids}`)
          .then(({ data }) => {
            if (data.code == 0) {
              // 刷新表格
  ...
  defExport(evt) {
    ...
    let defDataExport = new DefDataExportComponent({
      propsData: { title: '定义导出', visible: true, uuid: uuids, type: 'bizProcessDefinition' }
    });
