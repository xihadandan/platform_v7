...
class DmsRoleDefinitionTableDevelopment extends WidgetTableDevelopment {
  get META() {
    return {
      name: '数据管理_文件库_权限组表格二开',
      hook: {
        delete: '删除权限组',
      }
    };
  ...
  delete(evt) {
    ...
    _this.$widget.$confirm({
      title: '确认框',
      content: `确定删除权限组[${selectedRows.map(row => row.name)}]吗？`,
      okText: '确定',
      cancelText: '取消',
      onOk() {
        $axios
          .post(`/proxy/api/dms/role/deleteAll?uuids=${uuids}`)
          .then(({ data: result }) => {
            if (result.code == 0) {
              // 刷新库分类
...
export default DmsRoleDefinitionTableDevelopment;
