...
class messageFormatManageTableDevelopment extends WidgetTableDevelopment {
  get META() {
    return {
      name: '消息格式管理表格二开',
      hook: {
  ...
  mounted() {
    ...
    _this.criterions = [];
    // 监听分类点击事件变化
    widget.pageContext.handleEvent('refetchMessageFormatManangeTable', ({ classifyUuid }) => {
      let criterions = [];
      if (classifyUuid) {
        criterions.push({
  ...
  delete() {
    const ids = _.map(this.$widget.selectedRows, 'id');
    if (ids.length == 0) {
  ...
  defExport(evt) {
    ...
    let defDataExport = new DefDataExportComponent({
      propsData: { title: '定义导出', visible: true, uuid: uuids, type: 'messageTemplate' }
    });
...
export default messageFormatManageTableDevelopment;
