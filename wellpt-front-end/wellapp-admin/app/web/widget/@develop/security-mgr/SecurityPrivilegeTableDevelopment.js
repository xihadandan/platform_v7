...
class SecurityPrivilegeTableDevelopment extends WidgetTableDevelopment {
  get META() {
    return {
      name: '权限管理_权限表格二开',
      hook: {
  ...
  delete(evt) {
    ...
    _this.$widget.$confirm({
      title: '确认框',
      content: `确定要删除权限[${selectedRows.map(row => row.name)}]吗？`,
      okText: '确定',
      cancelText: '取消',
      onOk() {
        $axios
          .post('/proxy/api/security/privilege/removeAll', uuids)
          .then(({ data: result }) => {
            if (result.code == 0) {
              // 刷新角色列表
              _this.$widget.refetch(true);
              _this.$widget.pageContext.emitEvent("privilege:deleted");
  ...
  defExport(evt) {
    ...
    let defDataExport = new DefDataExportComponent({
      propsData: { title: '定义导出', uuid: uuids, type: 'privilege' }
    });
...
export default SecurityPrivilegeTableDevelopment;
