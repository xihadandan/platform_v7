...
class datastoreManageTableDevelopment extends WidgetTableDevelopment {
  get META() {
    return {
      name: '数据仓库管理表格二开',
      hook: {
  ...
  mounted() {
    ...
    let _this = this;
    // 监听分类点击事件变化
    widget.pageContext.handleEvent('refetchDatastoreManageTable', ({ }) => {
      widget.refetch();
    });
  }
  delete() {
  ...
  deleteReq(ids) {
    ...
    widget.$confirm({
      ...
      onOk() {
        $axios
          .post('/proxy/api/datastore/deleteByUuids', ids)
          .then(({ data }) => {
            if (data.code == 0) {
  ...
  defExport(evt) {
    ...
    let defDataExport = new DefDataExportComponent({
      propsData: { title: '定义导出', visible: true, uuid: uuids, type: 'dataStoreDefinition' }
    });
...
export default datastoreManageTableDevelopment;
