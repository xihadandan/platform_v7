      ...
      <a-col :span="12" style="border: 1px solid #e8e8e8; padding: 7px; border-left: unset">
        <PerfectScrollbar :style="{ height: vHeight }">
          <a-tooltip>
            ...
            <a-alert banner type="info" class="pt-alert" message="关联功能信息" style="margin-bottom: 7px"></a-alert>
          </a-tooltip>
...
export default {
  name: 'OrgRoleResult',
  props: { orgElementUuid: String, userId: String, orgVersionUuid: String, panelHeight: String },
  inject: ['pageContext'],
  data() {
  ...
  methods: {
    ...
    fetchOrgElementRolePrivilegeTree() {
      $axios
        .get(`/proxy/api/org/organization/version/orgElement/getOrgElementRolePrivilegeTree`, {
          params: {
            orgElementUuid: this.orgElementUuid
          }
        })
        .then(({ data }) => {
    ...
    getOrgUserRolePrivilegeTree() {
      $axios
        .get(`/proxy/api/org/organization/version/user/getOrgUserRolePrivilegeTree`, {
          params: {
  ...
  beforeMount() {
    if (this.orgElementUuid != undefined) {
      this.fetchOrgElementRolePrivilegeTree();
    } else if (this.userId != undefined) {
      this.getOrgUserRolePrivilegeTree();
    }
  },
  mounted() {}
};
