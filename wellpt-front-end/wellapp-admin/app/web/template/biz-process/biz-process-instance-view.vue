<template>
  ...
  <a-layout v-else-if="bizData" class="process-view-container" theme="light">
    <a-layout-header class="process-view-header">
      <a-row>
        <a-col span="18" class="title">
          <h1>{{ bizData.title }}</h1>
        </a-col>
        <a-col span="6" class="process-action-container">
          <a-space>
    ...
    <a-layout-content class="process-view-content">
      <WidgetDyform
...
export default {
  ...
  computed: {
    isNewFormData() {
      return isEmpty(this.bizData.processInstUuid);
    },
    dataUuid() {
  ...
  methods: {
    ...
    loadData() {
      const _this = this;
      let processData = _this.processData || {};
      let processInstUuid = processData.processInstUuid || _this.getQueryString('processInstUuid', '');
      $axios.post('/proxy/api/biz/process/instance/get', { processInstUuid }).then(({ data }) => {
        if (data.data) {
    ...
    onDyformMounted() {
      this.loadProcessNodeInstances();
    },
    // 加载过程节点实例数据
    loadProcessNodeInstances() {
      if (!this.bizData.processNodePlaceHolder) {
        return;
      }

      this.getTableWidget().then(widget => {
        let $el = document.querySelector(`[w-code='${this.bizData.processNodePlaceHolder}']`);
        if ($el && $el.parentNode) {
    ...
    getTableWidget() {
      ...
      return _this
          ...
          let WidgetTable = Vue.extend({
            ...
            methods: {
              beforeLoadData() {
                ...
                dataSource.addParam('processInstUuid', _this.bizData.processInstUuid);
                dataSource.addParam('loadItemInstCount', true);
              }
            }
          });
        ...
        .catch(res => {
          _this.$message.error('阶段实例数据加载失败！');
        });
    },
    getTableDefinition() {
      let widgetTableId = 'TykdXdALUvvTSPKRNRzuDfJFZOmHKpci';
    ...
    reload: function (processInstUuid) {
      if (!isEmpty(processInstUuid)) {
        window.location.href = this.addSystemPrefix(`/biz/process/instance/view?processInstUuid=${processInstUuid}`);
      } else {
    ...
    save() {
      ...
      $axios
        .post('/proxy/api/biz/process/instance/save', bizData)
        .then(({ data }) => {
          if (data.data) {
