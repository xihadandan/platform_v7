define(["jquery", "constant", "commons", "server", "appContext", "appModal", "AppDyformDesignerUtils" , "HtmlWidgetDevelopment", "AppDyformDesignerEditExtendDevelopment"],
    function ($, constant, commons, server, appContext, appModal, DesignerUtils, HtmlWidgetDevelopment) {
        ...
        commons.inherit(AppDyformDesignerDevelopment, HtmlWidgetDevelopment, {
            init: function () {
                ...
                var isCopy = Browser.getQueryString('copy');
                addDyformTypeScript(formUuid,formType);
                // P:存储单据, V:展现单据, M: 手机单据 MST: 子单据(模板单据)
                _self.widget.element.find('.dyform-' + formType).removeClass('hide');
                if(!formUuid) {
                    _self.widget.element.find('.dyform-' + formType + '-new').addClass('hide');
                }
