(function (factory) {
    ...
    window.cleanUselessDefinition = DesignerUtils.cleanUselessDefinition = function (srcHtml,type) {
        ...
        for (var fieldName in fields) {
            if (fields[fieldName].master === true) {
                continue;
            } else if ($html.find(".value[name='" + fieldName + "']").size() == 0) {// 在模板中没有找到该字段,则删除该字段的定义
            ...
            } else if ($html.find(".value[name='" + fieldName + "']").attr('inputmode') === '130') { //排除真实值占位符
        ...
        for (var formUuid in subforms) {
            if ($html.find("table[formUuid='" + formUuid + "']").size() == 0) {// 在模板中没有找到该字段,则删除该从表
                // 的定义
        ...
        for (var tableViewId in tableViews) {
            if ($html.find('table[tableviewid="' + tableViewId + '"]').length == 0) {// 模板中没有找到视图列表，则删除视图列表定义
        ...
        for (var name in layouts) {
            if ($html.find(".layout[name='" + name + "']").size() == 0) {// 在模板中没有找到该字段,则删除该从表
                // 的定义
        ...
        if (typeof templates != "undefined") {
            for (var blockCode in templates) {
                if ($html.find(".template-wrapper[templateuuid='" + blockCode + "']").size() == 0) {// 在模板中没有找到该字段,则删除该子表单的定义
    ...
    DesignerUtils.validateAndSaveForm = function (evenSource) {
        ...
        setTimeout(function () {
            ...
            if (!isValid) {// 验证不通过
                return;
            }else if ((DesignerUtils.isFormTypeAsMstform() || DesignerUtils.isFormTypeAsPform()) && !(/^[a-zA-Z0-9_]+$/.test(tableId))) {// 存储单据才做这个检验
                if(tableId === '') {
    ...
    DesignerUtils.fillBasicPropertyTab = function (defintionObj) {
        var copyForm = Browser.getQueryString('copy');
        if (copyForm === '1') {
            defintionObj.id = "";
        ...
        function renderFieldsTree(treeNodes) {
            ...
            for (var i = 0; i < treeNodes.length; i++) {
                ...
                for (var j = 0; j < trees.length; j++) {
                    var treeNode = trees[j], needGrid = "true";
    ...
    DesignerUtils.initEvent = function () {
        ...
        for (var i = 0, len = zRootNodes.length; i < len; i++) {
            $("#dyformScriptContent").append($("<div>", {
            ...
            if (i != 0)
                aceCoderPlugin[zRootNodes[i].id].getContainer().hide();
        }
    ...
    DesignerUtils.getFormDefinition = function(evenSource){
        ...
        if (copyForm === '1') {
            ...
            $('#formUuid').val(formDefinition.formUuid);
        }
        ...
        if (deletedFieldNames && deletedFieldNames.length > 0) {// 删除提醒
        ...
        if (formDefinitionCopy) {
            if (formDefinitionCopy.fields)
                delete formDefinitionCopy.fields;
            if (formDefinitionCopy.subforms)
                delete formDefinitionCopy.subforms;
            if (formDefinitionCopy.html)
            ...
            success: function (result) {
                if (result.success == "true" || result.success == true) {
                    ...
                    appModal.alert("保存成功！",function () {
                        ...
                        var _hrefArr = location.href.split('&uuid=');
                        if(_hrefArr.length === 1) {
                            location.href = _hrefArr[0] + '&uuid=' + formUuid;
                        } else {
                            var _hrefArr2 = _hrefArr[1].split('&formType=');
                            if(_hrefArr2.length > 1) {
                                if(_hrefArr2[1].indexOf('&copy=') > -1) {
                                    _hrefArr2[1] = _hrefArr2[1].split('&copy=')[0];
                                }
                                location.href =  _hrefArr[0] + '&uuid=' + formUuid + '&formType=' + _hrefArr2[1];
                            }
                        }
                    });
                } else {
    ...
    DesignerUtils.initList = function (formUuid) {
        ...
        $(form_def_list_selector).bootstrapTable("destroy").bootstrapTable({
            data: _data,
            idField: "uuid",
            // striped: true,
            striped: false,
    ...
    var referenceRes = {
        datadic: [], serialnumer: [], jsModule: [], datastore: [], slaveForm: [], subForm: []
    };
    ...
    DesignerUtils.explainFormReferenceResouce = function () {
        ...
        for (var f in fields) {
            ...
            //数据字典资源
            if (im == dyFormInputMode.checkbox || im == dyFormInputMode.selectMutilFase
                || im == dyFormInputMode.radio || im == dyFormInputMode.comboSelect) {
        ...
        for (var k in formDefinition.templates) {
            ...
            server.JDS.call({
                ...
                success: function (result) {
                    ...
                    if (formDef) {
                        data.push({
                            uuid: k, name: t.templateName,
                            version: formDef.version, tableName: formDef.tableName, id: formDef.id
                        });
                        totalRes.total++;
                        totalRes.rows.push({
                            'uuid': totalRes.total,
    ...
    DesignerUtils.initRefJsModuleTable = function (totalRes) {
        ...
        for (var i = 0; i < referenceRes.jsModule.length; i++) {
            ...
            server.JDS.call({
                ...
                data: [referenceRes.jsModule[i].js], version: '',
                success: function (result) {
                    var jsModule = result.data;
                    if (jsModule) {
                        data.push({
                            'uuid': i, 'jsName': jsModule.name
                        });
                        totalRes.total++;
                        totalRes.rows.push({
                            'uuid': totalRes.total,
    ...
    DesignerUtils.initRefDataDicTable = function (totalRes) {
        ...
        for (var i = 0; i < referenceRes.datadic.length; i++) {
            ...
            if (dd.dictCode != '') {
                ...
                data.push({
                    'uuid': i, 'id': parts[0], 'name': parts[1], 'refField': dd.displayName
                });
                totalRes.total++;
                totalRes.rows.push({
                    'uuid': totalRes.total,
    ...
    DesignerUtils.init = function (uuid) {
            ...
            sid = setTimeout(function (t) {
                jsTemplates2.find("ul.js-tab-content-control").each(function (idx, elem) {
                    ...
                    $elem.find("li[plugin-code]").each(function (idx, eItem) {
                        ...
                        if (treeNode.name.indexOf(text) > -1 || treeNode.url.indexOf(text) > -1 ||
                            treeNode.iconSkin.indexOf(text) > -1) {
        ...
        var customCkeditorPath = ctx + "/resources/pt/js/dyform/definition/ckeditor";// 自定义ckeditor相关配置的路径
        ...
        window.editor = CKEDITOR.replace('moduleText', {
            ...
            toolbar: [
                ...
                ['Format', 'Font', 'FontSize'], ['TextColor', 'BGColor','Link'], [// 'Image',
                    'Table'], [CkPlugin.BLOCK, CkPlugin.TABS, CkPlugin.BUTTON, CkPlugin.TABLEVIEWCTL,CkPlugin.FILELIBRARYCTL, CkPlugin.TEMPLATECONTAINER, CkPlugin.TIPS], ['dyformpreview'], ['propertiesDialog'],// ['copyForm'],
                ['Source', 'Maximize']
            ...
            on: {
                instanceReady: function (ev) {
                    ...
                    var explainDropLocationNode = function (ui) {
                        ...
                        var count = tdNodes.count();
                        clearFocusLocation();
                        var scrollTop = $("#cke_moduleText iframe")[0].contentWindow.pageYOffset
                            || $("#cke_moduleText iframe").contents()[0].documentElement.scrollTop
                            || $("#cke_moduleText iframe").contents()[0].body.scrollTop || 0;
                        ...
                        for (var i = 0; i < count; i++) {
                            ...
                            var inOffsetTop = (ui.offset.top - outOffset.top - toolBarHeight) + (uiHeight / 2)
                                + scrollTop;

                            // console.log("location:[%s,%s] , leftRange:[%s,%s] , topRange:[%s,%s] , scrollTop:[%s]", inOffsetLeft,
                            //     inOffsetTop, beginLeft, endLeft, beginTop,
                            //     endTop, scrollTop);

                            if (beginLeft <= inOffsetLeft && endLeft >= inOffsetLeft && beginTop <= inOffsetTop
                                && endTop >= inOffsetTop) {// 在布局格子的范围内
                                return node;
                            }
                        }
                    }
                    ...
                    var scrollView = function (nodeOffset) {
                        ...
                        } else if (nScrollTop > 0 && nodeOffset.top <= (nScrollTop)) {// 向上滚动一段距离
                            $frameBody.scrollTop(nScrollTop - 100);
                        }
                    }
                    $(document.body).on("renderFieldsTree", function (event) {
                        // 拖拽绑定
                        $(".jsCanDraggable").draggable({
                            ...
                            iframeFix: true,
                            drag: function (event, ui) {// 拖拽过程中事件，实现红色虚框定位布局格子
                    ...
                    $("#cke_moduleText").droppable({
                        ...
                        drop: function (event, ui) {
                            ...
                            if (!node && needGrid) {// 找到落点格子，但是该控件是需要布局作为容器
                            ...
                            if (node) {// 添加到布局落点里面
                                ...
                                if (id && json && (json.name || json.templateName)) {
                                    ...
                                    if (pluginCode === CkPlugin.SUBFORM) {
                                        ...
                                        ctlHtml = "<img data-is-temp=\"true\" class=\"value\" templateUuid=\"" + json.templateUuid + "\" templateName=\"" + json.templateName + "\">";
                                    } else {
                    ...
                    var createDef = function (def) {
                        return CKEDITOR.tools.extend(def || {}, {
                            ...
                            refresh: function (editor, path) {
                                this.setState(path.contains('table', 1) ? CKEDITOR.TRISTATE_OFF
                ...
                keyup: function (evt) {
                    if (!(evt.data.keyCode == '8' || evt.data.keyCode == '13' || evt.data.keyCode == '46'
                        || evt.data.keyCode == 10000 || (evt.data.keyCode == '1114202'))) {
                        return true;
                    }
                    // 元素删除事件
        ...
        if (typeof uuid === "undefined" || uuid == null || uuid == "") {
            ...
            $('#version').val(version);
            $("#dyformVersion").html(version);
            $('#formType').val(Browser.getQueryString('formType'));
        } else {
        ...
        $("#customJsModule").wSelect2({
            ...
            height: 250
        }).val(formDefinition.customJsModule).trigger("change");

        // 存储服务
        var repository = formDefinition.repository || {};
        $("#repositoryMode").on("change", function () {
            var mode = $(this).val();
            onRepositoryModeChange(mode);
        }).val(repository.mode || "1").trigger("change").wSelect2({
        ...
        $("#customInterface").wSelect2({
            ...
            height: 250
        }).val(repository.customInterface).trigger("change");
