(function (factory) {
  ...
  DesignerUtils.compareProductFields = function (fields) {
    ...
    if (unusedFields.length) {
      $productField.each(function () {
        ...
        if ($.inArray(fieldName, unusedFields) > -1 && !$this.hasClass('unused')) {
  ...
  window.cleanUselessDefinition = DesignerUtils.cleanUselessDefinition = function (srcHtml, type) {
    ...
    for (var fieldName in fields) {
      if (fields[fieldName].master === true) {
        ...
        $html.find(".value[name='" + fieldName + "']").size() > 0 &&
        formDefinition.deletedFieldNames &&
        formDefinition.deletedFieldNames.indexOf(fieldName) > -1
      ) {
        var fIndex = formDefinition.deletedFieldNames.indexOf(fieldName);
        formDefinition.deletedFieldNames.splice(fIndex, 1);
      }
    }
  ...
  DesignerUtils.setPageAndDialogTile = function (uuid) {
    ...
    if (StringUtils.isBlank(uuid)) {
      ...
      switch (formDefinition.formType) {
        case 'P':
        case 'C':
          var editType = Browser.getQueryString('editType');
          if (editType && editType == 'receiveEditDesigner') {
            _title = '(' + formDefinition.name + ')收件文档展示单据';
          } else {
            _title = '(' + formDefinition.name + ')发件文档展示单据';
          }
          break;
        case 'MST':
  ...
  DesignerUtils.saveForm = function (evenSource) {
    ...
    var dmsDocExchangeConfigUuid = Browser.getQueryString('uuid');
    var dyformUuid = Browser.getQueryString('formUuid');
    var uuid = Browser.getQueryString('docUuid') || '';
    var editType = Browser.getQueryString('editType');
    $.ajax({
      headers: {
        'x-csrf-token': $.cookie('_csrfToken')
      },
      url: ctx + '/api/dms/doc/exc/dyform/saveOrUpdate',
      type: 'post',
      data: JSON.stringify({
        definitionJson: JSON.cStringify(formDefinitionData.formDefinition.definitionJson),
        dmsDocExchangeConfigUuid: dmsDocExchangeConfigUuid,
        dyformUuid: dyformUuid,
        editType: editType,
        uuid: uuid
      }),
      dataType: 'json',
        ...
        if (result.code == 0) {
          ...
          appModal.success({
            ...
            callback: function () {
              ...
              if (uuid) {
                var _hrefArr = location.href.replace(uuid, formUuid);
              } else {
                var _hrefArr = location.href.replace('docUuid=', 'docUuid=' + formUuid);
              }
              location.href = _hrefArr;
            }
          });
        } else {
          appModal.alert('保存失败\n' + result.data);
  ...
  DesignerUtils.getDefaultDefinition = function (formDefinition, defaultFormDefinition) {
    var newFormDefinition = formDefinition;
    for (var i in defaultFormDefinition.blocks) {
      if (!newFormDefinition.blocks) {
        newFormDefinition.blocks = {};
      }
      newFormDefinition.blocks[i] = defaultFormDefinition.blocks[i];
    }
    newFormDefinition.formTree = newFormDefinition.formTree
      ? newFormDefinition.formTree.concat(defaultFormDefinition.formTree)
      : _.cloneDeep(defaultFormDefinition.formTree);
    newFormDefinition.html += defaultFormDefinition.html;
    for (var i in defaultFormDefinition.fields) {
      newFormDefinition.fields[i] = defaultFormDefinition.fields[i];
    }
    return newFormDefinition;
  };
  // 初始化
  DesignerUtils.init = function (uuid, docUuid, defaultFormDefinition) {
    // 初始化开始
    window.formDefinition = docUuid ? defaultFormDefinition : new MainFormClass(); // 用于保存定义数据
    uuid = uuid || '';
    ...
    window.editor = CKEDITOR.replace('moduleText', {
      ...
      on: {
        instanceReady: function (ev) {
          if (!docUuid) {
            var newFormDefinition = DesignerUtils.getDefaultDefinition(formDefinition, defaultFormDefinition);
            formDefinition.product_fields = _.cloneDeep(newFormDefinition.fields);
          } else {
            var newFormDefinition = DesignerUtils.getDefaultDefinition(
              formDefinition,
              loadFormDefinition(Browser.getQueryString('formUuid'))
            );
            formDefinition.product_fields = _.cloneDeep(formDefinition.fields);
          }

          DesignerUtils.initModuleTree(formDefinition);
          DesignerUtils.initProductNav(loadFormDefinition(uuid));
          DesignerUtils.initDefaultProductNav(formDefinition);
          // 清除非放置落点的红色聚焦样式
        ...
        doubleclick: function (evt) {
          ...
          if (ctlName) {
            if (formDefinition.product_fields && formDefinition.product_fields[ctlName]) {
              setZzc(pluginName);
            }
          } else {
    ...
    if (typeof uuid === 'undefined' || uuid == null || uuid == '') {
      ...
      window.formDefinition = docUuid
        ? defaultFormDefinition
        : DesignerUtils.getDefaultDefinition(loadFormDefinition(uuid) || window.formDefinition, defaultFormDefinition); // 加载表单定义
