define(["constant", "commons", "server", "appContext", "appModal", "formBuilder", "AppPtMgrListViewWidgetDevelopment"],
    function (constant, commons, server, appContext, appModal, formBuilder, AppPtMgrListViewWidgetDevelopment) {
        ...
        // 平台管理_流程管理_信息格式_列表组件二开
        var AppWorkflowFormatListViewWidgetDevelopment = function () {
            AppPtMgrListViewWidgetDevelopment.apply(this, arguments);
        };
        // 接口方法
        commons.inherit(AppWorkflowFormatListViewWidgetDevelopment, AppPtMgrListViewWidgetDevelopment, {
            // 准备创建回调
            prepare: function () {
            ...
            btn_del: function () {
                ...
                appModal.confirm('确定要删除所选记录吗?', function (res) {
                    if (res) {
                        JDS.restfulPost({
							url: ctx + "/api/workflow/format/deleteAll",
							data: {uuids: uuids},
            ...
            btn_dependence: function () {
                this.onDependence('flowFormat');
            }
        });
        return AppWorkflowFormatListViewWidgetDevelopment;
    });
