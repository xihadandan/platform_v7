define(['constant', 'commons', 'server', 'HtmlWidgetDevelopment'], function (constant, commons, server, HtmlWidgetDevelopment) {
  var JDS = server.JDS;
  // 页面组件二开基础
  var AppWorkFlowCateWidgetDevelopment = function () {
    HtmlWidgetDevelopment.apply(this, arguments);
  };
  // 接口方法
  commons.inherit(AppWorkFlowCateWidgetDevelopment, HtmlWidgetDevelopment, {
    // 组件初始化
    init: function () {
      ...
      $('.msg-category-folder', $container)
        .off()
        .on('click', function () {
          var width = $(this).parents('.tab-pane').width();
      ...
      $('#flowAllCategory', $container)
        ...
        .on('click', function () {
          ...
          var msgFormateTable = $('#flow_category_content', $container)
            .parents('.panel-tab-content')
            .find('.ui-wBootstrapTable')
            .attr('id');
      ...
      function getList(val) {
        JDS.restfulPost({
          url: ctx + '/api/workflow/category/getAllBySystemUnitIdsLikeName',
          data: {
            name: val || ''
          },
      ...
      function getDetail(bean, uuid, $dialog) {
        JDS.restfulGet({
          url: ctx + '/api/workflow/category/get',
          data: {
            uuid: uuid
          },
      ...
      $('#flow_category_tree', $container).on('click', '.btn-delete', function (e) {
        ...
        appModal.confirm('确定删除分类吗', function (success) {
          if (success) {
            JDS.restfulPost({
              url: ctx + '/api/workflow/category/deleteWhenNotUsed',
              data: {
                uuid: uuid
              },
              contentType: 'application/x-www-form-urlencoded',
  ...
  return AppWorkFlowCateWidgetDevelopment;
});
