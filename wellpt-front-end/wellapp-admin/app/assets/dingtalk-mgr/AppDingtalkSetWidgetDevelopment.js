    ...
    init: function () {
      ...
      $('#system_unit_id')
        ...
        .on('change', function (e, type) {
          if (!type) {
            var orgVersionId = '',
              systemUnitId = $('#system_unit_id').val();
            if ($.trim(systemUnitId).length > 0) {
              $.ajax({
                url: ctx + '/api/org/version/getDefaultVersionBySystemUnitId',
                type: 'get',
                data: {
          ...
          if (i.indexOf('org') > -1) {
            if (i == 'org.org_version_id' && $.trim(data[i]).length) {
              ...
              if (lastMsg['time']) {
                ...
                $('#syncTime')
                  .attr('data-code', lastMsg['code'])
                  .html('上次同步时间：' + lastMsg['time'])
                  .show();
      ...
      function getMessage(code) {
        var message =
          code == 0
            ? "<div style='text-align: left;'>组织同步失败！</div><div style='color:#999;text-align: left;'>异常信息：<span style='color:#f00;'>代码错误或异常</span></div>"
            : '钉钉组织架构同步完成！';
        return message;
      }
