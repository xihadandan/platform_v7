define(['constant', 'commons', 'server', 'moment', 'ListViewWidgetDevelopment'], function (
  ...
  'use strict';
  var AppDingtalkOrgSyncLogListDevelopment = function () {
  ...
  commons.inherit(AppDingtalkOrgSyncLogListDevelopment, ListViewWidgetDevelopment, {
    ...
    beforeLoadData(options, configuration, request) {
      ...
      for (var i = 0; i < criterions.length; i++) {
        if (criterions[i].columnIndex && criterions[i].columnIndex == 'SYNC_STATUS' && criterions[i].value == "''") {
          index = i;
          break;
        }
      }
  ...
  return AppDingtalkOrgSyncLogListDevelopment;
});
