define(['constant', 'commons', 'server', 'appContext', 'appModal', 'AppPtMgrListViewWidgetDevelopment', 'AppPtMgrCommons'], function (
  ...
  // 平台管理_产品集成_模块_数据仓库列表_视图组件二开
  var AppDatastoreListViewWidgetDevelopment = function () {
    AppPtMgrListViewWidgetDevelopment.apply(this, arguments);
  };

  // 接口方法
  commons.inherit(AppDatastoreListViewWidgetDevelopment, AppPtMgrListViewWidgetDevelopment, {
    // 组件准备
    prepare: function () {},
    ...
    beforeRender: function (options, configuration) {
      ...
      this.widget.addParam('includeRef', 1);
    },

    afterRender: function (options, configuration) {
      var _self = this;
      if (!_self._moduleId()) {
        _self.widget.element.find('.li_class_reference_datastore').hide();
        _self.widget.element.find('.li_class_btn_cancel_ref').hide();
      }
      this.widget.on('AppDatastoreListView.refresh', function () {
        _self.refresh();
      });
    },
            ...
            server.JDS.call({
              ...
              success: function (result) {
                if (result.success) {
                  var refTitle = '';
                  if (result.data) {
                    refTitle = (result.data.appSystemBean ? result.data.appSystemBean.name + '/' : '') + result.data.name;
                  }
                  $td.append(
                    $('<a>', {
    ...
    refrenceDatastoreDialogHtml: function () {
      ...
      var $row2 = $('<div>', { class: 'row form-group mt10' }).append(
        $('<div>', { class: 'col-sm-2 required' }).text('引用数据仓库'),
        $('<div>', { class: 'col-sm-10' }).append(
          $('<input>', {
            type: 'hidden',
            id: 'moduleDatastoreInput',
            style: 'width:400px;'
          })
        )
      );
    ...
    //引用其他模块的数据仓库
    reference_datastore: function () {
      //弹窗
      var _self = this;
      var $dialog;
      var dialogOpts = {
        title: '引用数据仓库',
        message: _self.refrenceDatastoreDialogHtml(),
        buttons: {
          confirm: {
            label: '确定',
            ...
            callback: function (result) {
              ...
              var moduleDsUuid = $('#moduleDatastoreInput').val();
              if (!moduleDsUuid) {
                return false;
              }
              server.JDS.call({
                  ...
                  [
                    {
                      ...
                      entityClass: 'CdDatastoreDefinitionRefEntity'
                    }
                  ]
                ],
                version: '',
        ...
        shown: function () {
          ...
          $moduleSelect
            .on('change', function () {
              ...
              $('#moduleDatastoreInput').wSelect2({
                valueField: 'moduleDatastoreInput',
                remoteSearch: false,
                serviceName: 'cdDataStoreDefinitionService',
                queryMethod: 'loadSelectDefinitionByModule',
                params: {
                  moduleId: moduleId,
                  idProperty: 'uuid',
    ...
    btn_delete: function () {
      ...
      if (rowData.length > 0) {
        ...
        appModal.confirm('确认要删除数据仓库吗?', function (result) {
          if (result) {
            // server.JDS.call({
            //   service: 'cdDataStoreDefinitionService.deleteByUuids',
            //   version: '',
            server.JDS.restfulPost({
              url: '/proxy/api/datastore/deleteByUuids',
              data: (function () {
                var uuids = [];
              ...
              success: function (result) {
                ...
                $(_self.widget.element).parent().next().find('.ui-wHtml').clearForm(true);
				$(_self.widget.element).parent().next().find('#type', '.ui-wHtml').trigger('change');
    ...
    btn_add: function () {
      $('.btn_class_btn_delete', this.widget.element).prop('disabled', false);
      // 触发应用列表行点击事件
      this.widget.trigger('AppDatastoreListView.editRow', {
        ui: this.widget
      });
    },
    ...
    onClickRow: function (rowNum, rowData, $element, field) {
      $('.btn_class_btn_delete', this.widget.element).prop('disabled', rowData.isRef === 1);
      // 触发应用列表行点击事件
      this.widget.trigger('AppDatastoreListView.editRow', {
        rowData: rowData,
        ui: this.widget
    ...
    btn_cancel_ref: function () {
      ...
      appModal.confirm('确认取消引用数据仓库吗?', function (result) {
        if (result) {
          server.JDS.call({
  ...
  return AppDatastoreListViewWidgetDevelopment;
});
