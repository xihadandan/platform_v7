define(["constant", "commons", "server", "appContext", "appModal", "AppPtMgrListViewWidgetDevelopment",
    ...
    var AppMessageTemplateListViewWidgetDevelopment = function () {
        AppPtMgrListViewWidgetDevelopment.apply(this, arguments);
    };

    // 接口方法
    commons.inherit(AppMessageTemplateListViewWidgetDevelopment, AppPtMgrListViewWidgetDevelopment, {
        // 组件准备
        prepare: function () {
        },
        ...
        afterRender: function (options, configuration) {
            var _self=this;
            this.widget.on('AppMessageTemplateListView.refresh',function(){
                _self.refresh();
            });
        },
        ...
        definition_export: function () {
            ...
            if (rowData.length > 0) {
                ...
                appModal.alert('请选择导出的消息格式！');
            }

        },

        refrenceMsgTemplateDialogHtml: function () {
            ...
            var $row1 = $("<div>", {"class": "row form-group"}).append(
                ...
                $("<div>", {"class": "col-sm-10"}).append(
                    $("<input>", {
                        "type": "hidden", "id": "moduleBelong", "style": "width:400px;"
                    })
                )
            );
            var $row2 = $("<div>", {"class": "row form-group"}).append(
                $("<div>", {"class": "col-sm-2 required"}).text('引用消息格式'),
                $("<div>", {"class": "col-sm-10"}).append(
                    $("<input>", {
                        "type": "hidden", "id": "moduleMsgTemplateInput", "style": "width:400px;"
                    })
                )
            );
        ...
        //引用其他模块的消息格式
        reference_msgtemplate: function () {
            ...
            var dialogOpts = {
                title: '引用消息格式',
                message: _self.refrenceMsgTemplateDialogHtml(),
                buttons: {
                    confirm: {
                        label: "确定",
                        ...
                        callback: function (result) {
                            ...
                            var moduleDsUuid = $("#moduleMsgTemplateInput").val();
                            if (!moduleDsUuid) {
                                return false;
                            }
                            server.JDS.call({
                                ...
                                data: [[{
                                    ...
                                    entityClass: 'MessageTemplateRefEntity'
                                }]],
                                version: '',
                                success: function (result) {
                                    if (result.success) {
                                        commitResult = true;
                                        appModal.info('引用消息格式成功');
                                        _self.refresh();
                                    }
                                },
                                error: function (jqXHR) {
                ...
                shown: function () {
                    ...
                    $moduleSelect.on('change', function () {
                        ...
                        $("#moduleMsgTemplateInput").wSelect2({
                            valueField: "moduleMsgTemplateInput",
                            remoteSearch: false,
        ...
        btn_delete: function () {
            ...
            if (rowData.length > 0) {
                ...
                appModal.confirm("确认要删除消息格式吗?", function (result) {
                    if (result) {
                        server.JDS.call({
                            service: "messageTemplateService.deleteAllByUuids",
                            version: '',
                            data: [(function () {
                                var uuids = [];
        ...
        lineEnderButtonHtmlFormat: function (format, row, index) {
            ...
            $html.find(row.isRef === 1 ? '.btn_class_btn_delete' : '.btn_class_btn_cancel_ref').remove();
        ...
        btn_add: function () {
            ...
            // 触发消息格式列表行点击新增事件
            this.widget.trigger("AppMsgTemplateListView.editRow", {ui: this.widget});
        },

        // 行点击查看详情
        onClickRow: function (rowNum, rowData, $element, field) {
            // 触发消息格式列表行点击事件
            this.widget.trigger("AppMsgTemplateListView.editRow", {
                rowData: rowData,
                ui: this.widget
            });
        },
        ...
        btn_cancel_ref: function () {
            ...
            appModal.confirm("确认取消引用消息格式吗?", function (result) {
                if (result) {
                    server.JDS.call({
    ...
    return AppMessageTemplateListViewWidgetDevelopment;
});