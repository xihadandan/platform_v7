define(['constant', 'commons', 'server', 'appContext', 'appModal', 'AppPtMgrListViewWidgetDevelopment', 'AppPtMgrCommons'], function (
  ...
  // 平台管理_业务流程管理_过程节点办件列表_视图组件二开
  var AppBizProcessNodeInstanceListViewWidgetDevelopment = function () {
    AppPtMgrListViewWidgetDevelopment.apply(this, arguments);
  };

  // 接口方法
  commons.inherit(AppBizProcessNodeInstanceListViewWidgetDevelopment, AppPtMgrListViewWidgetDevelopment, {
    // 组件准备
    prepare: function () {},
    ...
    onClickRow: function (rowNum, rowData, $element, field) {
      var _self = this;
      // 查看过程结点
      var url = `/biz/process/node/instance/view?processNodeInstUuid=${rowData.uuid}`;
      var options = {};
      options.url = url;
  ...
  return AppBizProcessNodeInstanceListViewWidgetDevelopment;
});
