define(['constant', 'commons', 'server', 'HtmlWidgetDevelopment'], function (constant, commons, server, HtmlWidgetDevelopment) {
  var JDS = server.JDS;
  // 平台应用_基础数据管理_计时服务分类二开
  var AppTimeServiceCategoryDevelopment = function () {
    HtmlWidgetDevelopment.apply(this, arguments);
  };
  // 接口方法
  commons.inherit(AppTimeServiceCategoryDevelopment, HtmlWidgetDevelopment, {
    // 组件初始化
    init: function () {
      var $container = this.widget.element;
      getList();
      $('#addTSCategory', $container)
        .off()
        .on('click', function () {
          openDialog('新建计时服务分类');
        });

      $('#searchTSCategory', $container)
        .off()
        .on('click', function () {
          var val = $('#fullTSName', $container).val();
          getList(val);
        });

      $('#fullTSName', $container)
        .off()
        .on('keyup', function () {
          if ($(this).val() != '') {
            $('#deleteTSCategory', $container).show();
          } else {
            $('#deleteTSCategory', $container).hide();
          }
        })
      ...
      $('#deleteTSCategory', $container)
        .off()
        .on('click', function () {
          $('#fullTSName', $container).val('');
          $(this).hide();
          getList();
        });

      $('#ts_category_content', $container)
        .off()
        ...
        .on('click', function () {
          ...
          if ($(this).find('i').hasClass('icon-ptkj-youshouzhan')) {
            ...
            $(this)
              ...
              .animate({ width: width - 310 + 'px' }, 300);
            ...
            $(this)
              .parents('.msg-content-list')
              .find('.ui-wBootstrapTable')
              .animate({ width: width - 50 + 'px' }, 300);
            $(this).find('i').addClass('icon-ptkj-youshouzhan').removeClass('icon-ptkj-zuoshouzhan');
          }
        });

      $('#tsAllCategory', $container)
        .off('click')
        .on('click', function () {
          $('.msg-category-item', $container).removeClass('hasSelectCate');
          var msgFormateTable = $('#ts_category_content', $container).parents('.msg-content-list').find('.ui-wBootstrapTable').attr('id');
          $('#' + msgFormateTable).wBootstrapTable('removeParam', 'categoryUuid');
          $('#' + msgFormateTable)
            .wBootstrapTable('addParam', 'categoryUuid', 'all')
            .wBootstrapTable('refresh');
        });

      $('#ts_category_tree', $container)
        .off()
          ...
          success: function (result) {
            if (result.success || result.code == 0) {
              ...
              for (var i = 0; i < data.length; i++) {
                ...
                var className = '';
                if (data.length > 12 && i > data.length - 4) {
                  className = 'detail-up';
                }

                if (data[i].id != 'flowTiming') {
                  ifDeleteElement = "<li class='btn-delete'><i class='iconfont icon-ptkj-shanchu'></i>删除</li>";
                }
                ...
                lis +=
                  ...
                  "<ul class='category-item-operate' style='height:auto;'>" +
              ...
              $('#ts_category_tree', $container).html(lis);
            }
          }
        });
      }
      ...
      function getDetail(bean, uuid, $dialog) {
        $.ajax({
          url: ctx + '/api/ts/timer/category/get',
          data: { uuid: uuid },
          contentType: 'application/x-www-form-urlencoded',
          success: function (result) {
            if (result.success || result.code == 0) {
              ...
              // 判断 id='flowTiming' 不可编辑- 还原成可编辑
              $('#id').attr('readonly', true);
              // if (bean && bean.id && bean.id === 'flowTiming') {
              //   $('#name').attr('disabled', 'true');
              //   $('#id').attr('disabled', 'true');
              //   $('#addIcon').attr('disabled', 'true');
              //   $('#code').attr('disabled', 'true');
              //   $('#remark').attr('disabled', 'true');
              //   $('#selectIconBg').off('click');
              // }
            }
          }
        });
          ...
          buttons: {
            ok: {
              ...
              callback: function () {
                ...
                $.ajax({
                  url: ctx + '/api/ts/timer/category/save',
                  data: bean,
                  type: 'post',
                  dataType: 'json',
                  success: function (result) {
      ...
      function getDialogContent() {
        ...
        html +=
          ...
          '</tr>' +
          "<tr class='field'>" +
          "<td class='label-td' style='width:20%'><font style='color:#f00;'>*</font>ID</td>" +
      ...
      $('#ts_category_tree', $container).on('click', '.btn-edit', function (e) {
        e.stopPropagation();
        var uuid = $(this).parents('.msg-category-item').data('uuid');
        openDialog('编辑计时服务分类', uuid);
      });

      $('#ts_category_tree', $container).on('click', '.btn-delete', function (e) {
        e.stopPropagation();
        ...
        appModal.confirm('确定删除分类吗', function (success) {
          if (success) {
            $.ajax({
              url: ctx + '/api/ts/timer/category/deleteAll',
              type: 'post',
              data: { uuids: [uuid] },
              dataType: 'json',
              success: function (result) {
                if (result.code == 0) {
                  ...
                  $el.remove();
                  $('#tsAllCategory', $container).trigger('click');
                } else {
                  appModal.error(result.msg);
                }
              }
  ...
  return AppTimeServiceCategoryDevelopment;
});
