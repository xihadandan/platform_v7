define(['constant', 'commons', 'server', 'appContext', 'appModal', 'AppPtMgrListViewWidgetDevelopment', 'AppPtMgrCommons'], function (
  ...
  // 平台管理_产品集成_模块_权限列表_视图组件二开
  var AppSecurityPrivilegeListViewWidgetDevelopment = function () {
    AppPtMgrListViewWidgetDevelopment.apply(this, arguments);
  };

  // 接口方法
  commons.inherit(AppSecurityPrivilegeListViewWidgetDevelopment, AppPtMgrListViewWidgetDevelopment, {
    // 组件准备
    prepare: function () {},
    ...
    afterRender: function (options, configuration) {
      var _self = this;
      this.widget.on('AppSecurityPrivilegeListView.refresh', function () {
        _self.refresh();
      });
    },
    ...
    btn_delete: function () {
      ...
      if (rowData.length > 0) {
        ...
        appModal.confirm('确认要删除权限吗?', function (result) {
          if (result) {
            var uuids = [];
            ...
            server.JDS.restfulPost({
              url: `/proxy/api/security/privilege/removeAll`,
              data: uuids,
    ...
    btn_add: function () {
      // 触发权限列表行点击新增事件
      this.widget.trigger('AppSecurityPrivilegeListView.editRow', {
        ui: this.widget
      });
    },
  ...
  return AppSecurityPrivilegeListViewWidgetDevelopment;
});
