...
class WidgetDyformDevelopment extends VueWidgetDevelopment {
   ...
   * @param {刷新参数} options
   */
  setFieldOptionsRefresh(code, options = {}) {
    this.dyform.setFieldOptionsRefresh(code, options);
  }
  ...
  clearFormData(formUuid) {
    this.wDyform.clearFormData(formUuid);
  }
   ...
   * @param {*} fieldName 字段编码
   * @param {*} options 刷新参数
   * @param {*} rowKey 有值则刷新该行字段选项，无值刷新全部
   */
  setSubformFieldOptionsRefresh(formId, fieldName, options = {}, rowKey) {
    this.dyform.setSubformFieldOptionsRefresh(formId, fieldName, options, rowKey);
  }
