...
(function (global, factory) {
  ...
  function (global) {
    ...
    function WpsStart(clientType, name, func, param, useHttps, callback, tryCount, bPop) {
      ...
      if (useHttps) url = 'https://127.0.0.1:58891/' + clientType + '/runParams';
    ...
    function WpsStartWrapExInner(clientType, name, func, param, useHttps, callback, tryCount, bPop) {
      ...
      if (useHttps) rspUrl = 'https://127.0.0.1:58891/transferEcho/runParams';
      var time = new Date();
      var cmdId = 'js' + time.getTime() + '_' + exId;
      var infocontent = JSON.stringify(param.param);
      var funcEx = 'var res = ' + func;
      var cbCode =
        "var xhr = new XMLHttpRequest();xhr.open('POST', '" + rspUrl + "');xhr.send(JSON.stringify({id: '" + cmdId + "', response: res}));"; //res 为func执行返回值
      ...
      if (useHttps) url = 'https://127.0.0.1:58891/transfer/runParams';
      var data = 'ksowebstartup' + clientType + '://' + baseData;
      var wrapper = {
        id: cmdId,
        app: clientType,
    ...
    function WpsStartWrapVersionInner(clientType, name, func, param, useHttps, callback) {
      if (serverVersion == 'wait') {
        ...
        if (useHttps) url = 'https://127.0.0.1:58891/version';
        startWps({
          url: url,
          data: '',
