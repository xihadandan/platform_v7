...
jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
	init: function(el, options) {
		this.settings = jQuery.extend(true, {
			...
			resize_timeout: 500,
			change_timeout: 150,
			fgcolor: {a:'#44D89F',c:'#6395F9',d:'#F6AAA0',  // color for differences (soft color)
				ca:'#fff',cc:'#fff',cd:'#fff'},    // color for currently active difference (bright color)
			bgcolor: '#eee',
			...
			resize: function(init) {
				...
				var content_width = w / 2.0 - 2 * 8;
				var content_height = h;
				var self = jQuery(el);
				self.find('.mergely-column').css({ width: content_width + 'px' });
				self.find('.mergely-column').css({ height: content_height + 'px' });
				self.find('.mergely-canvas, .mergely-margin, .mergely-column textarea, .CodeMirror-scroll, .cm-s-default').css({ height: content_height - 42 + 'px' });
	...
	bind: function(el) {
		...
		var editorLhs = jQuery(`<div style="position:relative;width:'${width}'; height:'${height}'" id="editor-lhs" class="mergely-column"><div class="mergely-column-title">操作前</div><textarea id="text-lhs"></textarea></div>`);
		...
		var editorRhs = jQuery(`<div style="width:'${width}'; height:'${height}'" id="editor-rhs" class="mergely-column"><div class="mergely-column-title">操作后</div><textarea id="text-rhs"></textarea></div>`);
	...
	_is_change_in_view: function(side, vp, change) {
		if(change.op == "c" && change['rhs-line-to'] == 0 && change['lhs-line-to'] > 0){
			return true;
		}
		if(change.op == "c" && change['rhs-y-end'] >= change['lhs-y-end']){
			return false;
		}
					...
					lcs.diff(
						function added (from, to) {
							if (self._is_change_in_view('rhs', rhsvp, change)) {
								...
								marktext.push([red, {line:k, ch:from}, {line:k, ch:to}, {className: 'mergely ch e rhs'}]);
							}
						},
						function removed (from, to) {
							if (self._is_change_in_view('lhs', lhsvp, change)) {
								...
								marktext.push([led, {line:j, ch:from}, {line:j, ch:to}, {className: 'mergely ch e lhs'}]);
							}
						}
					);
				}
