    ...
    */
    ...
    Class.prototype.list = function (type, index) {
        ...
        if (type === 'year') { //年列表
            ...
            lay(elemYM[isCN ? 0 : 1]).attr('lay-ym', listYM[0] + '-' + listYM[1])
                .html(listYM[0] + text);
        } else if (type === 'week') { //周列表
            lay.each(new Array(15), function (i) {
                var li = lay.elem('li', {
                    'lay-ym': i
                }), ymd = { year: listYM[0], week: i };
                i + 1 == listYM[1] && lay(li).addClass(THIS);
                li.innerHTML = isCN ? '第' + i + '周' : i + 'weeks';
                ul.appendChild(li);
                if (listYM[0] < that.firstDate.year) {
                    ymd.date = options.min.date;
                } else if (listYM[0] >= that.firstDate.year) {
        ...
        if (type === 'year' || type === 'month' || type === 'week') {
            ...
            lay(ul).find('li').on('click', function () {
                ...
                if (options.type === 'year' || options.type === 'month' || options.type === 'week') {
                    ...
                    } else if (options.type === 'week' && type === 'year') {
                        that.listYM[index][0] = ym;
                        isAlone && (that[['startDate', 'endDate'][index]].year = ym);
                        that.list('week', index);
                    } else {
    ...
    Class.prototype.change = function (index) {
        var that = this
            , options = that.config
            , dateTime = options.dateTime
            , isAlone = options.range && (options.type === 'year' || options.type === 'month' || options.type === 'week')
