...
!(function () {
  ...
  var isLayui = window.layui && layui.define,
    ...
    lay = function (selector) {
      return new LAY(selector, arguments);
    },
    //DOM构造器
    LAY = function (selector, args) {
      var index = 0,
        nativeDOM =
        typeof selector === 'object' ? [selector] :
        ((this.selector = selector),
          args[1] && args[1].length > 0 ? args[1][0].querySelectorAll(selector || null) : document.querySelectorAll(selector || null));
  ...
  */
  ...
  LAY.prototype.on = function (eventName, fn) {
    return this.each(function (index, item) {
      item = typeof item == 'object' && item[0] ? item[0] : item;
      try {
        if (item) {
          item.attachEvent ?
  ...
  */
  ...
  Class.prototype.init = function () {
    ...
    //默认赋值
    if (options.value && options.isInitValue && !options.autoWeek) {
  ...
  Class.prototype.render = function () {
    ...
    lay('#' + that.elemID, options.scope || document)
      .off()
      .on(
        'mousedown',
        function (e) {
          options.elem[0].focus();
          // e.preventDefault();
          if (document.all) {
            //兼容ie
  ...
  Class.prototype.position = function () {
    ...
    if (top + elemHeight + margin > winArea()) {
      top =
        ...
        ?
        rect.top - elemHeight - elem.offsetHeight :
        winArea() - elemHeight;
      top = top - margin * 2;
    }
  ...
  Class.prototype.limit = function (elem, date, index, time) {
    ...
    if (options.range) {
      $(options.elem).trigger("changeStatus");
    }
  ...
  Class.prototype.list = function (type, index) {
    ...
    if (type === 'year') {
      ...
      lay.each(new Array(12), function (i) {
        ...
        div.innerHTML = lang.month[i] + (isCN ? '月' : '');
        li.appendChild(div);
        ul.appendChild(li);
      ...
      var renderTimeArr = [];
      var renderTimeType = '';
      if (that.config.format.indexOf('ss') > -1) {
        renderTimeArr = [24, 60, 60];
        renderTimeType = 'time-hms';
      } else if (that.config.format.indexOf('mm') > -1) {
        renderTimeArr = [24, 60];
        renderTimeType = 'time-hm';
      } else {
        renderTimeArr = [24];
        renderTimeType = 'time-h';
      }
      ul.classList.add(renderTimeType);
      lay.each(renderTimeArr, function (i, item) {
        var li = lay.elem('li'),
        ...
        .on('click', function () {
          ...
          if (index === 0) {
            ...
            if (isAlone) {
              ...
              //date/datetime类型
              var YM = type === 'year' ? that.getAsYM(ym, listYM[1] - 1, 'sub') : that.getAsYM(listYM[0], ym + 1, 'sub');
              lay.extend(dateTime, {
                year: YM[0],
                month: YM[1]
              });
        ...
        scroll = function () {
          ...
          lay(ul)
            ...
            .each(function (i) {
              var ol = this,
                li = lay(ol).find('li');
              ol.scrollTop = 30 * that[startEnd][hms[i]];
              if (ol.scrollTop <= 0) {
              ...
              if (options.range) {
                ...
                if (_start > _end) {
                  if (startEnd === 'startTime') {
                    that['endTime'] = $.extend({}, that[startEnd]);
                    if ($('.laydate-time-list').length > 1) {
                      that.list('time', 1);
                    }
                    that.setSwitchVal('endTime', that[startEnd]);
                  } else {
                    that['startTime'] = $.extend({}, that[startEnd]);
                    if ($('.laydate-time-list').length > 1) {
                      that.list('time', 0);
                    }
                    that.setSwitchVal('startTime', that[startEnd]);
                  }
                }
                that.setSwitchVal(startEnd, that[startEnd]);
              } else {
                dateTime[hms[i]] = value;
                if (options.type === 'datetime') {
                  that.setSwitchVal('startTime', dateTime);
                }
              }
              lay(ol)
                .find('.' + THIS)
  ...
  //关闭列表
  Class.prototype.closeList = function (index) {
    var that = this,
      options = that.config;
    lay.each(that.elemCont, function (i, item) {
      if (!index || index == i) {
        lay(this)
          .find('.' + ELEM_LIST)
          .remove();
        lay(that.elemMain[i]).removeClass('laydate-ym-show laydate-time-show');
        lay(that.elemMain[i])
          .find('.' + ELEM_TIME_TEXT)
          .remove();
      }
    ...
    lay.each(tds, function (i, item) {
      ...
      if (startTime !== endTime) {
        if (thisTime === startTime) {
          lay(item).addClass('range-start').removeClass('range-end');
        }
        if (thisTime === endTime) {
          lay(item).addClass('range-end').removeClass('range-start');
        }
      } else {
        if (thisTime === startTime) {
          lay(item).addClass('range-start range-end');
        }
      }
    });
  };
    ...
    var that = this,
      ...
      active = {
        ...
        confirm: function () {
          if (options.range) {
            if (!that.endDate) {
              that.setSwitchEndData();
            }
            if (!that.endDate) return that.hint('请先选择日期范围');
            if (lay(btn).hasClass('laydate-not-same')) return false;
  ...
  Class.prototype.change = function (index) {
    ...
    return {
      prevYear: function () {
        ...
        if (lay('.laydate-prev-y', options.scope || document)[index].className.indexOf('disabled') > -1) return;
        dateTime.year--;
        that.checkDate('limit').calendar(dateTime, index);
        options.range || that.done(null, 'change');
      },
      prevMonth: function () {
        if (lay('.laydate-prev-m', options.scope || document)[index].className.indexOf('disabled') > -1) return;
      ...
      nextMonth: function () {
        if (lay('.laydate-next-m', options.scope || document)[index].className.indexOf('disabled') > -1) return;
      ...
      nextYear: function () {
        ...
        if (lay('.laydate-next-y', options.scope || document)[index].className.indexOf('disabled') > -1) return;
        dateTime.year++;
        that.checkDate('limit').calendar(dateTime, index);
        options.range || that.done(null, 'change');
      }
  ...
  Class.prototype.changeEvent = function () {
    ...
    lay('.switch-ctl', options.scope || document).on('focus', function () {
      var _this = this;
      var _id = _this.id;
      var type = _id.split('_')[0];
      var index = _id.split('_')[2];
      if (type === 'time') {
        that.list('time', index);
      } else {
        that.closeList(index);
      }
    });
  };
  ...
  Class.prototype.isInput = function (elem) {
    if (elem.tagName) {
      ...
      return /input|textarea/.test(elem[0].tagName.toLocaleLowerCase());
    }
  };
  ...
  Class.prototype.events = function () {
    var that = this,
      ...
      showEvent = function (elem, bind) {
        elem.on(options.trigger, function () {
          ...
          if ($(that.bindElem).attr("readonly") && $(that.bindElem).hasClass("editableClass")) {
            return false;
          }
          that.render();
        });
    ...
    //绑定关闭控件事件
    // if (!lay(options.elem[0]).hasClass('editableClass')) {
    //   lay(options.elem[0]).on('blur', function () {
    //     lay('#' + that.elemID).remove();
    //   })
    // }
    ...
    lay(top.document)
      .on('click', function (e) {
        if (
          ...
          !lay('#' + that.elemID, options.scope || top.document).length
        ) {
          return;
        }
        that.remove();
