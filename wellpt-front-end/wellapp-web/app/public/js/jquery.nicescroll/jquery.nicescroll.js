  ...
  var NiceScrollClass = function (myopt, me) {
    ...
    function getWidthToPixel(dom, prop, chkheight) {
      ...
      if (isNaN(px)) {
        ...
        var brd =
          px == 3 ? (chkheight ? self.win.outerHeight() - self.win.innerHeight() : self.win.outerWidth() - self.win.innerWidth()) : 1; //DON'T TRUST CSS
            ...
            self.observerbody = new ClsMutationObserver(function (mutations) {
              mutations.forEach(function (mut) {
                if (mut.type == 'attributes') {
                  return $body.hasClass('modal-open') && $body.hasClass('modal-dialog') && !$.contains($('.modal-dialog')[0], self.doc[0])
                    ? self.hide()
    ...
    this.onmousewheel = function (e) {
      ...
      // bug#46120: 树形下拉在页面底部的位置时，鼠标在下拉框内滚动上拉会和外部页面的滚动条一起上拉
      if ($(e.target).closest('.well-select ~ .w-zTree-content').length) {
        return false;
      }
