    ...
    var loadSnippetFile = function (id) {
        ...
        config.loadModule(snippetFilePath, function (m) {
            if (m) {
                ...
                if (m.scope === 'javascript') {
                    m.includeScopes = [];
                    //判断可用API
                    if (window.appPageDesigner) {
                        //cms
                        m.includeScopes.push('cms_api_snippets');
                    }
                    if (window.dyform_constant || window.parent.dyform_constant) {
                        //dyform
                        m.includeScopes.push('dyform_api_snippets');
                    }


                }
                if (m.includeScopes && snippetManager.snippetMap[m.scope]) {
