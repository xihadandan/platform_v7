					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						lcdata[nm] = cm.formatter && typeof(cm.formatter) === 'string' && cm.formatter == 'date' ? $.unformat.date(data[nm],cm) : data[nm];
						 if(cm.edittype == "fileupload"){
							 ...
							 el.innerHTML="<div>" + data[cm.name].length + "个附件</div>"; 
							 
							 
							 var ctlID =   rowid + "_" + cm.name;
							  ...
							  var fileupload = new WellFileUpload(ctlID);
							   ...
							   //WellFileUpload.files[ctlID+"_tmp"] = data[cm.name];
							   
							   ////console.log("tttt:" + JSON.stringify( WellFileUpload.tmp[ctlID]));
							  // //console.log(ctlID + ": "+ JSON.stringify( WellFileUpload.tmp[ctlID]));
							  // //console.log();
							  fileupload.init(false,  $( $(el).find("div")[0]),  false , true, data[cm.name]);
							  
								//  //console.log(WellFileUpload.files[ctlID].length);
								//$( $(el).find("div")[0]).html(WellFileUpload.files[ctlID].length + "个附件"); 
						...
						//console.log(cm.edittype);////////////////////////////////////formatter/////
			...
			case "custom" :
				...
				try {
					if($.isFunction(options.custom_element)) {
						var celm = options.custom_element.call(this,vl,options);//////////////custom_element//////////
						//console.log();
						if(celm) {
					...
					//if($.isFunction(options.custom_element)) {
						...
						//var celm = options.custom_element.call(this,vl,options);//////////////custom_element//////////
						 ...
						 var el = document.createElement("div");
						  el.type="text";
						  el.value = vl;
						  var ctlID =   options.id;
						  ////console.log(JSON.stringify(vl));
						  //console.log("custom_element:" + ctlID);
						  var fileupload = new WellFileUpload(ctlID);
						  var files = [];
						   var fileList = [];
						  
						  if(WellFileUpload.files[ctlID] && WellFileUpload.files[ctlID].length > 0){
							  files = WellFileUpload.files[ctlID];
							  fileList = files.slice(0);
						  }else{
							  
								if(WellFileUpload.files[ctlID+"_tmp"]){
									 files = WellFileUpload.files[ctlID+"_tmp"];
									 fileList = files.slice(0); 
									 WellFileUpload.files[ctlID+"_tmp"].length = 0; 
								} 
		...
		return this.each(function(){
			...
			if(fr >= 0) {
				...
				$.each($t.p.colModel, function(i,n){
					...
					if(this.edittype != "fileupload" //如果是附件的，则不做处理
					
						&&this.editable === true && this.name in $t.p.savedRow[fr] && !$(this).hasClass('not-editable-cell')) {
						ares[this.name] = $t.p.savedRow[fr][this.name];
					}
					/*
					if(this.edittype == "fileupload"){
						var files = WellFileUpload.files[rowid + "_" +  this.name]; 
						var length = 0;
						if(files){
							length = files.length;
						}else{
