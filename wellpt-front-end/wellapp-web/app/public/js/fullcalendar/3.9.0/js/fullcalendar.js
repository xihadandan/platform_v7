            ...
            function buildSlotSegLevels(segs) {
                ...
                for (i = 0; i < segs.length; i++) {
                    ...
                    for (j = 0; j < levels.length; j++) {
                        if (!computeSlotSegCollisions(seg, levels[j],"level").length) {
                            break;
                        }
                    }
                    seg.level = j;
            ...
            function computeForwardSlotSegs(levels) {
                ...
                for (i = 0; i < levels.length; i++) {
                    ...
                    for (j = 0; j < level.length; j++) {
                        ...
                        seg.forwardSegs = [];
                        if(j<level.length - 1){
                            for (h = j + 1; h < level.length; h++) {
                                if(isSlotSegCollision1(seg,level[h])){
                                    seg.forwardSegs.push(level[h])
                                }
                            }
                        }else{
                            for (k = i + 1; k < levels.length; k++) {
                                computeSlotSegCollisions(seg, levels[k],"forward", seg.forwardSegs);
                            }
                        }
                    }
                }
...
// Append into an optionally-supplied `results` array and return.
            function computeSlotSegCollisions(seg, otherSegs, type, results) {
                if (results === void 0) { results = []; }
                for (var i = 0; i < otherSegs.length; i++) {
                    if (type=="level" && isSlotSegCollision(seg, otherSegs[i])) {
                        results.push(otherSegs[i]);
                    }else if (type=="forward" && isSlotSegCollision1(seg, otherSegs[i])){
                        results.push(otherSegs[i]);
                    }
                }
                return results;
...
// Do these segments occupy the same vertical space?
            function isSlotSegCollision(seg1, seg2) {
                return parseInt(seg1.startMs/1000/60) > parseInt(seg2.startMs/1000/60);
            }

            function isSlotSegCollision1(seg1, seg2) {
                return parseInt(seg1.startMs/1000/60) == parseInt(seg2.startMs/1000/60);
            }
