...
var Context = /** @class */ (function () {
	...
	Context.prototype.insertText = function( text ){
    	if(this.modules.editor) {
	    	var lastRange = this.modules.editor.lastRange;
	    	if(lastRange) {
	    		lastRange.select();
	    	}
	    	this.modules.editor.insertNode(dom.createText(text));
    	} else {
    		log.error("editor is no exists, invoke method of code");
    		this.code(text);
    	}
	};
	
	Context.prototype.insertHtml = function( html ){
    	if(this.modules.editor) {
        	var lastRange = this.modules.editor.lastRange;
        	if(lastRange) {
        		lastRange.select();
        	}
        	var contentsContainer = $('<div></div>').html(html)[0];
            var childNodes = contentsContainer.childNodes;
            for(var i = 0; i < childNodes.length; i++) {
            	this.modules.editor.insertNode(childNodes[i]);
            }
    	} else {
    		log.error("editor is no exists, invoke method of code");
    		this.code(html);
