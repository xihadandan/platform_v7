<template>
  <!-- 环节事件监听 -->
  <w-tree-select
    class="workflow-tree-select"
    :value="directionListener"
    :treeData="treeData"
    :treeCheckable="treeCheckable"
    :replaceFields="{
      children: 'children',
      title: 'name',
      key: 'id',
      value: 'id'
    }"
    @change="changeDirectionListener"
  />
</template>
<script>
import WTreeSelect from '../components/w-tree-select';
export default {
  name: 'DirectionListenerTreeSelect',
  props: {
    value: {
      type: String
    },
    treeCheckable: {
      type: Boolean,
      default: false // 显示checkbox, true时multiple为true表示多选
    }
  },
  components: {
    WTreeSelect
  },
  computed: {
    directionListener() {
      return this.value;
    }
  },
  data() {
    return {
      treeData: []
    };
  },
  created() {
    this.getDirectionListeners();
  },
  methods: {
    // 获取流向事件监听
    getDirectionListeners() {
      const params = {
        args: JSON.stringify([-1]),
        methodName: 'getDirectionListeners',
        serviceName: 'flowSchemeService'
      };
      this.$axios
        .post('/json/data/services', {
          ...params
        })
        .then(res => {
          if (res.status === 200) {
            if (res.data && res.data.code === 0) {
              const data = res.data.data;
              this.treeData = data;
            }
          }
        });
    },
    changeDirectionListener(value, label, extra) {
      this.$emit('input', value);
      this.$emit('change', ...arguments);
    }
  }
};
</script>
