...
export default {
  ...
  computed: {
    options() {
      let options = [];
      if (this.beforeOptions && this.beforeOptions.length) {
        options = this.beforeOptions;
      }
      if (this.graph.instance) {
        options = options.concat(this.graph.instance.tasksData);
        if (this.hasSubFlow && this.graph.instance.subflowsData.length) {
          options = options.concat(this.graph.instance.subflowsData);
        }
      }
      if (this.afterOptions && this.afterOptions.length) {
        options = options.concat(this.afterOptions);
      }
      return options;
    },
    currentValue() {
