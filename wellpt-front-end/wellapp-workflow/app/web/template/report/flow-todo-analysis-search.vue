<template>
  <div>
    <flow-report-head title="待办流程分析" @onSearchClick="onSearchClick" @onResetClick="onResetClick">
...
export default {
  ...
  beforeMount() {
    ...
    this.pageContext.handleEvent('showDeptCustomSearchOfFlowTodo', () => {
      ...
      if (!this.loadedDeptCustomSearchOfFlowTodo) {
        setTimeout(() => {
          this.pageContext.emitEvent('searchDeptFlowTodo', this.searchForm);
        }, 500);
  ...
  mounted() {
    setTimeout(() => {
      this.pageContext.emitEvent('searchUserFlowTodo', this.searchForm);
    }, 500);
  },
  beforeDestroy() {
    this.pageContext.offEvent('searchUserFlowTodo');
    this.pageContext.offEvent('searchDeptFlowTodo');
  },
  methods: {
