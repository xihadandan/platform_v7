...
export default {
  ...
  beforeMount() {
    this.pageContext.handleEvent('searchDeptFlowTodo', searchForm => {
    ...
    loadDataset(searchForm = {}) {
      ...
      $axios
        .post('/json/data/services', {
          ...
          args: JSON.stringify(['com.wellsoft.pt.app.workflow.report.dataset.FlowTodoByDeptSummaryEchartDatasetLoad', { ...searchForm }])
        })
        .then(({ data: result }) => {
          this.$loading(false);
          if (result.data) {
            result.data.forEach(item => {
              item.source &&
                item.source.forEach(sourceItem => {
                  ...
                  } else if (sourceItem.hasOwnProperty('maxFlowTodoCountByDeptGrouyByFlow')) {
