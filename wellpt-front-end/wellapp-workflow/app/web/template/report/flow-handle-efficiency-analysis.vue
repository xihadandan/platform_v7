<template>
  <div>
    <flow-report-head title="办理效率分析" @onSearchClick="onSearchClick" @onResetClick="onResetClick">
...
export default {
  ...
  beforeMount() {
    ...
    this.pageContext.handleEvent('showUserCustomSearchOfFlowHandleEfficiency', () => {
      this.showUserTodo = true;
    });
    this.loadedDeptFlowHandleEfficiency = false;
    this.pageContext.handleEvent('showDeptCustomSearchOfFlowHandleEfficiency', () => {
      this.showUserTodo = false;
      if (!this.loadedDeptFlowHandleEfficiency) {
        setTimeout(() => {
          this.pageContext.emitEvent('searchDeptFlowHandleEfficiency', this.searchForm);
        }, 500);
        this.loadedDeptFlowHandleEfficiency = true;
      }
    });
  },
  mounted() {
    setTimeout(() => {
      this.pageContext.emitEvent('searchUserFlowHandleEfficiency', this.searchForm);
    }, 500);
  },
  beforeDestroy() {
    this.pageContext.offEvent('searchUserFlowHandleEfficiency');
    this.pageContext.offEvent('searchDeptFlowHandleEfficiency');
  },
  methods: {
...
<style lang="less">
div[w-id='page_flow_handle_efficiency_report'] {
  .widget-vpage-content {
    background-color: var(--w-widget-page-layout-bg-color) !important;

    div[w-id='BrEIrlCuSIBeKICzpPWNfcfxzAAkrnAg'] {
      border-radius: 2px;
    }
  }
}
