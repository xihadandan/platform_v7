...
class FlowHandleEfficiencyByUserChartDevelopment extends FlowChartDevelopmentBase {

  get META() {
    return {
      name: '流程统计分析_办理效率分析按人员统计',
      hook: {
        itemClick: '查看详情',
      }
    }
  ...
  created() {
    ...
    this.getSetting().then(setting => {
      this.maxBarCount = setting.flowHandleEfficiency && setting.flowHandleEfficiency.maxUserBarCount || 12;
    });
  }

  mounted() {
    this.getPageContext().handleEvent('searchUserFlowHandleEfficiency', searchForm => {
      let params = {};
  ...
  itemClick($event) {
    ...
    let title = `人员办理效率 - ${$event.name}`;
    _this.getPageContext().emitEvent('FfdIBzXAaevnxIUSvcLdtXCUgBHpFbsj:showDrawer', {
      eventParams: {
        ..._this.$widget.params,
        userId,
        title
    ...
    options.toolbox = {
      feature: {
        myTool1: {
          ...
          onclick: function () {
            _this.getPageContext().emitEvent('UOCOJEdmnIMlhxztzVzidNxVWBHiLyYV:showDrawer', {
              eventParams: {
                ..._this.$widget.params,
...
export default FlowHandleEfficiencyByUserChartDevelopment;
