...
class FlowHandleOverdueOfCountByDeptChartDevelopment extends FlowChartDevelopmentBase {

  get META() {
    return {
      name: '流程统计分析_办理逾期分析_部门逾期次数',
      hook: {
        itemClick: '查看详情',
      }
    }
  ...
  created() {
    ...
    this.getSetting().then(setting => {
      this.maxBarCount = setting.flowHandleOverdue && setting.flowHandleOverdue.maxDeptBarCount || 12;
    });
  }

  mounted() {
    this.getPageContext().handleEvent('searchDeptFlowHandleOverdue', searchForm => {
      let params = {};
  ...
  itemClick($event) {
    ...
    let title = `部门逾期明细 - ${$event.name}`;
    _this.getPageContext().emitEvent('DdFVgrVkXDbeLCpLKoPovoWwSckWFISd:showDrawer', {
      eventParams: {
        ..._this.$widget.params,
        deptId,
        title
    ...
    options.toolbox = {
      feature: {
        myTool1: {
          ...
          onclick: function () {
            _this.getPageContext().emitEvent('DulaNKKpwHarGXuQCiyfiTyhrYnzFVlB:showDrawer', {
              eventParams: {
                ..._this.$widget.params,
...
export default FlowHandleOverdueOfCountByDeptChartDevelopment;
