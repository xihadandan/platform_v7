...
class FlowTodoCountByDeptChartDevelopment extends FlowChartDevelopmentBase {

  get META() {
    return {
      name: '流程统计分析_待办流程分析按部门_待办数量排名',
      hook: {
        itemClick: '查看详情',
      }
    }
  ...
  mounted() {
    this.getPageContext().handleEvent('searchDeptFlowTodo', searchForm => {
      let params = {};
  ...
  itemClick($event) {
    ...
    let title = `部门待办 - ${$event.name}`;
    _this.getPageContext().emitEvent('LyKyPzwKDyoKvUtONCglYHzyXpTpKQUT:showDrawer', {
      eventParams: {
        ..._this.$widget.params,
        deptId,
        title
    ...
    options.toolbox = {
      feature: {
        myTool1: {
          ...
          onclick: function () {
            _this.getPageContext().emitEvent('rtnLZPIRoBIpHagsytaqBFtaTPORByTF:showDrawer', {
              eventParams: {
                ..._this.$widget.params,
...
export default FlowTodoCountByDeptChartDevelopment;
