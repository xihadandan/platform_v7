...
class FlowHandleOverdueOfCountByUserChartDevelopment extends FlowChartDevelopmentBase {

  get META() {
    return {
      name: '流程统计分析_办理逾期分析_人员逾期次数',
      hook: {
        itemClick: '查看详情',
      }
    }
  ...
  mounted() {
    ...
    this.getPageContext().handleEvent('HdCoJppgJgrMhBjtWcHXgPEHWVuIzdZO:drawer:afterVisibleChange',
      ({ visible, getContainer }) => {
        if (!visible) {
          getContainer().style.overflowX = 'visible';
        }
      })
  }
    ...
    options.toolbox = {
      feature: {
        myTool1: {
          ...
          onclick: function () {
            _this.getPageContext().emitEvent('UFFDYHPyBskzdeEsOavdCPkeJlxTllZS:showDrawer', {
              eventParams: {
                ..._this.$widget.params,
...
export default FlowHandleOverdueOfCountByUserChartDevelopment;
