...
class FlowEfficiencyAvgCompletedTimeChartDevelopment extends FlowChartDevelopmentBase {
  ...
  created() {
    ...
    this.getSetting().then(setting => {
      this.maxBarCount = setting.flowEfficiency && setting.flowEfficiency.maxTaskBarCount || 12;
    });
  }
  ...
  beforeChartRender(options) {
    ...
    options.tooltip = Object.assign(options.tooltip || {}, {
      ...
      formatter: (params, ticket) => {
        let param = params[0];
        let avgTaskDurationDay = 0;
        let avgTaskDurationHour = 0;
        let avgTaskDurationMinute = 0;
        let avgTaskDurationInMinute = parseFloat(param.data.avgTaskDurationInMinute);
        if (avgTaskDurationInMinute >= 60 * 24) {
          avgTaskDurationDay = parseInt(avgTaskDurationInMinute / (60 * 24));
        }
        if (avgTaskDurationInMinute >= 60) {
          avgTaskDurationHour = parseInt((avgTaskDurationInMinute - avgTaskDurationDay * 60 * 24) / 60);
        }
        avgTaskDurationMinute = avgTaskDurationInMinute % 60;
        let timeLabel = "";
        if (avgTaskDurationDay > 0) {
          timeLabel += avgTaskDurationDay + "天";
        }
        if (avgTaskDurationHour > 0) {
          timeLabel += avgTaskDurationHour + "小时";
        }
        timeLabel += avgTaskDurationMinute + "分钟";
    ...
    options.toolbox = {
      feature: {
        myTool1: {
          ...
          onclick: function () {
            _this.getPageContext().emitEvent('eQzAOCJwfqfgaUhaVqAcoamEQPcXRVno:showDrawer', {
              eventParams: {
                ..._this.$widget.params,
