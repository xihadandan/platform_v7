...
class FlowInstanceCountByCategoryChartDevelopment extends FlowChartDevelopmentBase {

  get META() {
    return {
      name: '流程统计分析_流程实例发起数量按流程分类统计图表二开',
      hook: {
        itemClick: '查看详情',
      }
    }
  ...
  created() {
    ...
    this.getSetting().then(setting => {
      this.maxBarCount = setting.flowInstCount && setting.flowInstCount.maxFlowBarCount || 12;
    });
  }
  ...
  itemClick($event) {
    const _this = this;
    let categoryUuid = $event.data.categoryUuid;
    let title = `${_this.$widget.configuration.titleConfig.text} - ${$event.name}`;
    _this.getPageContext().emitEvent('XcDWZWLjfnXOflgYOeiymSpjSyMVLNSs:showDrawer', {
      eventParams: {
        ..._this.$widget.params,
        categoryUuid,
        title
  ...
  beforeChartRender(options) {
    ...
    options.tooltip = Object.assign(options.tooltip || {}, {
      ...
      formatter: (params, ticket) => {
        let arrays = [...params];
        arrays.reverse();
        let label = arrays[0].axisValueLabel + "<br/>";
        arrays.forEach(param => {
          label += param.marker + param.seriesName + "<span style='padding-left:20px' />" + param.data.数量 + "<br/>";
        });
        return label;
      }
    });
