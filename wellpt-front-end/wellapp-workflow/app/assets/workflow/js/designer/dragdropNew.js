...
dd.getCmd = function(d_o, d_cmd, d_cmdStr) {
    var d_i = d_o.id.indexOf(d_cmd), d_j, d_y = (d_i >= 0) * 1;
    if (d_y) {
...
dd.getCmdVal = function(d_o, d_cmd, d_cmdStr, int0) {
    var d_i = d_o.id.indexOf(d_cmd), d_j, d_y = (d_o.id.indexOf(d_cmd) >= 0) ? dd.Int(d_o.id.substring(d_o.id.indexOf(d_cmd) + d_cmd.length)) : int0 ? -1 : 0;
...
dd.addElt = function(d_o, d_p) {
    ...
    if (d_p) {
        d_p.copies[d_p.copies.length] = d_p.copies[d_o.name] = d_o;
    }
}
;
dd.mkWzDom = function() {
...
dd.addProps = function(d_o) {
    ...
    if (d_o.is_image) {
        ...
        if (d_o.div && typeof d_o.div.style != "undefined") {
            d_o.css = d_o.div.style;
        }
        ...
        if (dd.n6 && d_o.div) {
            if ((d_c = d_o.bgColor).indexOf("rgb") >= 0) {
                ...
                d_o.bgColor = "#";
                for (var d_i = 0; d_i < d_c.length; d_i++) {
                    d_o.bgColor += parseInt(d_c[d_i]).toString(16);
                }
            } else {
                d_o.bgColor = d_c;
            }
...
dd.initz = function() {
    ...
    if (dd.op6) {
        ...
        if (dd.n6 || dd.ie || dd.op && !dd.op6 || dd.w3c) {
            dd.recalc(1);
        }
    }
    ...
    if (d_drag && document.onmousemove != DRAG) {
        ...
        if (d_resize && document.onmousemove != RESIZE) {
...
function WINSZ(d_x) {
    if (d_x) {
        if (dd.n4 || dd.op6 && d_x & 2) {
            ...
            dd.iH = innerHeight;
            if (dd.op6) {
                setTimeout("WINSZ()", 511);
            }
        }
        window.onresize = new Function("WINSZ();");
    } else {
        if ((dd.n4 || dd.op6) && (innerWidth != dd.iW || innerHeight != dd.iH)) {
            location.reload();
        } else {
            if (dd.op6) {
                setTimeout("WINSZ()", 511);
...
function DDObj(d_o, d_i) {
    ...
    if (this.oimg) {
        ...
        switch (this._class) {
            case 'img_subflow':
                this.customDom = '<div class="subflow-left"><i class="iconfont icon-oa-banliguocheng"></i></div>';
                break;
        }
        this.htm = '<img name="' + this.id + 'NImG" src="' + this.oimg.src + '" width="' + this.w + '" height="' + this.h + '">';
        this.t_htm = '<div id="' + this.id + '" class="flowControl ' + this._class + '" style="position:absolute;left:' + (this.cssx = this.x = this.defx) + "px;top:" + (this.cssy = this.y = this.defy) + "px;width:" + this.w + "px;height:" + this.h + 'px;" onmouseover="if (window.my_OverFunc) my_OverFunc(\'' + this.name + "');\" onmouseout=\"if (window.my_OutFunc) my_OutFunc('" + this.name + "');\">" + this.customDom + this.htm + "</div>";
    } else {
        if (!!(this.div = dd.getDiv(this.id)) && typeof this.div.style != "undefined") {
...
DDObj.prototype.resizeTo = function(d_w, d_h, d_o) {
    ...
    if (dd.n4) {
        ...
        if (typeof this.css.pixelWidth != "undefined") {
...
DDObj.prototype.copy = function(d_n, d_p) {
    ...
    while (d_n--) {
        ...
        if (dd.n4) {
            ...
            if (dd.db.insertAdjacentHTML) {
                dd.db.insertAdjacentHTML("AfterBegin", d_o.t_htm);
            } else {
                if (document.createElement && dd.db && dd.db.appendChild) {
...
DDObj.prototype.addChild = function(d_kd, detach, defp) {
    ...
    if (defp) {
        ...
        if (this == d_kd.defparent && d_kd.is_image) {
...
DDObj.prototype._setOpaRel = function(d_x, d_kd, d_y) {
    if (this.diaphan || d_kd) {
        ...
        if (dd.n6) {
            ...
            if (dd.ie && !dd.iemac && typeof this.div.filters != "undefined") {
        ...
        if (this.original.copies) {
            ...
            for (var d_i = 0; d_i < this.original.copies.length; d_i++) {
                if ((d_o = this.original.copies[d_i]) != this) {
                    d_os[d_os.length] = d_os[d_o.name] = d_o;
                }
            }
...
function DRAG(d_ev) {
    ...
    if (dd.ie4 || dd.w3c || dd.n6 || dd.obj.children.length > 15) {
        if (dd.wait) {
            return false;
        }
        dd.wait = 1;
...
function SET_DHTML() {
    ...
    var d_a = SET_DHTML.arguments, d_ai, d_htm = "", d_o, d_i = d_a.length;
    while (d_i--) {
        if (dd.op6) {
            var d_t0 = (new Date()).getTime();
            while ((new Date()).getTime() - d_t0 < 153) {}
        }
...
var NO_SCROLL = "";
function my_PickFunc_0() {}
function my_DragFunc_0() {}
function my_ResizeFunc_0() {}
function my_DropFunc_0() {}
