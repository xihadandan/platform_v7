...
function OnShowDialog(html, title, hight, width) {
    var ActiveDoc = wps.EtApplication().ActiveWorkbook;
    if(ActiveDoc){
...
function OnlineEditOpenFile(fileUrl) {
    ...
    if (fileUrl) {
        ...
        wps.EtApplication().Workbooks.OpenFromUrl(fileUrl,"","OnDownFail");
        doc = wps.EtApplication().ActiveWorkbook;
    } else {
        if(wps.EtApplication().ActiveWorkbook&&wps.EtApplication().ActiveWorkbook.Name.indexOf("文档")!=-1){
              doc = wps.EtApplication().ActiveWorkbook;
        }else{
            doc = wps.EtApplication().Workbooks.Add();
        }
    }
    return doc;
}
...
function deleteCustom(){
    ...
    data.forEach(function(it) {
        var item = doc.CustomDocumentProperties.Item(it);
        if(item){
            item.Delete();
        }
    })
