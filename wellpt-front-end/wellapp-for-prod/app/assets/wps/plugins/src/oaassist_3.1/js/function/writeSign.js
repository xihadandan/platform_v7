...
function OnWriteSign(params) {
  ...
  // 为true  采用图片原始的宽高，          否则为false
  var origin = wps.PluginStorage.getItem("originWidthHeight");
  var originWidthHeight;
  if (origin) {
    originWidthHeight = origin;
  }
  if (webSetPicWidth) {
  ...
  // 是否使用图片原始高度   0，使用图片原始高度   ，  1，使用传进来的设置的高度
  if (1 == originWidthHeight) {
    if (picHeight) {
