...
function splitCourtFile(info) {
    ...
    if (haschiefFlag == 0) { //无审判长
        templateFile = "http://dev.wpseco.cn/wps-oa/template/getFileData/" + 79
    } else { //有审判长(2个审判员)
        templateFile = "http://dev.wpseco.cn/wps-oa/" + "template/getFileData/" + 80
    }
...
function initMenu() {
    ...
    //从当前文档的CustomDocumentProperties中提取ribbonExt
    var ribbonExt = propGetFromStorage(doc.DocID,"ribbonExt");
    if (ribbonExt) {
        ribonJson = JSON.parse(ribbonExt.Value); //将ribbonExt的值转换为JSON
        // alert(JSON.stringify(ribonJson));
        var buttonGroup = [];
        for (var key in ribonJson) {
            //第一层key代表代表按钮ID
            if (key == "btnShowRevision") {
                var json3 = ribonJson[key];
                var json4 = {};
                for (var key3 in json3) {
                    json4["name"] = key;
                    json4["title"] = "打开/关闭显示痕迹";
                    if (key3 == 'GetImage') {
                        json4["GetImage"] = "./" + json3[key3]; //获取按钮标签图标
                    }

                }
                buttonGroup.push(json4);
            }
            if (key != "btnFilePath" && key != "btnPrintDOC") {
                var json1 = ribonJson[key]; //获取按钮ID对应的属性JSON对象
                var json2 = {};
                json2["name"] = key;

                for (var key2 in json1) {
                    if (key2 == "OnGetVisible" || key2 == "OnGetEnabled") {
                        if (json1[key2]) {
                            continue;
                        } else {
                            break;
                    ...
                    if (key2 == "OnGetLabel") {
                        if (json1[key2]) {
                            json2["title"] = json1[key2]; //获取按钮标签名称
                        }
                    }
                    if (key2 == 'GetImage') {
                        json2["GetImage"] = "./" + json1[key2]; //获取按钮标签图标
                    }

                }
                if (json2["title"]) {
                    buttonGroup.push(json2);
                }
            }
            if (key == "btnPrintDOC") {
                var storage = wps.PluginStorage;
                var btnFilePrintflag = storage.getItem("btnFilePrintflag");
                if (btnFilePrintflag == "true") {
                    var json5 = ribonJson[key];
                    var json6 = {};
                    for (var key4 in json5) {
                        json6["name"] = key;

                        if (key4 == "OnGetLabel") {
                            if (json5[key4]) {
                                json6["title"] = json5[key4]; //获取按钮标签名称
                            }
                        }

                        if (key4 == 'GetImage') {
                            json6["GetImage"] = "./" + json5[key4]; //获取按钮标签图标
                        }

                    }
                    buttonGroup.push(json6);
                }

            }

        }
        // alert(JSON.stringify(buttonGroup));
        vuedata.routes = buttonGroup;
