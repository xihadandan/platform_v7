...
function OnUploadToServerSuccess(resp) {
  var l_doc = wps.WppApplication().ActivePresentation;
  if (wps.confirm('文件上传成功！点击“确定”关闭文件，“取消”继续编辑。')) {
    if (l_doc) {
...
function OnBtnSaveToServer() {
  ...
  if (!wps.confirm('本地文件保存成功！是否上传文件？')) {
    return;
  }
...
function OnAction(control) {
  ...
  var strId = eleId; // typeof (control) == "object" ? control.Id : control;
  switch (strId) {
    case 'FileSave': {
      if (pCheckIfOADoc()) {
        //文档来源是业务系统的，做自定义
        ...
        wps.WppApplication().CommandBars.ExecuteMso('SaveAll');
