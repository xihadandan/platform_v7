  ...
  if (l_strFileUrl) {
    ...
    if (pCheckIsExistOpenOADoc(l_strFileUrl) == true) {
      ...
      if (l_strFileUrl.startWith('http')) {
        ...
        if (doc) doc.SaveAs2(($FileName = wps.Env.GetTempPath() + '/' + doc.Name), undefined, undefined, undefined, false);
      }
    }
  } else {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      data.forEach(function (it) {
        ...
        if (!Bookmarks.Exists(bookmark.Name)) Bookmarks.Add(bookmark.Name, bookmark.Range.SetRange(bookStart, bookEnd + (end - start)));
      });
    }
  });
}
  ...
  for (var key = '' in params) {
    switch (
      ...
      case 'userName'.toUpperCase(): //修改当前文档用户名
        ...
        wps.PluginStorage.setItem('oaUserName', params[key]);
