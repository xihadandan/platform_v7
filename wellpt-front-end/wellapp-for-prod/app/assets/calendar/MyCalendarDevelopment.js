define(['commons', 'constant', 'server', 'CalendarWidgetDevelopment', 'appModal', 'moment', 'formBuilder', 'layer'], function (
  commons,
  constant,
  server,
  ...
  commons.inherit(MyCalendarDevelopment, CalendarWidgetDevelopment, {
    ...
    init: function () {
      ...
      var _self = this;
      //定义独立的新增日程按钮的点击事件
      $($(_self.widget.element).parents('.tab-pane')[0])
        .off('click')
        .on('click', '#btn_new_event', function (e) {
          _self.dayClick(moment(), e, null);
        });
      _self.formatHomePageTime();
      $('.well-calendar-body-date')
        .off('click')
        .on('click', function () {
          _self.formatHomePageTime();
        });
      // 页面容器监听左导航事件
      _self.widget.pageContainer.on('onShowBootstrapTab-Calendar', function (tab) {
        _self.onShowBootstrapTab(tab);
      });
      // TODO 监听左导航的刷新事件
      _self.widget.pageContainer.on('onCalendarDelete-Calendar', function (tab) {
        _self.getWidget().refresh();
      });
    },
    onShowBootstrapTab: function (tab) {},
    ...
    dayClick: function (date, jsEvent, view) {
      var _self = this;
      //关注入口进来,不触发新建事项
      var $page = this.widget.pageContainer.element.find('.oa-calendar-tab-class');
      // 旧关注入口有添加关注人按钮，新的用当前tabs页面是否有.attention-page样式
      if ($('button.js-add-attention-user').length > 0 || $page.find('.tab-pane.active .attention-page')[0]) {
        return false;
      } else {
        //格式化周期开始和结束时间时候把秒去掉
    ...
    getTitle: function (configuration) {
      configuration.titleJson.title = '日程';
    },
    ...
    eventAfterRender: function (event, element, view) {
      ...
      if (event.customTitle) {
        ...
        $(element)
          .find('.fc-title')
          .html(event.customTitle)
          .attr('title', ($(element).find('.fc-time').data('full') || $(element).find('.fc-time').text()) + ' ' + event.customTitle);
      } else if (event.isCanView == true) {
        //自己的日程不需要显示名称
        if (currUser.userId == event.calendarCreator) {
          $(element).find('.fc-time').css('display', 'none');
          $(element)
            .find('.fc-title')
            .attr('title', ($(element).find('.fc-time').data('full') || $(element).find('.fc-time').text()) + ' ' + event.title);
        } else {
          $(element).find('.fc-time').css('display', 'none');
          $(element)
            .find('.fc-title')
            .html('[' + event.calendarCreatorName + ']' + event.title);
          $(element)
            .find('.fc-title')
            .attr(
              'title',
              ($(element).find('.fc-time').data('full') || $(element).find('.fc-time').text()) +
                ' ' +
                '[' +
                event.calendarCreatorName +
                ']' +
                event.title
            );
        }
        ...
        $(element)
          .find('.fc-title')
          .html(event.calendarCreatorName ? '[' + event.calendarCreatorName + ']已安排' : event.title);
        $(element)
          .find('.fc-title')
          .attr('title', event.calendarCreatorName ? '[' + event.calendarCreatorName + ']已安排' : event.title);
      }
    ...
    eventClick: function (event, jsEvent, view) {
      //格式化周期开始和结束时间
      if (event.startTime) {
        event.startTime = commons.DateUtils.format(new Date(event.startTime), 'yyyy-MM-dd HH:mm');
      }
      if (event.endTime) {
        event.endTime = commons.DateUtils.format(new Date(event.endTime), 'yyyy-MM-dd HH:mm');
      }
      if (event.repeatPeriodStartTime) {
        event.repeatPeriodStartTime = commons.DateUtils.format(new Date(event.repeatPeriodStartTime), 'yyyy-MM-dd HH:mm');
      }
      if (event.repeatPeriodEndTime) {
        event.repeatPeriodEndTime = commons.DateUtils.format(new Date(event.repeatPeriodEndTime), 'yyyy-MM-dd HH:mm');
      }
        ...
        var $dialog = appModal.dialog({
          ...
          shown: function (_$dialog) {
            ...
            var titleOpts = {
              ...
              type: 'label',
              value: event.title,
              text: event.calendarCreatorName ? '[' + event.calendarCreatorName + ']已安排' : event.title
            };
    ...
    addOtherValidateRules: function (validateOptions, configuration) {
      ...
      validateOptions.rules['itemform_repeatPeriodEndTime'] = {
        checkRepeatPeriodEndTime: true
      };
      ...
      $.validator.addMethod(
        ...
        function (value, element) {
          ...
          return data && (data.id || data.text) ? true : false;
        },
        '请选择日程归属。'
      );

      $.validator.addMethod(
        'checkRepeatPeriodEndTime',
        function (value, element) {
          var isRepeat = $('#itemform_isRepeat').attr('checked');
          if (isRepeat) {
            //有设置重复进行检查
    ...
    afterEventDialogRender: function ($dialog, options, configuration) {
      ...
      //弹框进来默认先隐藏重复开始时间和结束时间
      $dialog.find('#itemform_repeatPeriodStartTime').parents('.form-group').hide();
      $dialog.find('#itemform_repeatPeriodEndTime').parents('.form-group').hide();
      ...
      if (1 == options.event.isRepeat) {
        ...
        if ('day' == conf.repeatType) {
          //按天重复
          var values = conf.repeatValue ? conf.repeatValue.split(';') : [];
          $.each(values, function (i) {
            $('#itemform_repeatDay_' + values[i])
              .attr('checked', 'checked')
              .trigger('change');
          });
        } else if ('week' == conf.repeatType) {
          //按周重复
          var values = conf.repeatValue ? conf.repeatValue.split(';') : [];
          $.each(values, function (i) {
            $('#itemform_repeatWeek_' + values[i])
              .attr('checked', 'checked')
              .trigger('change');
          });
        } else if ('month' == conf.repeatType) {
          //按月重复
          $('#js-repeat-month-options').val(conf.repeatValue).trigger('change');
        } else if ('year' == conf.repeatType) {
          //按年重复
      ...
      if (StringUtils.isBlank(options.event.uuid)) {
        ...
        var $selectMenu = $('.ui-wBootstrapTabs .metismenuul .nav-menu-active,#calendar_category_tree>.hasSelectCate');
        if ($selectMenu.length > 0) {
          ...
          // 没有当前选中的，则找默认日历本
          $selectMenu = $(
            ".ui-wBootstrapTabs .metismenuul a[title='默认日历本'],#calendar_category_tree .msg-category-item[title='默认日历本']>.msg-category-text"
          ).parents('li');
        ...
        var hideBlock = $('#form_calendar_event div.js-item-row:gt(5)');
        $($('#form_calendar_event div.js-item-row')[$('#form_calendar_event div.js-item-row').length - 1]).after(
          "<div class='text-primary' id='btn' style=' text-align: center;width: 100%;cursor:pointer;'>查看更多<span class='iconfont icon-ptkj-xianmiaoshuangjiantou-xia'></span></div>"
        );
        ...
        if (options.event.noticeTypes) {
          ...
          $.each(options.event.noticeTypes, function (i, dom) {
            $("label[for='itemform_noticeTypes']")
              .parents('.row')
              .find("input[value='" + dom + "']")
              .attr('checked', 'checked');
          });
        }
      ...
      $('#itemform_endTime,#itemform_startTime')
        .off('blur')
        .on('blur', function () {
          if (!isFirstValid) {
            isFirstValid = true;
            setInterval(function () {
              if ($('#form_calendar_event').validate() != undefined) {
                $('#form_calendar_event').validate().element($('#itemform_endTime'));
              }
            }, 500);
          }
        });
      //给重复开始时间和重复结束时间绑定触发事件
      var isFirstValid1 = false;
      $('#itemform_repeatPeriodEndTime,#itemform_repeatPeriodStartTime')
        .off('blur')
        .on('blur', function () {
          var isRepeat = $('#itemform_isRepeat').attr('checked');
          if (!isFirstValid1 && isRepeat) {
            isFirstValid1 = true;
            setInterval(function () {
              if ($('#form_calendar_event').validate() != undefined) {
    ...
    _initSelectAndCheckboxEvent: function () {
      ...
      $('#itemform_isRepeat').change(function () {
        ...
        if (checked) {
          ...
          $('#itemform_repeatPeriodEndTime').parents('.form-group').show();
          $('#itemform_repeatPeriodStartTime').parents('.form-group').show();
        } else {
          $('#js-row-repeat-conf').hide();
          $('#itemform_repeatPeriodEndTime').parents('.form-group').hide();
          $('#itemform_repeatPeriodStartTime').parents('.form-group').hide();
        }
      });
      //定义提醒checkbox的事件
      ...
      $('#itemform_isAll').change(function () {
        var checked = $(this).attr('checked');
        let $itemformStartTime = $('#itemform_startTime');
        let $itemformEndTime = $('#itemform_endTime');
        if (checked) {
          oldStart = $itemformStartTime.val();
          oldEnd = $itemformEndTime.val();

          let startForAllDay = moment(oldStart).format('YYYY-MM-DD 00:00');
          $itemformStartTime.val(startForAllDay);
          $itemformStartTime.next().datetimepicker().val(startForAllDay);
          $itemformStartTime.next().attr('disabled', 'true');

          let endForAllDay = moment(oldEnd).format('YYYY-MM-DD 23:59');
          $itemformEndTime.val(endForAllDay);
          $itemformEndTime.next().datetimepicker().val(endForAllDay);
          $itemformEndTime.next().attr('disabled', 'true');
        } else {
          if (oldStart) {
            $itemformStartTime.val(oldStart);
            $itemformStartTime.next().datetimepicker().val(oldStart);
          }
          if (oldEnd) {
            $itemformEndTime.val(oldEnd);
            $itemformEndTime.next().datetimepicker().val(oldEnd);
          }
          $itemformStartTime.next().attr('disabled', null);
          $itemformEndTime.next().attr('disabled', null);
        }
      });
    ...
    create3checkRow: function ($dialog) {
      ...
      $row.find('label').html('<span class="label-span"></span>');
      ...
      $.each(options, function (key, value) {
        ...
        var checkbox =
          "<label class='checkbox-inline label-formbuilder'>" +
          "<input class='w-itemform-option' style='display: inline-block;' value='1' height='34px' id=" +
          id +
          ' name=' +
          id +
          " type='checkbox'/>" +
    ...
    createRepeatConfRow: function ($dialog) {
      ...
      $row.find('label').html('<span class="label-span">重复时间</span>');
      var $div = $row.find('div.controls');
      var $groupDiv = $('<div>', {
    ...
    _addCheckboxs: function (options, $div, idPrev) {
      $.each(options, function (key, value) {
        ...
        var label =
          "<label class='checkbox-inline label-formbuilder'>" +
          "<input class='w-itemform-option' style='display: inline-block;margin-right:2px' height='34px' value=" +
          key +
          ' id=' +
          id +
          ' name=' +
    ...
    createRepeatYearConf: function ($dialog) {
      ...
      $div
        .append("<input type='text' class='form-control w-itemform-option' height='34px' name='itemform_repeatYear'>")
        .append("<span class='input-group-addon'><i class='glyphicon glyphicon-calendar'></i></span>");
    ...
    createRemindConfRow: function ($dialog) {
      ...
      $row.find('label').html('<span class="label-span">提前通知</span>');
      var $div = $row.find('div.controls');
      var $groupDiv = $('<div>', {
        class: 'form-contorl'
      });
    ...
    afterCollectEventData: function (options, configuration, newFormData) {
      ...
      if (newFormData.repeatPeriodStartTime) {
        //收集的时候把秒补全
        newFormData['repeatPeriodStartTime'] = commons.DateUtils.format(
          new Date(newFormData['repeatPeriodStartTime']),
          'yyyy-MM-dd HH:mm:ss'
        );
      }
      if (newFormData.repeatPeriodEndTime) {
        newFormData['repeatPeriodEndTime'] = commons.DateUtils.format(new Date(newFormData['repeatPeriodEndTime']), 'yyyy-MM-dd HH:mm:ss');
      }
    ...
    deleteEventClick: function (event, configuration, options) {
      var _self = this;
      var title = '提示';
      var $html = '<div id="delComfirmDialog"><div>请问是否确定要删除该日程?</div></div>';
      if (event.isRepeat && event.repeatMarkId) {
        title = '删除日程';
        $html =
          '<div id="delComfirmDialog"><div>您将删除此重复日程,请选择生效范围</div><div class="mt10"><input id="radio_0" type="radio" name="radio" value="0" checked><label for="radio_0">仅本次日程</label></div><div class="mt10"><input type="radio" id="radio_1" name="radio"  value="1"><label for="radio_1">全部日程</label></div></div>';
      }
      appModal.dialog({
        title: title,
        message: $html,
        ...
        buttons: {
          confirm: {
            ...
            callback: function (result) {
              if (result) {
                if (event.isRepeat && event.repeatMarkId) {
                  var radioVal = $("#delComfirmDialog input[name='radio']:checked").val();
                  if (radioVal === '1') {
                    server.JDS.call({
    ...
    onLeftSidebarItemClick: function (menuItemEvent, options, configuration) {
      ...
      if (menuItem) {
        ...
        if ('myAll' == type) {
          ...
          this.addOtherConditions([filterCriterion]);
        } else if ('inCalendar' == type) {
          //我的日程-全部,
          var filterCriterion = {
            columnIndex: 'belongObjId',
            value: menuItem.uuids,
            type: 'in'
          };
          this.addOtherConditions([filterCriterion]);
          ...
          this.addOtherConditions([filterCriterion]);
        } else if ('userAll' == type) {
          //我关注的个人-全部
          var userId = menuItem.uuids ? menuItem.uuids.split(';') : [''];
          var filterCriterion = {
            columnIndex: 'calendarCreator',
            value: userId,
            type: 'in'
          ...
          this.addOtherConditions([filterCriterion]);
          this.getWidget().ajaxParams['from'] = 'attention';
        } else if ('groupAll' == type) {
          //我关注的群组-全部
          var userIds = menuItem.uuids ? menuItem.uuids.split(';') : [''];
          var filterCriterion = {
            columnIndex: 'calendarCreator',
            value: userIds,
            type: 'in'
    ...
    createViewFinishStatusRow: function ($dialog, options) {
      ...
      $row.find('label').html('<span class="label-span">当前状态</span>');
      var $div = $row.find('div.controls');
      var $span = $('<span>', {
    ...
    createViewEventCreatorRow: function ($dialog, options) {
      ...
      var _self = this;
      var $row = this.createViewRowHtml($dialog);
      $row.find('label').html('<span class="label-span">日程创建者</span>');
      var $div = $row.find('div.controls');
      var $span = $('<span>', {
        class: 'w-viewform-option'
        // height: '34px'
    ...
    createViewEventOwnerRow: function ($dialog, options) {
      var user = server.SpringSecurityUtils.getUserDetails(options.event.calendarCreator);
      var _self = this;
      var $row = this.createViewRowHtml($dialog);
      $row.find('label').html('<span class="label-span">日程所属</span>');
      var $div = $row.find('div.controls');
      var $span = $('<span>', {
        class: 'w-viewform-option'
        // height: '34px'
    ...
    createViewRepeatRow: function ($dialog, options) {
      var _self = this;
      var $row = this.createViewRowHtml($dialog);
      $row.find('label').html('<span class="label-span">重复</span>');
      var $div = $row.find('div.controls');
      var $span = $('<span>', {
        class: 'w-viewform-option'
        // height: '34px'
    ...
    afterViewEventDialogRender: function ($dialog, options, configuration) {
      ...
      if (options.event.isRepeat == 1) {
        ...
        $dialog.find('#viewform_repeatPeriodEndTime').parents('.form-group').after($repeatRow);
      } else {
        $dialog.find('#viewform_repeatPeriodStartTime').parents('.form-group').hide();
        $dialog.find('#viewform_repeatPeriodEndTime').parents('.form-group').hide();
      }
      ...
      //格式化周期开始和结束时间
      if (options.event.startTime) {
        $dialog.find('#viewform_' + 'startTime').html(options.event.startTime);
      }
      if (options.event.endTime) {
        $dialog.find('#viewform_' + 'endTime').html(options.event.endTime);
      }
    ...
    addFinishBtn: function ($dialog, options) {
      //插入完成按钮
      var _self = this;
      var $finishBtn = $("<button class='well-btn w-btn-primary w-line-btn js-btn-status' type='button'></button>");
      //未完成状态，则需要出现完成按钮
      $finishBtn.html(options.event.isFinish == 1 ? '未完成' : '已完成');
        ...
        server.JDS.call({
          ...
          success: function (result) {
            if (result) {
              ...
              var iconColor = widget.getEventStatus && widget.getEventStatus(isFinish + '');
              iconColor = iconColor || {
                color: {
                  iconColor: getUserTheme().colorValue
                }
              };
              options.event.backgroundColor = iconColor.color.iconColor + ' !important';
              widget._updateEvent(options.event);
            }
          }
        });
      });
    ...
    getAfterCollectEventDataCB: function (options, configuration, newFormData) {
      ...
      if (newFormData.startTime && newFormData.endTime) {
        ...
        server.JDS.call({
          ...
          data: [newFormData.startTime, newFormData.endTime, options.event.uuid],
          async: false,
          success: function (result) {
            if (result && result.success) {
              if (result.data && !options.event.readySave) {
                //冲突
                var alertDefaults = {
                  tipsCode: '666',
                  title: '提示框', // 标题
