define(['commons', 'constant', 'server', 'CalendarWidgetDevelopment', 'appModal', 'moment', 'formBuilder', 'layer'], function (
  ...
  var BaseCalendarDevelopment = function (wWidget) {
    CalendarWidgetDevelopment.apply(this, arguments);
  };
  // 接口方法
  commons.inherit(BaseCalendarDevelopment, CalendarWidgetDevelopment, {
    // 重复的类型
    // repeatTypeOptions: {
    //   day: '每天',
    //   week: '每周',
    ...
    init: function () {
      // console.log('BaseCalendarDevelopment.init');
      // var _self = this;
      // //定义独立的新增日程按钮的点击事件
    ...
    //     //如果是从月视图进来的的，则默认是当前时间 || date._ambigTime(fullCalendar组件属性值)
    //     if ((view && view.type === 'month') || date._ambigTime) {
    //       date._isUTC = false;
    //       date._d = new Date(date._d.getFullYear(), date._d.getMonth(), date._d.getDate(), 8, 0);
    //       startTime = date.format('YYYY-MM-DD HH:mm');
    //       endTime = date.add(30, 'minutes').format('YYYY-MM-DD HH:mm');
    //     }
    //
    //     // 弹出新建创建窗口
    ...
    // statusFilter: function (status, dateRange) {
    // var _self = this;
    ...
    //   if (status[0] === 'done') {
    //     var filterCriterion = {
    //       columnIndex: 'isFinish',
    //       value: '1',
    //       type: 'eq'
    //     };
    //     _self.addOtherConditions([filterCriterion]);
    //   } else if (status[0] === 'uncomplete') {
    //     var filterCriterion = {
    //       columnIndex: 'isFinish',
    //       value: '0',
    //       type: 'eq'
    ...
    //   // var filterCriterion = { columnIndex: 'createTime', value: dateRange[0], type: 'gt' };
    //   // _self.addOtherConditions([filterCriterion]);
    // }
    //
    // if (dateRange && dateRange[1]) {
    //   _viewEnd = moment(dateRange[1] + ' 23:59:59').format('YYYY-MM-DD HH:mm:ss');
    //   // var filterCriterion = { columnIndex: 'createTime', value: dateRange[1], type: 'lt' };
    //   // _self.addOtherConditions([filterCriterion]);
    // }
    //
    ...
    dataSourceProcessing: function (dataSource) {
      ...
      //   var done = widget.getEventStatus && widget.getEventStatus('done');
      //   done = done || {
      //     color: {
      //       iconColor: '#fff'
      //     }
      //   };
      //   var uncomplete = widget.getEventStatus && widget.getEventStatus('uncomplete');
      ...
      //     if (event.isFinish == 0) {
      //       event.backgroundColor = uncomplete.color.iconColor + ' !important';
      //     } else if (event.isFinish == 1) {
      //       event.backgroundColor = done.color.iconColor + ' !important';
      //     }
      //   });
    ...
    afterEventDialogRender: function ($dialog, options, configuration) {
      // var _self = this;
      // console.log('BaseCalendarDevelopment.afterEventDialogRender');
    ...
    addFinishBtn: function ($dialog, options) {
      ...
      //           }
      //         };
      //         options.event.backgroundColor = (isFinish ? done.color.iconColor : uncomplete.color.iconColor) + ' !important';
  ...
  return BaseCalendarDevelopment;
});
