define(['jquery', 'commons', 'constant', 'server', 'CalendarWidgetDevelopment', 'appModal', 'moment', 'formBuilder'], function (
  $,
  commons,
  constant,
  server,
    ...
    commons.inherit(MyCalendarDevelopment, CalendarWidgetDevelopment, {
      ...
      afterCollectEventData: function (options, configuration, newFormData) {
        ...
        if (newFormData.repeatPeriodEndTime) {
          newFormData['repeatPeriodEndTime'] = commons.DateUtils.format(
            new Date(newFormData['repeatPeriodEndTime']),
            'yyyy-MM-dd HH:mm:ss'
          );
        }
      ...
      otherValidateRules: function (data) {
        ...
        if (data.eventContent && data.eventContent.length > 200) {
          ...
          (data.repeat === 'day' && data.repeatDay.length <= 0) ||
