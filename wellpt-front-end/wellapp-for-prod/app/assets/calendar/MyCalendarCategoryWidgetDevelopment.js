define(['constant', 'commons', 'server', 'HtmlWidgetDevelopment'], function (constant, commons, server, HtmlWidgetDevelopment) {
  var JDS = server.JDS;
  // 页面组件二开基础
  var MyCalendarCategoryWidgetDevelopment = function () {
    HtmlWidgetDevelopment.apply(this, arguments);
  };
  // 接口方法
  commons.inherit(MyCalendarCategoryWidgetDevelopment, HtmlWidgetDevelopment, {
    // 组件初始化
    init: function () {
      var self = this;
      var $container = this.widget.element;
      $container.addClass('calendar-category');
      function searchList(val) {
        $('#calendar_category_tree>.msg-category-item').each(function (idx, elem) {
          var $elem = $(elem);
          var title = $elem.attr('title') + '';
          if ($.trim(val).length <= 0 || title.indexOf(val) > -1) {
            $elem.show();
          } else {
            $elem.hide();
          }
      ...
      getList();
      $('#addCalendarCategory', $container)
        .off()
        .on('click', function () {
          openDialog('新建日历分类');
        });

      $('#searchCalendarCategory', $container)
        .off()
        .on('click', function () {
          var val = $('#fullCalendarName', $container).val();
          searchList(val);
        });
      $('#fullCalendarName', $container)
        .off()
        .on('keyup', function () {
          if ($(this).val() != '') {
            $('#deleteCalendarCategory', $container).show();
          } else {
            $('#deleteCalendarCategory', $container).hide();
          }
        })
      ...
      $('#deleteCalendarCategory', $container)
        .off()
        .on('click', function () {
          $('#fullCalendarName', $container).val('');
          $(this).hide();
          searchList();
        });

      $('#calendar_category_content', $container)
        .off()
        .on('click', '.hasList', function () {
          if ($(this).hasClass('icon-ptkj-shixinjiantou-you')) {
            ...
            $('#calendar_category_tree', $container).slideDown();
          } else {
            $(this).removeClass('icon-ptkj-shixinjiantou-xia').addClass('icon-ptkj-shixinjiantou-you');
            $('#calendar_category_tree', $container).slideUp();
          }
        });
      ...
      $('.msg-category-folder', $container)
        ...
        .on('click', function () {
          ...
          if ($(this).find('i').hasClass('icon-ptkj-youshouzhan')) {
            $(this).parents('.msg-content-list').find('.ui-wHtml').animate({ width: '230px' }, 300).find('.msg-category-wrap').show();
            $(this).parents('.msg-category-content').animate({ width: '220px' }, 300);
            $(this).find('i').removeClass('icon-ptkj-youshouzhan').addClass('icon-ptkj-zuoshouzhan');
          } else {
            $(this).parents('.msg-content-list').find('.ui-wHtml').animate({ width: '0' }, 300).find('.msg-category-wrap').hide();
            $(this).parents('.msg-category-content').animate({ width: '0' }, 300);
            $(this).find('i').addClass('icon-ptkj-youshouzhan').removeClass('icon-ptkj-zuoshouzhan');
          }
        });

      $('#calendarAllCategory', $container)
        .off('click')
        .on('click', function () {
          $('.msg-category-item', $container).removeClass('hasSelectCate');
        });

      $('#calendar_category_tree', $container)
        .off()
        .on('click', '.msg-category-item', function (e) {
          e.stopPropagation();
          var $target = $(e.target);
          if ($target.is('label.noclick') || $target.is('input.noclick')) {
            return;
          }
          $(this).addClass('hasSelectCate').siblings().removeClass('hasSelectCate');
          self.widget.trigger(constant.WIDGET_EVENT.LeftSidebarItemClick, {
            selectedItem: {
              menuItem: {
                type: 'myCalendar',
                calendar: $(this).data('menuitem')
              }
            }
          });
        });
      ...
      function getList(val) {
        JDS.restfulPost({
          url: ctx + '/api/calendar/category/getAllBySystemUnitIdsLikeName',
          data: { name: val || '' },
          success: function (result) {
            if (result.success || result.code == 0) {
              ...
              for (var i = 0; i < data.length; i++) {
                ...
                var title = '';
                if (data[i].calendarName.length > 9 && remark != '') {
                  title = data[i].calendarName + '\n' + remark;
                } else if (data[i].calendarName.length <= 9 && remark != '') {
                  title = remark;
                } else if (data[i].calendarName.length > 9 && remark == '') {
                  title = data[i].calendarName;
                }
                data[i].type = 'CalendarCategory';
                data[i].calendar = { uuid: data[i].uuid };
                lis +=
                  ...
                  background +
                  "'></span>" +
                  "<input type='checkbox' name='calendar-category-item' class='well-ui-checkbox noclick' id='" +
                  data[i].uuid +
                  "'/><label for='" +
                  data[i].uuid +
                  "' class='noclick'></label>" +
              ...
              $('#calendar_category_tree', $container).html(lis);
            }
          }
        });
      }
      ...
      function getDetail(bean, uuid, $dialog) {
        JDS.restfulGet({
          url: ctx + '/api/calendar/category/get',
          data: { uuid: uuid },
          ...
          success: function (result) {
            if (result.success || result.code == 0) {
              ...
              $('#msgIconShow', $dialog).addClass(bean.icon).css({ background: bean.iconColor });
              $('#selectIconBg', $dialog).find('div').attr('data-color', bean.iconColor).css({ background: bean.iconColor });
            }
          }
        });
      }
          ...
          buttons: {
            ok: {
              ...
              callback: function () {
                ...
                JDS.restfulPost({
                  url: ctx + '/api/calendar/category/save',
                  data: bean,
                  success: function (result) {
                    if (result.success || result.code == 0) {
                      getList();
                  ...
                  error: function (err) {
                    appModal.error({ message: err.responseJSON.msg, timer: 3000 });
                  }
                });
              }
            },
          ...
          shown: function () {
            ...
            $('.bg-choose-item', $dialog.find('.icon-bg-wrap'))
              ...
              .on('click', function () {
                ...
                $('#msgIconShow', $dialog).css({ background: color });
              });
            // 关闭色块
                ...
                if ($('.minicolors', $dialog).size() > 0) {
                  ...
                  $('#iconColor', $dialog).minicolors({
                    ...
                    change: function (value, opacity) {
                      ...
                      $('#msgIconShow', $dialog).css({ background: value });
                    },
                    hide: function () {
      ...
      function getDialogContent() {
        ...
        html +=
          ...
          "<td><input type='text' name='calendarName' id='calendarName' class='form-control' placeholder='分类名称'/></td>" +
          '</tr>' +
          "<tr class='field'>" +
      ...
      $('#calendar_category_tree', $container).on('click', '.btn-edit', function (e) {
        e.stopPropagation();
        var uuid = $(this).parents('.msg-category-item').data('uuid');
        openDialog('编辑日历分类', uuid);
      });

      $('#calendar_category_tree', $container).on('click', '.btn-delete', function (e) {
        e.stopPropagation();
        ...
        appModal.confirm('确定删除分类吗', function (success) {
          if (success) {
            JDS.restfulPost({
              url: ctx + '/api/calendar/category/deleteWhenNotUsed',
              data: { uuid: uuid },
              contentType: 'application/x-www-form-urlencoded',
              success: function (result) {
                if (result.success || result.code == 0) {
                  if (result.data === 1 || result.data === 0) {
                    ...
                    $el.remove();
                    $('#calendarAllCategory', $container).trigger('click');
                  } else if (result.data === -1) {
                    appModal.error('日历分类下存在日历，无法删除！');
                  }
                } else {
                  appModal.error('系统异常');
                }
      ...
      $container.on('change', 'input#calendar-category-checkall', function (event) {
        var checked = $(event.target).prop('checked');
        $('input[name=calendar-category-item]').prop('checked', $(event.target).prop('checked'));
        var menuItem = checked
          ? {
              type: 'myAll'
            }
          : {
      ...
      $container.on('change', 'input[name=calendar-category-item]', function (event) {
        event.preventDefault();
        event.stopPropagation();
        var $items = $('input[name=calendar-category-item]');
        var $checkedItem = $items.filter(':checked');
        $('input#calendar-category-checkall').prop('checked', $items.length === $checkedItem.length);
        var uuids = $.map($checkedItem, function (elem, idx) {
          return $(elem).closest('.msg-category-item').data('menuitem').uuid;
        });
        uuids.push('nouuid');
        self.widget.trigger(constant.WIDGET_EVENT.LeftSidebarItemClick, {
          selectedItem: {
  ...
  return MyCalendarCategoryWidgetDevelopment;
});
