...
export function createDyform(formUuid, namespace) {
  return {
    ...
    subformDataMD5: {}, // 从表行数据md5，用于检测数据是否改变
    ...
    deleteSubform(formUuid, index) {
      if (this.subform[formUuid] != undefined) {
        ...
        if (index == undefined) {
          for (let i = 0; i < this.subform[formUuid].length; i++) {
            delFun(i--);
          }
        } else {
          delFun(index);
    ...
    setText(textId, value) {
      if (this.$textset && this.$textset[textId]) {
    ...
    setFieldOptionsRefresh(fieldName, options) {
      let $field = this.$fieldset[fieldName];
      if ($field) {
        $field.refreshOptions(options);
      }
    },
    ...
    setSubformFieldOptionsRefresh(formId, fieldName, options = {}, rowKey) {
      let $subform = this.$subform[formId];
      if ($subform) {
        $subform.setFieldOptionsRefresh(fieldName, options, rowKey);
      }
    },
