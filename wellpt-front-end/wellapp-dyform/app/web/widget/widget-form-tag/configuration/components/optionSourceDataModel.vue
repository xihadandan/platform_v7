<template>
  ...
  <div>
    <a-form-model-item class="display-b" :label-col="{}" :wrapper-col="{}">
      <template slot="label">
        <span
          ...
          :class="options.dataModelUuid ? 'ant-btn-link' : ''"
          @click="redirectDataModelDesign(options.dataModelUuid)"
          :title="options.dataModelUuid ? '打开数据模型' : ''"
        >
          数据模型选择
        </span>
      </template>
      <DataModelSelectModal
        v-model="options.dataModelUuid"
        ref="dataModelSelect"
        :displayModal="true"
        :dtype="['TABLE', 'VIEW']"
    ...
    <a-form-model-item v-for="(opt, i) in fieldOptions" :key="i" :label="opt.label" class="display-b" :label-col="{}" :wrapper-col="{}">
      <a-select
...
export default {
  ...
  methods: {
    ...
    onChangeDataModelSelect() {
      ...
      if (this.options.dataModelUuid == undefined) {
        this.columnIndexOptions.splice(0, this.columnIndexOptions.length);
        this.$emit('columnIndexOptionChange', this.columnIndexOptions);
      } else {
        this.$refs.dataModelSelect.fetchDataModelColumns(this.options.dataModelUuid).then(list => {
          if (list) {
            this.columnIndexOptions.splice(0, this.columnIndexOptions.length);
