...
class FlowVisaDyformDataController extends Controller {
  ...
  async dyformDataViewer() {
    ...
    let dataUuid = '';
    try {
      const result = await ctx.curl(app.config.backendURL + '/api/workflow/visa/inspection/getFlowInstanceDetail', {
        method: 'get',
      ...
      if (result.data.data != null) {
        formUuid = result.data.data.formUuid;
        dataUuid = result.data.data.dataUuid;
      } else {
        this.app.logger.error(result.data.msg);
      }
    } catch (error) {
      this.app.logger.error(error);
    }
    ...
    const definitionJs = await ctx.service.dyformDefinitionService.explainJavascripts(formUuid, loadjs);
    const requirejsConfig = ctx.helper.createRequirejsConfig(definitionJs.javascripts);

    // 样式
    const css = await ctx.service.webResourceService.getThemeCss(DYFORMDATA['wDataViewer'].css, 'default');
...
module.exports = FlowVisaDyformDataController;
