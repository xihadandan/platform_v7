	...
	$.fn.wradio = function(option) {
		...
		if (typeof option == 'string') {
			if (option === 'getObject') { // 通过getObject来获取实例
				var $this = $(this), data = $this.data('wradio');
				if (data) {
					return data; // 返回实例对象
				} else {
		...
		var $this = $(this), data = $this.data('wradio'), options = typeof option == 'object'
				&& option;
		if (!data) {
			data = new Radio($(this), options);
			/*
			 * var datacopy={}; var data1=$.extend(datacopy,data); var
			 * extenddata=$.extend(data,$.wControlInterface); var
			 * data2=$.extend(extenddata,data1);
			 */
