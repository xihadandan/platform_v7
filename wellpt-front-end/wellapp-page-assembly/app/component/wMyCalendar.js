 ...
 * 资源日历组件的解析类
 */
class wMyCalendar extends Component {
  constructor() {
    super();
    this.name = '资源日历组件(新)';
    this.css = ['select2', 'bootstrap-datetimepicker', 'fullcalendar-resourceview', 'fullcalendar', 'well_calendar', 'wMyCalendar'];
    this.category = 'app';
  ...
  async explainDefinitionJson(ctx, json) {
    ...
    // timing.start('wMyCalendar explainDefinitionJson');
    ...
    // ctx.app.logger.info('wMyCalendar explainDefinitionJson 耗时: %d ms', timing.end('wMyCalendar explainDefinitionJson').duration);
  }
