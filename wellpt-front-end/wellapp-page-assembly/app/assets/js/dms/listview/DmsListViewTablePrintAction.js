define(['jquery', 'commons', 'constant', 'server', 'appContext', 'DmsListViewActionBase'], function (
  $,
  commons,
  constant,
  server,
  ...
  // 视图列表表格打印操作
  var DmsListViewTablePrintAction = function () {
    DmsListViewActionBase.apply(this, arguments);
  };
  // 视图列表复制操作，等同于编辑操作，然后把对应的dataUuid清空
  commons.inherit(DmsListViewTablePrintAction, DmsListViewActionBase, {
    btn_list_view_table_print: function (options) {
      var _self = this;
    ...
    doPrint: function (templateId, options) {
      ...
      if (ui && ui.getSelections) {
        ...
        if (selectionLength === 0 && StringUtils.isNotBlank(promptMsg)) {
          ...
          _self.dmsDataServices.performed({
            ui: ui,
            event: ui.event,
            params: params,
            urlParams: urlParams,
    ...
    btn_file_manager_list_view_as_prints: function (options) {
      var _self = this;
      var actionFunction = options.appFunction || {};
      // 2、执行后台Java操作
      var ui = options.ui;
      // 2.1 视图操作
      if (ui && ui.getSelection) {
        // 当前操作的数据
        var selection = options.rowData != null ? options.rowData : ui.getSelection();
        if (!selection.length) {
          appModal.error('请先选择记录！');
          return false;
        }
        var folderUuids = _.map(selection, function (item) {
          return item.folderUuid;
        });
        _self.getFilePrintTemplates(folderUuids);
      }
    },
    ...
    printHandle: function (templateId, templateUuid) {
      ...
      for (var i = 0; i < selection.length; i++) {
        var dmsObject = selection[i];
        dataUuids.push(dmsObject.dataUuid);
        formUuids.push(dmsObject.formUuid || dmsObject.dataDefUuid);
        contentTypes.push(dmsObject.contentType);
      }
      var params = {
        dataUuids: dataUuids,
        formUuids: formUuids,
      ...
      JDS.restfulPost({
        url: ctx + '/proxy/api/dms/printtemplate/prints',
        data: params,
        mask: true,
        success: function (result) {
          if (result.code == 0) {
  ...
  return DmsListViewTablePrintAction;
});
