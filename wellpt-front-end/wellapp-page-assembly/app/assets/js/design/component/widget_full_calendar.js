define([
  ...
  var checkRequire = function (propertyNames, options, $container) {
    ...
    for (var i = 0; i < propertyNames.length; i++) {
      ...
      if (
        (!options.enableCalendarView && propertyName === 'eventLimitNum') ||
        (options.isSimple && $.inArray(propertyName, ['name', 'categoryCode']) < 0)
      ) {
        return true;
      }
  ...
  configurer.prototype.initBaseInfo = function (configuration, $container) {
    ...
    $('#isSimple', $container)
      .on('change', function () {
        var $this = $(this);
        if ($this.is(':checked')) {
          $('.notSimple', $container).hide();
          $('.isSimple', $container).show();
        } else {
          $('.notSimple', $container).show();
          $('.isSimple', $container).hide();
        }
      })
      .trigger('change');
  };
