...
(function (factory) {
  ...
  var DefaultMsgEvent = {
    myMsgEvent: function () {
      ...
      $.ajax({
        ...
        url: ctx + '/message/inbox/queryRecentTenLists',
        success: function (result) {
          ...
          if (res.code == 0) {
            ...
            DefaultMsgEvent.getMsgIcons($myMsgWrap);
            // 消息弹窗
      ...
      $(_self.element).on('click', '#msgSetIcon', function (e) {
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if (res.code == 0) {
              list = res.data;
              var message = DefaultMsgEvent._initMsgSetDialog(list);
              DefaultMsgEvent._openMsgSetDialog(message, list);
            }
          }
        });
      });
        ...
        shown: function () {
          ...
          $('.msg-set-wrap').on('click', '.msg-set-folder', function () {
            if ($(this).hasClass('icon-ptkj-shixinjiantou-you')) {
              ...
              $(this).parents('li').find('.hasClassify').slideDown();
            } else {
              $(this).addClass('icon-ptkj-shixinjiantou-you').removeClass('icon-ptkj-shixinjiantou-xia');
              $(this).parents('li').find('.hasClassify').slideUp();
            }
          });
        }
      });
  ...
  window.DefaultMsgEvent = DefaultMsgEvent;
  return DefaultMsgEvent;
});
