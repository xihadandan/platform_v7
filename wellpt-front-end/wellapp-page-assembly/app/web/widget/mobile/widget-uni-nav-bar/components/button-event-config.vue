<template>
  ...
  >
    ...
    <template v-if="eventModel.actionType === 'showPopup'">
      <a-form-model-item label="弹出方式">
        <a-radio-group v-model="eventModel.popupType" :options="popupTypeOptions"></a-radio-group>
      </a-form-model-item>
    </template>
    ...
    <div v-if="allowEventParams">
      ...
      >
        ...
        <template slot="paramValueSlot" slot-scope="text, record">
          <template v-if="originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId]">
            <template v-if="record.valueSource">
              <a-select
                v-if="record.valueSource.inputType == 'select'"
                :getPopupContainer="getSelectPopupContainer"
                :options="
                  originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId]
                    ? originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId].options || []
                    : []
                "
              ...
              />
              <a-select
                v-else-if="record.valueSource.inputType == 'group-select'"
                :getPopupContainer="getSelectPopupContainer"
                v-model="record.paramValue"
                show-arrow
                allow-clear
              ...
              >
                <a-select-opt-group
                  v-for="(grp, i) in originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId]
                    ? originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId].options || []
                    : []"
              ...
              <a-tree-select
                v-else-if="record.valueSource.inputType == 'tree-select'"
                show-search
                ...
                "
                :replaceFields="
                  originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId].replaceFields || {
                    children: 'children',
                    title: 'title',
                    key: 'key',
                    value: 'value'
                ...
                "
                :showCheckedStrategy="
                  originalWidgetEventParamValueSourceMap[eventModel.eventWid + ':' + eventModel.eventId].showCheckedStrategy || 'SHOW_CHILD'
                "
              ></a-tree-select>
            </template>
  ...
  computed: {
    ...
    actionTypeOptions() {
      const options = [
        ...
        { label: '显示抽屉', value: 'showPopup' },
        { label: '脚本代码', value: 'jsFunction' },
        { label: '触发组件事件', value: 'widgetEvent' },
        { label: '数据管理', value: 'dataManager' }
      ];
      return options;
    },
    targetPosition() {
    ...
    originalWidgetEventParamValueSourceMap() {
      let map = {};
      if (this.eventModel.actionType === 'widgetEvent' && this.eventModel.eventWid && this.eventModel.eventId) {
        ...
        if (wid && eventId && map[`${wid}:${eventId}`] == undefined) {
          let w = this.designer.widgetIdMap[wid];
          if (w) {
            let defaultEvents = this.designer.widgetDefaultEvents[w.id];
            if (defaultEvents) {
              for (let i = 0, len = defaultEvents.length; i < len; i++) {
                if (defaultEvents[i].id === eventId && defaultEvents[i].eventParams != undefined) {
                  for (let e = 0, elen = defaultEvents[i].eventParams.length; e < elen; e++) {
                    if (defaultEvents[i].eventParams[e].valueSource) {
                      map[`${wid}:${eventId}`] = defaultEvents[i].eventParams[e].valueSource;
                    }
                  }
                }
              }
