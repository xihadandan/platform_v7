<template>
  ...
  >
    ...
    <template v-if="eventModel.actionType === 'showDrawer'">
      <a-form-model-item label="弹出方式">
        <a-radio-group v-model="button.drawerWidget.configuration.direction" :options="popupTypeOptions"></a-radio-group>
      </a-form-model-item>
      <a-form-model-item label="渲染组件名">
        <a-input v-model="button.drawerWidget.configuration.templateName"></a-input>
      </a-form-model-item>
    </template>
    ...
    <a-form-model-item
      v-if="designer != undefined && eventRule.dyformSetable != false && widget && widget.wtype !== 'WidgetTable'"
      v-show="eventModel.actionType === 'dataManager'"
      label="关联数据表格"
    >
      <a-select
        :style="{ width: '100%' }"
        :options="filterWidgetSelectOptions(['WidgetTable'])"
    ...
    <div v-show="eventModel.actionType === 'redirectPage'" v-if="eventRule.pageSelect !== false">
      ...
      <a-form-model-item label="选择页面" v-show="eventModel.pageType === 'page'">
        <a-row type="flex">
          <a-col flex="auto" :style="{ width: eventModel.pageUuid || eventModel.pageId ? 'calc(100% - 80px)' : '100%' }">
            <a-select
              allowClear
              :filter-option="filterSelectOption"
              :showSearch="true"
          ...
          <a-col flex="70px" v-if="eventModel.pageUuid || eventModel.pageId">
    ...
    <div v-show="eventModel.actionType === 'workflow'" v-if="eventRule.workflowSelect !== false">
      <a-form-model-item>
        <template slot="label">
          <a-select v-if="widget && widget.wtype == 'WidgetTable'" v-model="eventModel.workflowIdSource" style="width: 90%">
            <a-select-option value="constant">选择流程</a-select-option>
            <a-select-option value="selectedRowColumnValue">流程 ID 列</a-select-option>
          </a-select>
          <template v-else>选择流程</template>
        </template>
        <a-select
          v-if="eventModel.workflowIdSource === 'constant' || eventModel.workflowIdSource == undefined"
          allowClear
          showSearch
          :filter-option="filterSelectOption"
          :style="{ width: '100%' }"
          ...
          :getPopupContainer="getSelectPopupContainer"
        ></a-select>
        <template v-if="eventModel.workflowIdSource == 'selectedRowColumnValue'">
          <a-select v-model="eventModel.workflowIdColumn" allow-clear :style="{ width: '100%' }">
            <template v-for="(col, c) in widget.configuration.columns">
              <a-select-option :key="'workflowIdCol_' + col.id" :value="col.dataIndex">
                {{ col.title }}
          ...
          <a-alert message="发起的流程定义是已选行数据对应的列值, 可用于发起与行数据同类流程业务"></a-alert>
        </template>
      </a-form-model-item>
      <a-form-model-item>
        <template slot="label">
    ...
    <a-form-model-item
      label="加载后选中元素"
      v-if="eventRule.positionSelectable !== false && ['redirectPage'].includes(eventModel.actionType)"
    >
      <a-select v-model="eventModel.pageLoadedSelectElementId" mode="multiple" @select="onSelectPageSelectElement">
        <a-select-opt-group v-for="(wgt, w) in filterPageSelectableElements" :key="'spse_' + w" :label="wgt.title">
          <a-select-option v-for="(opt, i) in wgt.children" :key="'spse_' + w + '_' + i" :value="opt.id" :group="wgt">
            {{ opt.title }}
          </a-select-option>
        </a-select-opt-group>
      </a-select>
    ...
    <a-form-model-item label="目标标题" v-if="eventModel.targetPosition && eventModel.actionType == 'redirectPage'">
      <slot name="redirectPageTitle">
        <a-input v-model="eventModel.title" />
      </slot>
    </a-form-model-item>
    <slot name="extraOptions" :eventConfig="eventModel"></slot>
    <div v-if="allowEventParams">
      ...
      >
        ...
        <template slot="paramKeySlot" slot-scope="text, record">
          ...
          <a-input v-model="record.paramKey" style="width: 100px"></a-input>
        </template>
        <template slot="paramValueSlot" slot-scope="text, record">
          <template v-if="originalEventParamValueSourceMap != undefined && originalEventParamValueSourceMap[record.paramKey] != undefined">
            <template v-if="originalEventParamValueSourceMap[record.paramKey].valueSource">
              <a-select
                v-if="originalEventParamValueSourceMap[record.paramKey].valueSource.inputType == 'select'"
                :getPopupContainer="getSelectPopupContainer"
                :options="originalEventParamValueSourceMap[record.paramKey].valueSource.options || []"
                v-model="record.paramValue"
                show-arrow
                allow-clear
              />
              <a-select
                v-else-if="originalEventParamValueSourceMap[record.paramKey].valueSource.inputType == 'multi-select'"
                :options="originalEventParamValueSourceMap[record.paramKey].valueSource.options || []"
                :getPopupContainer="getSelectPopupContainer"
                v-model="record.paramValue"
                show-arrow
              ...
              />
              <a-select
                v-else-if="originalEventParamValueSourceMap[record.paramKey].valueSource.inputType == 'group-select'"
                :getPopupContainer="getSelectPopupContainer"
                v-model="record.paramValue"
                show-arrow
                allow-clear
              ...
              >
                <a-select-opt-group
                  v-for="(grp, i) in originalEventParamValueSourceMap[record.paramKey].valueSource.options || []"
                  :key="'optionGroup-' + grp.uuid"
                >
                  <span slot="label" class="select-group-divider">
                    <label :title="grp.label">{{ grp.label }}</label>
                  </span>
                  <a-select-option v-for="(opt, index) in grp.options" :key="opt.value" :value="opt.value" :title="opt.label">
                    {{ opt.label }}
                  </a-select-option>
                </a-select-opt-group>
              </a-select>
              <a-tree-select
                v-else-if="originalEventParamValueSourceMap[record.paramKey].valueSource.inputType == 'tree-select'"
                show-search
                ...
                show-arrow
                allow-clear
                :tree-data="originalEventParamValueSourceMap[record.paramKey].valueSource.options || []"
                :replaceFields="
                  originalEventParamValueSourceMap[record.paramKey].replaceFields || {
                    children: 'children',
                    title: 'title',
                    key: 'key',
                    value: 'value'
                ...
                "
                :showCheckedStrategy="originalEventParamValueSourceMap[record.paramKey].showCheckedStrategy || 'SHOW_CHILD'"
              ></a-tree-select>
            </template>
            <a-auto-complete
              v-else
              :data-source="originalEventParamValueSourceMap[record.paramKey].valueScope || []"
              v-model.trim="record.paramValue"
              allow-clear
            />
          </template>
          <a-input v-model="record.paramValue" allow-clear v-else />
        </template>
...
import WidgetCodeEditor from './widget-code-editor.vue';
...
export default {
  name: 'WidgetEventHandler',
  inject: ['appId', 'subAppIds', 'injectEventActionType'],
  props: {
  ...
  data() {
    ...
    return {
      ...
      eventParamColumns: [
        ...
        { title: '备注', width: 100, dataIndex: 'remark', scopedSlots: { customRender: 'remarkSlot' } }
      ],
  ...
  computed: {
    ...
    actionTypeOptions() {
      ...
      if (this.injectEventActionType) {
        options = [...this.injectEventActionType, ...options];
      }
      return options;
    },
    targetPosition() {
    ...
    vWidgetOptions() {
      ...
      if (this.designer) {
        for (let wid in this.designer.widgetIdMap) {
          let w = this.designer.widgetIdMap[wid];
          options.push({ label: w.configuration.isDatabaseField ? w.configuration.name || w.title : w.title, value: w.id, wtype: w.wtype });
        }
      }
      return options;
    },
    ...
    originalEventParamValueSourceMap() {
      let map = {};
      if (this.eventModel.actionType === 'widgetEvent' && this.eventModel.eventWid && this.eventModel.eventId) {
        // 组件事件参数
        let wid = this.eventModel.eventWid,
          eventId = this.eventModel.eventId;
        if (wid && eventId) {
          let w = this.designer.widgetIdMap[wid];
          if (w) {
            let defaultEvents = this.designer.widgetDefaultEvents[w.id];
                ...
                if (defaultEvents[i].id === eventId && defaultEvents[i].eventParams != undefined) {
                  for (let e = 0, elen = defaultEvents[i].eventParams.length; e < elen; e++) {
                    if (defaultEvents[i].eventParams[e]) {
                      map[`${defaultEvents[i].eventParams[e].paramKey}`] = {
        ...
        ['jsFunction', 'dataManager'].includes(this.eventModel.actionType) &&
        (this.eventModel.action || this.eventModel.jsFunction)
      ) {
            ...
            if (meta && meta.hook) {
              for (let h in meta.hook) {
                ...
                if (typeof meta.hook[h] !== 'string' && meta.hook[h].eventParams) {
                  if (this.eventModel.action == value) {
  ...
  created() {
    ...
    if (this.eventModel.workflowIdSource == undefined) {
      this.$set(this.eventModel, 'workflowIdSource', 'constant');
    }
  },
  methods: {
    getDrawerContainer() {
    ...
    onClickOpenDesignPage() {
      let key = this.eventModel.pageUuid || this.eventModel.pageId;
      let page = this.pageUuidMap[key];
      if (page) {
        window.open(`/${page.wtype == 'vUniPage' ? 'uni-' : ''}page-designer/index?uuid=${page.uuid}`, '_blank');
      }
    },
    ...
    onChangeActionType(v, option) {
      ...
      if (v === 'showDrawer' && this.button.drawerWidget === undefined) {
        const wgtId = generateId();
        const drawerWidget = {
          wtype: 'WidgetTemplate',
          id: wgtId,
          configuration: {
            title: '自定义模板',
            code: undefined, //编码
        ...
        this.$set(this.button, 'drawerWidget', drawerWidget);
      }
    },
    ...
    onSelectPageChange(v, node) {
      ...
      if (v) {
        ...
        this.getPageDefinitionJson(uuid).then(data => {
          ...
          this.getPageFilterSelectableElements(json);
        });
      } else {
    ...
    getPageFilterSelectableElements(json) {
      let { appWidgetDefinitionElements, functionElements } = json;
      let filterSelectableElements = [];
      if (appWidgetDefinitionElements && functionElements) {
        let wMap = {};
        for (let w of appWidgetDefinitionElements) {
          let item = { wtype: w.wtype, title: w.title, id: w.id, children: [] };
          wMap[w.id] = item;
          filterSelectableElements.push(item);
        }
        for (let wid in wMap) {
          let fElements = functionElements[wid];
          if (fElements) {
            let item = wMap[wid];
            for (let f of fElements) {
              if (f.type == 'tab' || f.selectable) {
                item.children.push({
                  id: f.id,
                  title: wMap[f.id] != undefined ? wMap[f.id].title : f.name
                });
              }
            }
          }
        }
        this.filterPageSelectableElements.splice(0, this.filterPageSelectableElements.length);
        this.filterPageSelectableElements.push(
          ...filterSelectableElements.filter(item => {
    ...
    fetchWorkflowOptions() {
      ...
      let appIds = [].concat(this.appId || []).concat(this.subAppIds || []);
      if (appIds.length > 0) {
        $axios.post('/proxy/api/workflow/definition/module/query', appIds).then(res => {
          _this.workflowOptions = getWorkflowOptions(res.data || {});
        });
      }
    },
    fetchActionOptions() {
    ...
    onSelectPageSelectElement(value, option) {
      let group = option.data.attrs.group;
      for (let c of group.children) {
        if (this.eventModel.pageLoadedSelectElementId.includes(c.id) && c.id !== value) {
          this.eventModel.pageLoadedSelectElementId.splice(this.eventModel.pageLoadedSelectElementId.indexOf(c.id), 1);
        }
      }
    },
    filterWidgetSelectOptions(input) {
      let options = [];
      for (let w of this.vWidgetOptions) {
        if ((typeof input == 'string' && w.wtype == input) || (Array.isArray(input) && input.includes(w.wtype))) {
          options.push(w);
        }
        if (w.wtype == 'WidgetDataManagerView') {
          if (input == 'WidgetTable' || (Array.isArray(input) && input.includes('WidgetTable'))) {
            options.push({
              label: designer.widgetIdMap[w.value].configuration.WidgetTable.title,
              value: designer.widgetIdMap[w.value].configuration.WidgetTable.id
            });
          }
        }
      }
  ...
  mounted() {
    ...
    if (this.eventModel.pageUuid) {
      this.getPageDefinitionJson(this.eventModel.pageUuid).then(data => {
        let json = JSON.parse(data.definitionJson);
        this.getPageFilterSelectableElements(json);
      });
    }
