  ...
  <div>
    <a-form-model-item class="item-lh" :label-col="{ style: { width: '100%' } }">
      <template slot="label">
        ...
        <template v-else>满足条件时{{ controlDescription }}</template>
      </template>
    </a-form-model-item>
    <a-form-model-item
      :label="undefined"
      v-if="configuration[configCode].enable"
          ...
          <template v-for="(con, i) in configuration[configCode].conditions">
            <a-row type="flex" :key="'visible_con_' + i">
              ...
              <a-col flex="50px" style="align-self: center; text-align: center">
                <a-button type="link" size="small" @click="configuration[configCode].conditions.splice(i, 1)" title="删除">
...
import { operatorOptions } from './constant.js';
import moment from 'moment';

export default {
  name: 'DataConditionalControlConfiguration',
  ...
  created() {
    if (!this.configuration.hasOwnProperty(this.configCode)) {
