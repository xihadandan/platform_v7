            ...
            <template v-if="importRule.importFileTemplate.fileID">
              ...
              >
                ...
                <template slot="content">
                  <ImportSheetConfiguration :import-rule="importRule" :widget="widget" :designer="designer" :button="button" />
                </template>
              </WidgetDesignDrawer>
            </template>
          </div>
...
export default {
  ...
  beforeCreate() {},
  created() {
    if (Object.keys(this.importRule.importFileTemplate).length == 0) {
      this.$set(this.importRule.importFileTemplate, 'fileName', undefined);
      this.$set(this.importRule.importFileTemplate, 'fileID', undefined);
      this.$set(this.importRule.importFileTemplate, 'url', undefined);
    }
  },
  beforeMount() {
    // 获取vue模板实例
  ...
  methods: {
    ...
    customRequest(options, afterUpload) {
      ...
      this.$set(this.importRule.importFileTemplate, 'fileID', undefined);
      this.$set(this.importRule.importFileTemplate, 'url', undefined);
      WidgetTable.methods.customFileUploadRequest(options).then(dbFile => {
