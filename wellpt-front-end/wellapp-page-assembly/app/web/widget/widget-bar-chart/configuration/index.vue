  ...
  <a-form-model :model="widget.configuration" labelAlign="left" :wrapper-col="{ style: { textAlign: 'right' } }" :colon="false">
    <a-tabs default-active-key="1">
      <a-tab-pane key="1" tab="设置">
        ...
        <a-collapse :bordered="false" expandIconPosition="right" defaultActiveKey="datasetSetting">
          ...
          <a-collapse-panel key="axisSetting" header="坐标系">
            <AxisConfiguration :widget="widget" :designer="designer" />
          </a-collapse-panel>
...
import DatasetConfiguration from './dataset-configuration.vue';
import SeriesListConfiguration from './series-list-configuration.vue';
import AxisConfiguration from './axis-configuration.vue';
import ColorConfiguration from './color-configuration.vue';
import TitleConfiguration from './title-configuration.vue';
import LegendConfiguration from './legend-configuration.vue';
import EchartsVersion from './echarts-version.vue';
import { generateId } from '@framework/vue/utils/util';

export default {
  name: 'WidgetBarChartConfiguration',
  ...
  configuration(widget) {
    ...
    if (widget != undefined && widget.useScope == 'bigScreen') {
      ...
      conf.axis[0].x.axisLabel.color = '#fff';
      conf.axis[0].y.axisLabel.color = '#fff';
    }
    return conf;
  }
};
