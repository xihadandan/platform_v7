...
import WorkFlowInteraction from "./uni-WorkFlowInteraction.js";
...
var WorkFlow = function () {
  WorkFlowInteraction.apply(this, arguments);
...
Object.assign(WorkFlow.prototype, WorkFlowInteraction.prototype, {
  ...
  _ajaxService: function (ajaxUrl, type, data, successCallback, failureCallback) {
    ...
    if (_action) {
      uni.showLoading({ title: _action ? _action + "中" : "提交中" });
    }
    uni.request({
      method: type,
      url: ajaxUrl,
  ...
  complete: function (successCallback, failureCallback) {
    ...
    let ajaxUrl = "/api/workflow/work/complete";
  ...
  getPrintTemplates: function (successCallback, failureCallback) {
    ...
    var ajaxUrl = "/api/workflow/work/getPrintTemplateTree";
    var data = {
  ...
  getDyformFileFieldDefinitions: function (successCallback, failureCallback) {
    var _self = this;
    _self._service(
      services.getDyformFileFieldDefinitions,
      [_self.workData.taskInstUuid],
      successCallback,
      failureCallback
    );
    ...
    uni.showModal({
      ...
      success: function (result) {
        if (result.confirm === true) {
          var ajaxUrl = "/api/workflow/work/falseDeleteByAdmin";
          var data = {
            taskInstUuids: taskInstUuids,
          };
          _self._ajaxPostService(ajaxUrl, data, successCallback, failureCallback);
  ...
  getSettings() {
    ...
    this._ajaxGetService(
      "/api/workflow/setting/list",
      {},
      ({ data: result }) => {
        if (result.data) {
          let settings = utils.deepClone(DEFAULT_SETTINGS);
