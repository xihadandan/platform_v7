<template>
  <uni-forms-item :label="itemLabel" :name="formModelItemProp" :label-position="widget.configuration.labelPosition">
    <template v-if="!displayAsLabel">
      <template v-if="range">
        <uni-w-datetime-picker
          :clearIcon="widget.configuration.clearBtnShow"
          :format="fixedFormat"
    ...
    <view v-else>{{ computedLabel }} </view>
  </uni-forms-item>
</template>
...
import { padCheck } from "@/uni_modules/uni-w-datetime-picker/components/uni-w-datetime-picker/util.js";
...
export default {
  ...
  computed: {
    ...
    placeholder() {
      let placeholder = "请选择时间";
      if (this.range) {
        placeholder = "开始时间";
        if (this.widget.configuration.placeholder) {
          let _placeholder = this.widget.configuration.placeholder.split(";");
      ...
      return this.widget.configuration.placeholder || placeholder;
    },
  ...
  methods: {
    setValue(value) {
      let newValue = value || this.formData[this.fieldCode];
      if (this.form.dataUuid) {
        if (newValue) {
          // newValue = padCheck(newValue, this.contentFormat, this.mode);
        }
        if (this.range) {
          let endValue = this.formData[this.endDateFieldKey];
          if (endValue) {
            // endValue = padCheck(endValue, this.contentFormat, this.mode);
          }
          if (newValue && endValue) {
            this.date = [newValue, endValue];
          }
        } else {
          if (newValue) {
            this.date = newValue;
    ...
    setRangeBySystemDate() {
      this.setSystemDate().then(() => {
        ...
        this.getSystemDate({
          ...
          this.minDate = date.format(this.contentFormat);
        });
        ...
        this.getSystemDate({
          ...
          this.maxDate = date.format(this.contentFormat);
        });
      });
    },
        ...
        this.$axios
          ...
          .then(({ data, headers }) => {
            if (data.code == 0 && data.data) {
              if (!this.systemDate) {
                this.systemDate = moment(headers.date).format(this.defaultPattern);
              }
