  ...
  >
    <template v-if="!displayAsLabel">
      <view v-if="widget.configuration.type == 'progress'" style="width: 100%">
        <view v-show="showInput" style="margin-bottom: 8px" class="flex f_y_c">
          <uni-w-number-box
...
export default {
  ...
  methods: {
    ...
    onChange() {
      this.changePercent();
      this.emitChange();
      this.$refs[this.widget.configuration.code].onFieldChange();
      console.log(this.formData[this.widget.configuration.code]);
    },
    changePercent() {
      if (this.widget.configuration.type == "progress") {
        let percent = this.formData[this.widget.configuration.code];
