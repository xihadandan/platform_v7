<template>
  <view class="w-swiper" :class="customClass" :style="swiperDotStyle" v-if="isEmptyUniJsModule">
    <uni-swiper-dot
      ...
      >
        <swiper-item v-for="item in swiperItems" :key="item.id">
          <view class="swiper-item" :style="swiperStyle" @click="clickSwiperItem(item)">
            ...
            <view class="swiper-text" v-if="item.text">{{ item.text }}</view>
          </view>
        </swiper-item>
      </swiper>
    </uni-swiper-dot>
...
export default {
  ...
  computed: {
    dotsStyles() {
      ...
      if (this.configuration.dotColor) {
        dotsStyles.backgroundColor = this.configuration.dotColor;
        dotsStyles.border = "1px " + this.configuration.dotColor + " solid";
      }
      if (this.configuration.dotSelColor) {
        dotsStyles.selectedBackgroundColor = this.configuration.dotSelColor;
        dotsStyles.selectedBorder = "1px " + this.configuration.dotSelColor + " solid";
      }
      return dotsStyles;
    },
  },
