        ...
        >
          <view class="uni-flex uni-column">
            ...
            <view class="uni-flex uni-row">
              ...
              <button
                ...
                style="width: 56px; background-color: #fff"
              >
                补号
              </button>
            </view>
          ...
          <view class="uni-flex uni-column">
            ...
            <view class="uni-flex uni-row" style="align-items: center">
              ...
              <uni-easyinput v-model="serialNumberSelectInputPointer" :clearable="false" />
...
export default {
  ...
  computed: {
    ...
    dialogTitle() {
      return this.widget.configuration.dialogTitle || "可编辑流水号";
    },
    dialogSnDefSelectLabel() {
      return this.widget.configuration.dialogSnDefSelectLabel || "流水号名称";
    },
    dialogSnSelectLabel() {
      return this.widget.configuration.dialogSnSelectLabel || "可选择流水号";
    },
    dialogSnLabel() {
      return this.widget.configuration.dialogSnLabel || "流水号";
    },
    // 指针格式化前缀
    pointerFormatValuePrefix() {
      const _this = this;
  ...
  methods: {
    ...
    afterSaveDataError(params) {
      ...
      if (data.fieldName === this.fieldCode) {
        ...
        this.$confirm({
          title: "提示",
          content: h("div", {}, [
            h("p", `字段名称：${data.displayName}`),
            h("p", `${data.title}，是否重新提交` || "报错失败"),
          ]),
          onOk: () => {
    ...
    onFillSerialNumberClick() {
      const _this = this;
      if (isEmpty(_this.serialNumberDefinitionSelectId)) {
        uni.showToast({ icon: "error", title: "请选择流水号定义！" });
        return;
      }
      let params = _this.getSerialNumberBuildParams();
      params.serialNumberId = _this.serialNumberDefinitionSelectId;
    ...
    onChooseSerialNumberOk() {
      ...
      if (isEmpty(selectRecord.serialNo)) {
        uni.showToast({ icon: "error", title: "请选择流水号" });
        return;
      }
    ...
    userCustomSerialNumberOk() {
      ...
      if (!pointer) {
        uni.showToast({ icon: "error", title: "请输入有效的指针!" });
        return;
      }
      ...
      this.$axios
        ...
        .then((result) => {
          ...
          if (serialNumberInfo == null) {
            uni.showToast({ icon: "error", title: "该指针无效或已经被占用!" });
          } else {
        ...
        .catch((error) => {
          if (error.response.data && error.response.data.msg) {
            ...
            uni.showToast({ icon: "error", title: "服务请求异常！" });
          }
        });
    },
    // 用户选择的补号
      ...
      if (requiredRemoteCheck) {
        ...
        this.$axios
          ...
          .then((result) => {
            ...
            if (isOccupied == true) {
              uni.showToast({ icon: "error", title: "该流水号已经被占用!" });
            } else {
              checkOk(false);
            }
          });
