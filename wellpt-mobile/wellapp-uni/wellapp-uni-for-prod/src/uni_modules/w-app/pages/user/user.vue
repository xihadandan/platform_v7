<template>
  <view :style="theme" class="">
    <view class="center">
      ...
      <view v-if="hasLogin" class="center-list" @click="goToThemeSettings">
        <view class="center-list-item">
          <text class="list-icon">&#xe60b;</text>
          <text class="list-text">主题设置</text>
          <text class="navigat-arrow">&#xe65e;</text>
        </view>
      </view>
      <view v-if="hasLogin" class="center-list" @click="goToWorkbenchSettings">
        <view class="center-list-item">
          <text class="list-icon">&#xe60b;</text>
          <text class="list-text">工作台</text>
          <text class="navigat-arrow">&#xe65e;</text>
        </view>
      </view>
      <!-- #ifdef APP-PLUS -->
      <view class="center-list" @click="gotoAbout">
        <view class="center-list-item">
          <text class="list-icon">&#xe614;</text>
...
export default {
  ...
  methods: {
    ...
    bindLogout() {
      ...
      if (loginType === "local") {
        ...
        store.dispatch("logout").then(() => {
          if (this.forcedLogin) {
            uni.reLaunch({
              url: "/pages/oa/user/login" || "/pages/login/login",
            });
          }
        });
        return;
    ...
    goto() {
      uni.navigateTo({
        url: "/uni_modules/w-app/pages/pwd/update-password",
      });
    },
    goToThemeSettings() {
      uni.navigateTo({
        url: "/uni_modules/w-app/pages/theme/settings",
      });
    },
    goToWorkbenchSettings() {
      uni.navigateTo({
        url: "/uni_modules/w-app/pages/user/workbench_settings",
      });
    },
    gotoAbout() {
      uni.navigateTo({
        url: "/uni_modules/w-app/pages/about/about",
      });
    },
  },
};
