...
export function getMsgI18ns(msgJson, callbackContext) {
  let msg = msgJson && typeof msgJson === "object" ? msgJson.title : msgJson;
  let msgI18n = msg;
  if (!msg) {
    return msgI18n;
  }
  ...
  for (const key in msgMap) {
    if (msg.indexOf(msgMap[key]) !== -1) {
      if (key === msgMap.currentTaskSuspendNotOperated) {
        // 全部替换
        msgI18n = $widget.$t(`WorkflowWork.message.${key}`);
      } else {
        msgI18n = msg.replace(new RegExp(msgMap[key], "g"), $widget.$t(`WorkflowWork.message.${key}`, msgMap[key]));
      }
    }
  }
  if (typeof msgJson === "object") {
    msgJson.title = msgI18n;
    return msgJson;
  }
  return msgI18n;
}
...
function TaskNotAssignedDecisionMaker(faultData, options) {
  ...
  openOrgSelect({
    ...
    labelField: "",
    title:
      callbackContext.$widget.$t("WorkflowWork.modal.pleaseChooseDecisionMaker", "选择决策人") + "【" + title + "】",
    type: "all",
    multiple: true,
    selectTypes: "all",
    valueFormat: "justId",
...
function taskNotAssignedActionUser(faultData, options, userProp, labelProp, actionName) {
  ...
  if (users && users.length > 0) {
    title =
      callbackContext.$widget.$t("WorkflowWork.modal.chooseActionNameUser", { actionName }, `选择${actionName}人员`) +
      `【${taskName}】`;
...
function showUserByUnit(faultData, options, multiple) {
  ...
  var title =
    callbackContext.$widget.$t("WorkflowWork.modal.pleaseChooseUndertaker", "选择承办人") + "【" + taskName + "】";
  // 选择新的组织弹出框 zyguo
  var okCallback = function (values, labels, treeNodes) {
    var taskUsers = {};
    var taskId = eData.taskId;
    var taskUserJobPaths = workData.taskUserJobPaths || {};
    if (values && values.length > 0) {
      // 在原来的环节办理人上增加环节办理人
      taskUsers = workData.taskUsers;
      taskUsers[taskId] = values;
...
function openOrgSelect({
  ...
  if ($widget) {
    $widget.setErrorOrgSelectOptions({
      ...Props,
    });
  }
}
...
function SubFlowNotFound(faultData, options) {
  ...
  console.log(options);
  showToast({
    title: options.callbackContext.$widget.$t(
      "WorkflowWork.message.noSubWorkflowSetTip",
      "流程没有配置要发起的子流程!"
    ),
    icon: "error",
  });
...
function IdentityNotFlowPermission(faultData, options) {
  console.log(options);
  var eData = faultData.data;
  showToast(options.callbackContext, {
    title: eData.msg,
    icon: "error",
  });
}
...
function RollbackTaskNotFound(faultData, options) {
  ...
  var workFlow = options.workFlow;
  var workData = workFlow.getWorkData();
  var toTasks = setTaskI18s(eData.rollbackTasks, callbackContext.$widget);

  options.callbackContext.onError("rollbackTaskNotFound", {
    title: callbackContext.$widget.$t("WorkflowWork.modal.pleaseChooseRollbackTask", "选择退回环节"),
    toTasks: toTasks,
    options: options,
  });
}
...
function GotoTaskNotFound(faultData, options) {
  ...
  // var workFlow = options.workFlow;
  // var workData = workFlow.getWorkData();
  var toTasks = setTaskI18s(eData.toTasks, callbackContext.$widget);
  options.callbackContext.onError("gotoTaskNotFound", {
    title: callbackContext.$widget.$t("WorkflowWork.modal.pleaseChooseGotoTask", "选择特送环节"),
    fromTaskId: eData.fromTaskId,
    toTasks: toTasks,
    options: options,
  });
...
function SaveData(faultData, options) {
  ...
  if (workView && workView.getDyformWidget && isFunction(workView.getDyformWidget)) {
    ...
    showToast(options.callbackContext, {
      title: options.callbackContext.$widget.$t("WorkflowWork.message.saveFormDataError", "表单数据保存失败!"),
      icon: "error",
    });
  }
}
...
function RequiredFieldIsBlank(faultData, options) {
  ...
  console.log(options);
  showToast(options.callbackContext, {
    title: options.callbackContext.$widget.$t("WorkflowWork.message.formDataRequiredMiss", "表单必填域为空!"),
    icon: "error",
  });
}
...
function ChooseArchiveFolder(faultData, options) {
  ...
  showToast(options.callbackContext, {
    title: options.callbackContext.$widget.$t(
      "WorkflowWork.message.unSupportSelectFolder",
      "不支持选择归档夹，请在电脑端操作！"
    ),
    icon: "error",
  });
...
function MultiJobNotSelected(faultData, options) {
  ...
  for (var i = 0; i < faultData.data.jobs.length; i++) {
    let job = faultData.data.jobs[i];
    var text = "";
    if (job.jobNamePath) {
      var paths = job.jobNamePath.split("/");
      if (paths.length > 1) {
...
function Default(faultData) {
  ...
  showToast(faultData.callbackContext, {
    title:
      faultData.callbackContext.$widget.$t(
        "WorkflowWork.message.unknownWorkflowException",
        "工作流无法处理的未知异常"
      ) +
      ": " +
      msg,
...
function showToast(callbackContext, content) {
  content = getMsgI18ns(content, callbackContext);
  if (callbackContext && callbackContext.showToast) {
    callbackContext.showToast(content);
  } else {
    uni.showToast({
      title: content.title || "",
      icon: content.msgIcon || "error",
    });
    if (content.timer) {
      setTimeout(() => {
        if (isFunction(content.callback)) {
          content.callback.call();
        }
      }, content.timer);
    } else if (isFunction(content.callback)) {
      content.callback.call();
    }
  }
}
