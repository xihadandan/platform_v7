import { isFunction, isEmpty, trim, each, indexOf, map, assign, sortBy } from "lodash";
import WorkFlow from "./uni-WorkFlow.js";
import WorkFlowErrorHandler, { getMsgI18ns } from "./uni-WorkFlowErrorHandler.js";
...
WorkView.prototype.getActions = function () {
  ...
  if (this.options.settings) {
    buttonsSetting = this.options.settings["ACTION"].buttons;
  }
  ...
  each(buttons, function (button) {
    ...
    var setting = buttonsSetting.find((action) => action.code == code);
    // 国际化文本信息
    ...
    if (isValidJSON(button.className)) {
      ...
      icon = className.icon;
      type = className.type;
    } else if (setting && setting.style) {
      const className = setting.style;
      icon = className.icon;
      type = className.type;
    } else if (button.btnIcon) {
      icon = button.btnIcon;
    }
    if (button.sortOrder == 0 && setting && setting.sortOrder) {
      button.sortOrder = setting.sortOrder;
    }
...
WorkView.prototype.setEventAction = function (action, actionType, workData) {
  workData.action = this.getMsgI18ns("WorkflowWork.operation." + actionType, action);
...
WorkView.prototype.openToSignIfRequired = function (defaultLabel, action) {
  ...
  if (!isEmpty(opinionText)) {
    if (_self.isRequiredOpinionPosition() && _self.isTodo()) {
      if (!isEmpty(workData.opinionValue)) {
        return false;
      }
    } else {
      return false;
...
WorkView.prototype.opinionToWorkData = function (data) {
  var _self = this;
  if (!data) {
    let widgetOpinion = _self.$widget.signOptionOptions;
    data = {
      label: widgetOpinion.opinionLabel,
      files: widgetOpinion.opinionFiles,
      text: widgetOpinion.opinionText,
      value: widgetOpinion.opinionValue,
    };
  }
  var workData = _self.workFlow.getWorkData();
...
WorkView.prototype.collectFormData = function () {
  let dyformWidget = this.$widget.$refs["wDyform"];
    ...
    success: function (result) {
      ...
      if (!isOpinionRuleCheckDto.success) {
        ...
        if (isOpinionRuleCheckDto.alertAutoClose) {
          options = {
            title: isOpinionRuleCheckDto.message, // 消息内容，不能为空
            msgIcon: "error", // 消息类型:success、info、warning、error、alert
            timer: 3000,
          };
          _self.showToast(options);
        } else {
          uni.showModal({
            content: isOpinionRuleCheckDto.message,
            showCancel: false,
            confirmText: _self.$widget.$t("global.confirm", "确认"),
...
WorkView.prototype.reload = function () {
  ...
  if (!isEmpty(trim(taskInstUuid))) {
    ...
    var refreshUrl = pageUrl + "?aclRole=DRAFT&flowInstUuid=" + flowInstUuid + "&reload=1";
    // 关闭当前页面，跳转到应用内的某个页面。
    uni.redirectTo({
...
WorkView.prototype._saveUserSubmit = function (result) {
  ...
  if (isEmpty(submitTaskInstUuid) && isEmpty(submitTaskOperationUuid)) {
    ...
    if (nextTasks.length > 0) {
      each(nextTasks, function (tItem, tIndex) {
        var taskTodoUsers = data.taskTodoUsers[tItem.uuid];
        var msgToUser = _self.getMsgTips(
          taskTodoUsers,
          _self.getMsgI18ns("WorkflowWork.message.submittedTo", "已提交至"),
          tItem
        );
        // 自动提交
        _self.emitActionSuccess({
...
WorkView.prototype.onRollbackSuccess = function (result) {
  ...
  if (nextTasks.length > 0) {
    ...
    msgToUser = _self.getMsgTips(
      taskTodoUsers,
      _self.getMsgI18ns("WorkflowWork.message.rollbackTo", "已退回至"),
      nextTasks[0]
    );
  }
...
WorkView.prototype.onDirectRollbackSuccess = function (result) {
  ...
  // 提示
  let message = _self.getMsgI18ns("WorkflowWork.message.directRollbackSuccess", "直接退回成功") + "！" + msgToUser;
  // 触发操作成功事件
  _self.emitActionSuccess({
...
WorkView.prototype.onCancelSuccess = function () {
  ...
  _self.emitActionSuccess({
    action: "cancel",
    message: _self.getMsgI18ns("WorkflowWork.message.cancelSuccess", "撤回成功！"),
...
WorkView.prototype.onTransferSuccess = function (result) {
  ...
  _self.allowUnloadWorkData = true;
  var msgToUser =
    _self.getMsgI18ns("WorkflowWork.message.transferredTo", "已转办至") + _self.workFlow.transferUsers.join("，");
...
WorkView.prototype.onCounterSignSuccess = function (result) {
  ...
  _self.allowUnloadWorkData = true;
  var workData = _self.workFlow.getWorkData();
  let action = workData.action || _self.$widget.$t("WorkflowWork.modal.signUser", "会签");
  let msgToUser =
    _self.$widget.$t("WorkflowWork.message.alreadyTo", { action }, "已" + action + "至") +
    _self.workFlow.signUsers.join("，") +
    "！";

  // 提示
  let message = action + _self.$widget.$t("WorkflowWork.success", "成功") + "！" + msgToUser;
  // 触发操作成功事件
  _self.emitActionSuccess({
    action: "counterSign",
    result,
...
WorkView.prototype.onAddSignSuccess = function (result) {
  ...
  _self.allowUnloadWorkData = true;
  let workData = _self.workFlow.getWorkData();
  let action = workData.action || _self.$widget.$t("WorkflowWork.modal.addSignUser", "加签");
  let msgToUser =
    _self.$widget.$t("WorkflowWork.message.alreadyTo", { action }, "已" + action + "至") +
    _self.workFlow.addSignUsers.join("，") +
    "！";
...
WorkView.prototype.onCopyToSuccess = function (result) {
  const _this = this;
  let msgToUser =
    this.getMsgI18ns("WorkflowWork.message.copiedTo", "已抄送至") + this.workFlow.copyToUsers.join("，") + "！";
  // 提示
...
WorkView.prototype.print = function () {
  ...
  var success = function (result) {
    ...
    if (templates.length === 0) {
      _self.showToast({ title: _self.getMsgI18ns("WorkflowWork.message.flowNotTemplate", "流程没有配置套打模板！") });
  ...
  var failure = function () {
    _self.showToast({
      title: _self.getMsgI18ns("WorkflowWork.message.getPrintTemplateError", "获取套打模板报错！"),
      msgIcon: "error",
    });
  };
...
WorkView.prototype.handOver = function () {
  ...
  WorkFlowErrorHandler.prototype.openOrgSelect({
    title: this.$widget.$t("WorkflowWork.selectSpecialUser", "选择特送的人员"),
    type: "all",
    multiple: true,
    selectTypes: "all",
    valueFormat: "justId",
    ...
    callback: function (values) {
      if (values && values.length > 0) {
        ...
        _self.showToast({ title: _self.$widget.$t("WorkflowWork.selectHandOver", "请选择移交人员！") });
        _self.handOver();
      }
    },
  });
...
WorkView.prototype.doRemove = function (removeMethod, uuids) {
  ...
  uni.showModal({
    title: this.getMsgI18ns("WorkflowWork.message.confirmDelFlow", "确认删除流程？"),
    confirmText: this.getMsgI18ns("global.confirm", "确认"),
    cancelText: this.getMsgI18ns("global.cancel", "取消"),
    success: function (res) {
      if (res.confirm) {
...
WorkView.prototype.getOrgVersionIds = function () {
  ...
  _this.getOrgVersionIdsPromise = _this.$widget.$axios
    .get(
      `/api/workflow/work/getOrgVersionIds?flowInstUuid=${workData.flowInstUuid}&flowDefUuid=${workData.flowDefUuid}`
    )
    .then(({ data: result }) => {
      return result.data;
    });
...
WorkView.prototype.listGroupChatProvider = function () {
  ...
  _this.listGroupChatProviderPromise = _this.$widget.$axios
    .get("/api/workflow/work/listGroupChatProvider")
    .then(({ data: result }) => {
      if (result.data) {
        return result.data
...
WorkView.prototype._loadDingtalkCorpId = function () {
  return this.$widget.$axios
...
WorkView.prototype.startGroupChat = function () {
  ...
  let workData = _this.getWorkData();
  Promise.all([_this.getOrgVersionIds(), _this.listGroupChatProvider()]).then((results) => {
    let orgVersionIds = results[0];
    let providerOptions = results[1];
    let orgVersionId = null;
    if (orgVersionIds && orgVersionIds.length > 0) {
      orgVersionId = orgVersionIds[0];
    }
    ...
    let groupChat = {
      ...
      groupName: $widget.$t("WorkflowWork.groupChat", "群聊") + "：" + workData.title,
      memberIds: $widget._$USER.userId,
      ownerId: $widget._$USER.userId,
      provider: "feishu",
      corpId: "",
    };
    let dingtalkItem = providerOptions.find((item) => item.value == "dingtalk");
    if (dingtalkItem) {
      _this._loadDingtalkCorpId().then((corpId) => (groupChat.corpId = corpId));
    }

    $widget.startGroupChat({
      groupChat,
      orgVersionId,
      orgVersionIds,
      ...
      rules: {
        groupName: {
          rules: [{ required: true, errorMessage: $widget.$t("WorkflowWork.groupNameRequired", "请输入群名称！") }],
        },
        memberIds: {
          rules: [
            { required: true, errorMessage: $widget.$t("WorkflowWork.groupMemberRequired", "请选择群成员！") },
            {
              validateFunction: function (rule, value, data, callback) {
                if (value.split(";").length < 2) {
                  callback($widget.$t("WorkflowWork.groupMemberCountRequired", "至少选择两个成员"));
                }
                return true;
              },
            },
            {
              validateFunction: function (rule, value, data, callback) {
                if (!value.split(";").includes(_this.$widget._$USER.userId)) {
                  callback($widget.$t("WorkflowWork.groupMemberIncludeSelf", "成员必须包含自己"));
                }
                return true;
              },
            },
        ...
        provider: {
          rules: [{ required: true, errorMessage: $widget.$t("WorkflowWork.groupTypeRequired", "请选择群类型！") }],
        },
      },
      onOk(groupChat) {
        _this.showLoading($widget.$t("WorkflowWork.startGroupChat", "发起群聊") + "...");
        return $widget.$axios
          .post("/api/workflow/work/startGroupChat", groupChat)
          .then(({ data: result }) => {
            _this.hideLoading();
            if (result.code === 0) {
              _this.showToast({ title: $widget.$t("WorkflowWork.startGroupChatSuccess", "发起群聊成功") });
              return Promise.resolve(result.data);
            } else {
              _this.showToast({
                title: result.msg || $widget.$t("WorkflowWork.startGroupChatFailed", "发起群聊失败"),
              });
          ...
          .catch(({ response }) => {
            _this.hideLoading();
            _this.showToast({ title: (response && response.data && response.data.msg) || "服务异常！" });
          });
      },
    });
  });
...
WorkView.prototype.isAllowViewProcess = function () {
  ...
  // 送审批的流程实例uuid
  let approveFlowInstUuid = _self.getQueryString("approveFlowInstUuid");
...
WorkView.prototype.addSystemPrefix = function (url) {
  const _this = this;
  if (_this.$widget._$SYSTEM_ID && url && url.indexOf("system_id=prod_") == -1) {
    url += `&system_id=${_this.$widget._$SYSTEM_ID}`;
  }
  return url;
};
// 加载遮罩
WorkView.prototype.showLoading = function (opt) {
  const _self = this;
  uni.showLoading({
    title: getMsgI18ns(opt.tip || opt, this),
  });
};
// 隐藏遮罩
...
WorkView.prototype.showToast = function (opt) {
  uni.showToast({
    title: opt.title || "",
    duration: opt.timer,
    icon: opt.msgIcon || opt.icon || "success",
  });
