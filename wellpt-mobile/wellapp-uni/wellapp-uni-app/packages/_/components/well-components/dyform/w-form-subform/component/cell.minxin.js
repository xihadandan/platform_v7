...
import * as formulaApi from "../../w-dyform/formula-api.js";
import { template as stringTemplate, isEmpty, debounce } from "lodash";
export default {
  inject: ["widgetSubformContext", "dyform", "widgetDyformContext"],
  props: {
    widget: Object,
          ...
          events.forEach((e) => {
            ...
            if (e.condition && e.condition.enable && e.condition.conditions.length > 0) {
              ...
              for (let c = 0, clen = conditions.length; c < clen; c++) {
                ...
                let isTrue = utils.expressionCompare(metaData, code, operator, value);
                if (match == "any") {
                  // 满足任一条件就执行
                  if (isTrue) {
                    executeEvent = true;
    ...
    getFormulaCalculateValue() {
      ...
      if (this.option.formula != undefined) {
        try {
          ...
          v = compiler({
            __SUBFORM_DATA__: { ...this.form.formData },
            ...this.dyform.formData,
            ...this.formulaApiData,
          });
