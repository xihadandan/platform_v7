...
export default {
  ...
  data() {
    if (!this.widget.configuration.hasOwnProperty("uniConfiguration")) {
      this.$set(this.widget.configuration, "uniConfiguration", { inputBorder: false, width: 50 });
    }
    let configuration = this.widget.configuration;
    let width =
      configuration.uniConfiguration && configuration.uniConfiguration.width !== undefined
        ? configuration.uniConfiguration.width
        : 50;
    return {
      configuration,
      numberRange: {
  ...
  methods: {
        ...
        this.customRules.splice(1, 0, {
          ...
          validateFunction: function (rule, value, data, callback) {
            ...
            for (var key in numberRule.rule) {
              if (
                _self.formData[_self.widget.configuration.code] !== undefined &&
                _self.formData[_self.widget.configuration.code] !== null
              ) {
    ...
    setValue(val) {
      ...
      if (this.formData[this.widget.configuration.code] !== value) {
        this.formData[this.widget.configuration.code] = value;
      }
    ...
    getNumberRule: function (dbDataType) {
      ...
      if (dbDataType == "12") {
        ...
        msg = {
          ...
          maxlength: this.$t(
            "WidgetFormInputNumber.messages.doublePrecisionFloatingPointNumberMaxDecimal",
            "双精度浮点数,不得超过18个字符"
          ),
        };
