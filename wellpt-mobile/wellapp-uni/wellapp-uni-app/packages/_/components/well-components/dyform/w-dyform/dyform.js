...
export function createDyform(formUuid, namespace) {
  return {
    ...
    validate(callback) {
      if (this.$form) {
        this.$form.validate((messages, res) => {
          console.error("主表校验消息", messages);
          //messages为null时,校验成功
          let valid = !messages;
    ...
    showValidationErrors(valid, result) {
      if (!valid) {
        const msg = [];
        for (let r = 0; r < result.length; r++) {
          let key = result[r].key;
          let $field = this.$fieldset[key];
          if (key.startsWith("__REF_FORM_DATA__.")) {
            let parts = key.split(".");
            $field = this.refDyform[parts[1]].$fieldset[parts[2]];
          }
          // 判断是否在从表内
          if ($field == undefined) {
            //表单组件生成的真实参数转换成[fromUuid,rowId,code]
            //代理表单组件生成的真实参数转换成[fromUuid,code]
            let code = this.name2arr(key);
            let subformUuid = code[0];
            let fieldCode = code[2];
            if (!fieldCode) {
              fieldCode = code[1];
            }
            let rows = this.subform[subformUuid];
            if (rows.length > 0) {
              let subWidgetId = this.$subform[rows[0].formId].widget.id;
              $field = rows[0].$fieldset[fieldCode];
              $field.subtitle = rows[0].title;
              $field.subWidgetId = subWidgetId; // 从表组件id
            }
          }
          let _msg = [];
          if ($field) {
            _msg.push(result[r].errorMessage);
          }
          if (_msg.length) {
            let hasIndex = msg.findIndex((item) => item.widgetId == $field.widget.id);
            if (hasIndex == -1) {
              msg.push({
                id: $field.subWidgetId ? $field.subWidgetId : $field.widget.id, // 定位时,从表字段定位到从表,主表定位到字段本身
                widgetId: $field.widget.id,
                name:
                  ($field.subtitle ? "[" + $field.subtitle + "]" : "") +
                  $field.$t($field.widget.id, $field.widget.configuration.name),
                code: key,
                msg: _msg.join(";"),
              });
            }
        ...
        uni.$emit("showValidationErrors" + this.formUuid, msg);
        console.log("表单验证失败!");
        console.log(msg);
      } else {
        this.hideValidationErrors();
    ...
    hideValidationErrors() {
      uni.$emit("hideValidationErrors" + this.formUuid);
    },
