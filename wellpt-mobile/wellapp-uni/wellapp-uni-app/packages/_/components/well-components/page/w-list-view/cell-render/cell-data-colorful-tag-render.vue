<template>
  <view v-if="tags != undefined">
    <template v-for="(tag, i) in tags">
      <uni-w-tag :text="tag.text" :style="tagType ? getTagStyle(tag.color) : ''" :key="'tag_' + rowIndex + '_' + i">
      </uni-w-tag>
    </template>
  </view>
</template>
    ...
    getTagStyle(color) {
      if (color) {
        ...
        if (this.tagType == "border") {
          style["--w-tag-text-color"] = color;
          style["--w-tag-background"] = light;
          style["--w-tag-border-color"] = color;
        } else if (this.tagType == "bg-color") {
          style["--w-tag-text-color"] = `#FFFFFF`;
          style["--w-tag-background"] = color;
        } else {
          style["--w-tag-text-color"] = color;
          style["--w-tag-background"] = light;
        }
