<template>
  <view class="w-data-management-viewer" :class="customClassCom" v-if="isEmptyUniJsModule">
    <template v-if="widget.configuration && widget.configuration.WidgetTable">
      <w-table
        ref="table"
        :widget="widget.configuration.WidgetTable"
        :parent="widget"
        :widgetsOfParent="widgetsOfParent"
        @itemClick="tableViewItemClick"
        @buttonClick="tableViewButtonClick"
      >
      </w-table>
    </template>
    <view v-else class="w-data-viewer" v-for="item in items" :key="item.id">
      <w-list-view
        ref="listView"
...
export default {
  ...
  mounted() {
    const _self = this;
    if (_self.widget.configuration && _self.widget.configuration.WidgetTable) {
      this.$developJsInstance["BasicWidgetDyformDevelopment"] = appContext.jsInstance(
        "BasicWidgetDyformDevelopment",
        this
      );
    }
  },
  methods: {
    tableViewItemClick: function (args) {
      let options = args[0];
      options.$evtWidget = options.ui;
      options.ui = this;
      appContext.dispatchEvent(options);
    },
