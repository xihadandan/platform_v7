<template>
  <view class="uni-cursor-point">
    <view v-if="popMenu && content.length > 0" class="uni-fab" :style="uniFabStyle">
      <view
        :class="{
...
export default {
  ...
  data() {
    return {
      ...
      styles: {
        ...
        backgroundColor: "#fff",
        buttonColor: "var(--w-primary-color)",
        iconColor: "#fff",
        icon: "plusempty",
      },
      isChange: false,
  ...
  computed: {
    ...
    flexDirectionStart() {
      ...
      change = this.getPosition(1, "vertical", "top");
      return change;
    },
    flexDirectionEnd() {
      let change = this.isChange;
      change = this.getPosition(1, "vertical", "bottom");
      return change;
    },
    horizontalLeft() {
      let change = this.isChange;
      change = this.getPosition(2, "horizontal", "left");
      return change;
    },
    horizontalRight() {
      let change = this.isChange;
      change = this.getPosition(2, "horizontal", "right");
      return change;
    },
    uniFabStyle() {
      let style = {};
      ...
      if (this.horizontalRight) {
        style.right = "calc(10px - " + this.contentWidthMin + ")";
      } else if (this.flexDirectionEnd) {
        style.bottom = "calc(10px - " + this.contentWidthMin + ")";
      }
      return style;
    },
    // 计算 nvue bottom
