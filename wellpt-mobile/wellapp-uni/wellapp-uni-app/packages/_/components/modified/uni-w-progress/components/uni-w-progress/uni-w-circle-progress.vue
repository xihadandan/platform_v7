...
export default {
  ...
  data() {
    ...
    return {
      elBgId: "uCircleProgressBgId_" + id,
      elId: "uCircleProgressElId_" + id,
      // #ifndef MP-WEIXIN
      // elBgId: "uCircleProgressBgId_" + this.$u.guid(), // 非微信端的时候，需用动态的id，否则一个页面多个圆形进度条组件数据会混乱
      // elId: "uCircleProgressElId_" + this.$u.guid(),
      // #endif
      widthPx: this.width, //uni.upx2px(this.width), // 转成px后的整个组件的背景宽度
      borderWidthPx: this.borderWidth, //uni.upx2px(this.borderWidth), // 转成px后的圆环的宽度
  ...
  computed: {
    ...
    // 缺口总长
    gapRadian() {
      return (2 * Math.PI * this.gapDegree) / 360;
    },
    startAngle() {
      let angle = Math.PI / 2; // canvas画圆的起始角度，默认为3点钟方向，定位到12点钟方向
      if (this.gapRadian) {
        if (this.gapPosition == "top") {
          angle = -Math.PI / 2;
        } else if (this.gapPosition == "left") {
          angle = Math.PI;
        } else if (this.gapPosition == "right") {
          angle = 0;
        }
      }
      this.endAngle = angle + Math.PI * 2 - this.gapRadian / 2;
      return angle + this.gapRadian / 2;
    },
  },
  mounted() {
  ...
  methods: {
    drawProgressBg() {
      ...
      ctx.arc(radius, radius, radius - this.borderWidthPx, this.startAngle, this.endAngle, false);
      ctx.stroke(); // 对路径进行描绘
      ctx.draw();
    },
    drawCircleByProgress(progress) {
      ...
      if (progress) {
        ...
        let endAngle = ((2 * Math.PI - this.gapRadian) / 100) * progress + this.startAngle;
        ctx.beginPath();
        // 半径为整个canvas宽度的一半
        let radius = this.widthPx / 2;
