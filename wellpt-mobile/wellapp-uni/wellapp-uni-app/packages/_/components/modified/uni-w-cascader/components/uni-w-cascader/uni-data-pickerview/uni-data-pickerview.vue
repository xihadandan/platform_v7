<template>
  <view class="uni-w-cascader-pickerview">
    ...
    <view class="tab-c">
      <scroll-view class="list" :scroll-y="true">
        ...
        >
          <text class="item-text" :class="{ 'item-text-overflow': ellipsis }">{{ item[map.text] }}</text>
          <view
            class="check"
            v-if="selected.length > selectedIndex && item[map.value] == selected[selectedIndex].value"
          ></view>
          <view class="arrow" v-if="!item.isleaf || (item.data && item.data.isleaf)">
...
export default {
  ...
  methods: {
    ...
    onSelectedChange(node, isleaf) {
      ...
      if (isleaf) {
        this._dispatchEvent();
        this.selectIsParent = false;
      } else if (this.changeOnSelect) {
        this._dispatchEvent(this.changeOnSelect);
        this.selectIsParent = true;
      }
    ...
    _dispatchEvent(isParent) {
      this.$emit("change", this.selected.slice(0), isParent);
    },
  },
};
</script>
