<template>
  <view class="uni-w-subsection" :style="[subsectionStyle]" :class="[`uni-w-subsection--${mode}`]">
    <view
      class="uni-w-subsection-item"
      :style="[itemStyle(index)]"
      @tap="click(index)"
      :class="[noBorderRight(index), 'uni-w-subsection-item-' + index]"
      v-for="(item, index) in listInfo"
      :key="index"
    >
      <!-- 插槽样式需得加class="top"-->
      <slot name="prefix" :item="item"></slot>
      <view :style="[textStyle(index)]" class="uni-w-subsection-item-text w-ellipsis-1">
        {{ item[name] || item["name"] }}
      </view>
      <slot name="suffix" :item="item"></slot>
    </view>
    <view class="uni-w-subsection-item-bg" :style="[itemBarStyle]"></view>
  </view>
</template>
 ...
 * @property {String Number} height 组件高度，单位px（默认70）
 ...
 * @example <uni-w-subsection active-color="#ff9900"></uni-w-subsection>
 */
export default {
  name: "uni-w-subsection",
  props: {
    // tab的数据
    list: {
      type: Array,
    ...
    bgColor: {
      type: String,
      default: "var(--w-bg-color-mobile-bg)",
    },
    // mode = button时，滑块背景颜色
    buttonColor: {
      type: String,
  ...
  watch: {
    ...
    list(n, o) {
      ...
      // 用$nextTick等待视图更新完毕后再计算subsection的局部信息，否则可能因为subsection还没生成就获取，就会有问题
      this.$nextTick(() => {
        this.init();
      });
    },
  ...
  computed: {
    ...
    noBorderRight() {
      return (index) => {
        ...
        let classs = "";
        // 不显示右边的边框
        if (index < this.list.length - 1) classs += " uni-w-subsection-none-border-right";
        // 显示整个组件的左右边圆角
        if (index == 0) classs += " uni-w-subsection-item-first";
        if (index == this.list.length - 1) classs += " uni-w-subsection-item-last";
        return classs;
      };
    },
    // 文字的样式
    ...
    subsectionStyle() {
      ...
      style = Object.assign(style, this.subsectionCustomStyle);
      return style;
    },
    // 滑块的样式
    itemBarStyle() {
      ...
      if (this.mode == "button") {
        ...
        style.borderRadius = `${this.borderRadius}px`;
        style.padding = `${this.buttonPadding}px`;
        style.height = this.height - this.buttonPadding * 2 + "px";
        style.zIndex = 0;
      }
  ...
  methods: {
    ...
    changeSectionStatus(nVal) {
      if (this.mode == "subsection") {
        ...
        if (nVal == this.list.length - 1) {
          this.itemBgStyle.borderRadius = `0 ${this.borderRadius}px ${this.borderRadius}px 0`;
        }
        if (nVal == 0) {
          this.itemBgStyle.borderRadius = `${this.borderRadius}px 0 0 ${this.borderRadius}px`;
        }
    ...
    getTabsInfo() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        view.select(".uni-w-subsection-item-" + i).boundingClientRect();
      }
      view.exec((res) => {
        if (!res.length) {
          setTimeout(() => {
        ...
        res.map((val, index) => {
          ...
          this.listInfo[index].left = val.left;
        });
        // 初始化滑块的宽度
        if (this.mode == "subsection") {
...
.uni-w-subsection-item-bg {
  background-color: var(--w-primary-color);
  position: absolute;
  z-index: -1;
}

.uni-w-subsection-none-border-right {
  border-right: none !important;
}

.uni-w-subsection-item-first {
  border-top-left-radius: 4px;
  border-bottom-left-radius: 4px;
}

.uni-w-subsection-item-last {
  border-top-right-radius: 4px;
  border-bottom-right-radius: 4px;
}

.uni-w-subsection-item-text {
