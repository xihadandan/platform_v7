<template>
  <uni-w-empty v-if="unauthenticateText" type="jurisdiction" :text="unauthenticateText">
    <uni-w-button type="primary" text="返回" @click="goBack"></uni-w-button>
  </uni-w-empty>
  <w-work-view
    ref="workViewRef"
    :style="theme"
...
<script>
import { workFlowUtils, storage } from "wellapp-uni-framework";
...
export default {
  ...
  onLoad: function (options) {
    ...
    // #ifdef H5
    if (window.location && decodeURIComponent(window.location.href).includes("source=")) {
      uni.setStorageSync("submitSuccessRedirect", "/");
    }
    // #endif

    this.unauthenticateText = "";
    uni.showLoading({ title: this.$t("WorkflowWork.loading", "加载中") });
    this.options = options;
    if (!this.options.system_id) {
      this.options.system_id = this._$SYSTEM_ID;
    }
    ...
    if (toReload) {
      ...
      if (["TODO", "DRAFT"].indexOf(options.aclRole) == -1) {
        // 清除签署意见缓存
        uni.removeStorageSync("workflow_opinion_data");
      }
  ...
  computed: {
    ...
    // 连续签批状态
    isContinuousMode() {
      // _requestCode会缓存待办表单的请求数据
      if (!this.workViewOptions._requestCode) {
        return false;
      } else if (this.workViewOptions._requestCode) {
        let requestData = uni.getStorageSync(`cwvDataStoreParams_${this.options._requestCode}`);
        if (!requestData) {
          return false;
        }
      }
      return this.workViewOptions.continuousMode == "1";
    },
  },
  methods: {
    $t() {
    ...
    redirectToLogin() {
      let loginSuccessRedirect = INDEX_PAGE_PATH || "/pages/welldo/index";
      ...
      if (loginSuccessRedirect && loginSuccessRedirect.includes("source")) {
        ...
        this.setPageParameter("login_provider", source + "_sso");
      }
      uni.setStorageSync("loginSuccessRedirect", loginSuccessRedirect);
      uni.reLaunch({
        url: (LOGIN_PAGE_PATH || "/packages/_/pages/login/login") + `?${source}_sso=true`,
      });
    },
    workView(options) {
      if (options.isReLoad) {
    ...
    updateRouteParam(url, options) {
      if (this._$SYSTEM_ID && url && url.indexOf("system_id=prod_") == -1) {
        url += `&system_id=${this._$SYSTEM_ID}`;
      }
      uni.redirectTo({
        url: url,
      });
        ...
        _self.refreshParent({
          ...
          msgCallback: () => {
            if (_self.isContinuousMode) {
              ...
              // 提交成功后跳转页面
              let submitSuccessRedirect = uni.getStorageSync("submitSuccessRedirect");
              if (submitSuccessRedirect) {
                uni.removeStorageSync("submitSuccessRedirect");
                setTimeout(() => {
                  uni.redirectTo({
                    url: submitSuccessRedirect,
                  });
                }, 1000);
              } else {
                // 关闭当前窗口
