const os = require("os");
const generateMpWorkViewDevelopmentCode = function (source) {
  ...
  for (let index = 0; index < sources.length; index++) {
    ...
    if (lineCode != null && lineCode.indexOf(" #ifdef MP") != -1) {
      appendWorkViewDevelopmentCode(codes);
    }
  }
  return codes.join(splitter);
};

const appendWorkViewDevelopmentCode = function (codes) {
  ...
  for (let index = 0; index < developmentComponents.length; index++) {
    ...
    let componentCode = `<${component.name} v-if="workViewUniJsModule == '${component.name}' ||
      workViewUniJsModule == '${component.identifier}'"
      :workViewOptions="workViewOptions">
      </${component.name}>`;
...
module.exports = function (source) {
  ...
  let code = generateMpWorkViewDevelopmentCode(source);
  return code;
};
