...
public class TempObjectCache {
    ...
    private ObjectMap getReportMap(HttpServletRequest req) {
        ...
        if (StringUtils.isNotBlank(referer)) {
            MultiValueMap<String, String> params = UriComponentsBuilder.fromUri(URI.create(referer)).build().getQueryParams();
            if (params.containsKey("_loginid")) {//前端web服务的用户登录id
                sessionId = params.getFirst("_loginid");
            }
        } else {
            //从cookie获取
            Cookie[] cookies = req.getCookies();
            for (Cookie ck : cookies) {
                if ("_loginid".equalsIgnoreCase(ck.getName())) {
