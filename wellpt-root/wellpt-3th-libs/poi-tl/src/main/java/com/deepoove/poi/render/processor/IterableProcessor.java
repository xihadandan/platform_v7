...
public class IterableProcessor extends AbstractIterableProcessor {
    ...
    protected void handleIterable(IterableTemplate iterableTemplate, BodyContainer bodyContainer, Iterable<?> compute) {
        ...
        EnvIterator.foreach(compute.iterator(), model -> {
            Object rootModel = this.template.getConfig().getRootModel();

            if (model instanceof EnvModel) {
                if (rootModel instanceof Map && rootModel != model.getRoot()) {
                    Map rootModelMap = (Map) rootModel;
                    rootModelMap.put("$item", model.getRoot());
                    Map<String, Object> env = model.getEnv();
                    if (env != null && env.containsKey("_index")) {
                        rootModelMap.put("$itemIndex", (Integer) env.get("_index") + 1);
                    }
                    model.setRoot(rootModel);
                    next(iterableTemplate, bodyContainer, context, model);
                } else {
                    next(iterableTemplate, bodyContainer, context, model);
                }
            } else {
