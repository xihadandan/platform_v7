...
public class HtmlPreviewServletAction extends RenderPageServletAction {
	...
	public void execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(method!=null){
			...
			if(htmlReport==null){
				...
				if(isbuildLazyData(req)){
					//报表不初始化数据时，报表内容直接返回空字符串 2021-06-18
					context.put("content", "");
				}else{
	 ...
	 *
	 * @param req 
	 *@return Boolean true为不初化数据
	 **/
	private Boolean isbuildLazyData(HttpServletRequest req){
		String lazyData=req.getParameter("lazyData");
		final String noInitDataFlag = "1";
		if(StringUtils.isBlank(lazyData)){
			return Boolean.FALSE;
		}else if(noInitDataFlag.equals(lazyData)){
			return Boolean.TRUE;
		}
		return Boolean.FALSE;
	}
