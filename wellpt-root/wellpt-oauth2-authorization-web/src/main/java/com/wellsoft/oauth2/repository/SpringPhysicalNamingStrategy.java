...
public class SpringPhysicalNamingStrategy implements PhysicalNamingStrategy {
    ...
    private Identifier apply(Identifier name, JdbcEnvironment jdbcEnvironment) {
        if (name == null) {
            ...
            for (int i = 1; i < builder.length() - 1; ++i) {
                if (this.isUnderscoreRequired(builder.charAt(i - 1), builder.charAt(i),
                        builder.charAt(i + 1))) {
                    builder.insert(i++, '_');
                }
            }
    ...
    private boolean isUnderscoreRequired(char before, char current, char after) {
        ...
        return Character.isLowerCase(before) && Character.isUpperCase(
                current) && Character.isLowerCase(after);
    }
}