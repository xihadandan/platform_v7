...
public class IdUtils {
    ...
    private IdUtils() {
        ...
        String cluster = pc.get("app.cluster.id",
                String.valueOf(Long.parseLong(getHostIpTrim()) % 8)); // 集群ID
        String node = pc.get("app.node.id",
                String.valueOf(Long.parseLong(getCurrentPid()) % 256)); // 节点ID
        long clusterId = 0;
        long nodeId = 0;
        try {
