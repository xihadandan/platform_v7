...
public class ResponseUtils {
    ...
    public static void renderText(final HttpServletResponse response, final String text,
                                  final String... headers) {
        render(HeadUtils.TEXT_TYPE, text, response, headers);
    }
    ...
    public static void renderHtml(final HttpServletResponse response, final String html,
                                  final String... headers) {
        render(HeadUtils.HTML_TYPE, html, response, headers);
    }
    ...
    public static void renderXml(final HttpServletResponse response, final String xml,
                                 final String... headers) {
        render(HeadUtils.XML_TYPE, xml, response, headers);
    }
    ...
    public static void renderJson(final HttpServletResponse response, final String jsonString,
                                  final String... headers) {
        render(HeadUtils.JSON_TYPE, jsonString, response, headers);
    }
    ...
    public static void renderJson(HttpServletResponse response, final Object data,
                                  final String... headers) {
        response = initResponseHeader(HeadUtils.JSON_TYPE, response, headers);
        try {
    ...
    public static void renderJsonp(final HttpServletResponse response, final String callbackName,
        ...
        String result = new StringBuilder().append(callbackName).append("(").append(
                jsonString).append(");").toString();

        // 渲染Content-Type为javascript的返回内容,输出结果为javascript语句,
        // 如callback197("{html:'Hello World!!!'}");
        render(HeadUtils.JS_TYPE, result, response, headers);
    }
    ...
    private static HttpServletResponse initResponseHeader(final String contentType,
        ...
        if (noCache) {
            HeadUtils.setNoCacheHeader(response);
        }
