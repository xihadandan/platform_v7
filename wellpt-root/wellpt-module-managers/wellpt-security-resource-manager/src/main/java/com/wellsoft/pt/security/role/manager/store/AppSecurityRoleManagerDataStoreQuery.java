package com.wellsoft.pt.security.role.manager.store;
 ...
 */
@Component
public class AppSecurityRoleManagerDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public CriteriaMetadata initCriteriaMetadata(QueryContext queryContext) {
        ...
        criteriaMetadata.add("appId", "t1.app_id", "应用ID", String.class);
    ...
    public List<QueryItem> query(QueryContext queryContext) {
        List<QueryItem> queryItems = queryContext.getNativeDao().namedQuery("appSecurityRoleManagerQuery",
    ...
    public long count(QueryContext queryContext) {
        ...
        return total != -1 ? total : queryContext.getNativeDao().countByNamedQuery(
                "appSecurityRoleManagerQuery", getQueryParams(queryContext));
    }
    ...
    private Map<String, Object> getQueryParams(QueryContext queryContext) {
        ...
        queryParams.put("appId", queryContext.getQueryParams().get("appId"));
        queryParams.put("system", RequestSystemContextPathResolver.system());
        queryParams.put("tenant", userDetails.getTenantId());
        String currentUserUnitId = userDetails.getSystemUnitId();
        if (StringUtils.isBlank((String) queryParams.get("appId")) && !MultiOrgSystemUnit.PT_ID.equalsIgnoreCase(currentUserUnitId)) {
            queryParams.put("systemUnitIds", Lists.<String>
                    newArrayList(currentUserUnitId));
        }
        return queryParams;
    }
