package com.wellsoft.pt.security.privilege.manager.store;
 ...
 */
@Component
public class AppSecurityPrivilegeManagerDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public List<QueryItem> query(QueryContext queryContext) {
        List<QueryItem> queryItems = queryContext.getNativeDao().namedQuery(
                "appSecurityPrivilegeManagerQuery",
    ...
    public long count(QueryContext queryContext) {
        ...
        return total != -1 ? total : queryContext.getNativeDao().countByNamedQuery(
                "appSecurityPrivilegeManagerQuery", getQueryParams(queryContext));
    }
