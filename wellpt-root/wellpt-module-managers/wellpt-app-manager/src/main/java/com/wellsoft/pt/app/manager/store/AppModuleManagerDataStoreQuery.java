 ...
 */
@Component
public class AppModuleManagerDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public CriteriaMetadata initCriteriaMetadata(QueryContext queryContext) {
        ...
        criteriaMetadata.add("title", "t1.title", "标题", String.class);
        criteriaMetadata.add("jsModule", "t1.js_module", "加载自定义JavaScript模块，以模块化的形式开发JavaScript，多个以逗号隔开", String.class);
        criteriaMetadata.add("enabled", "t1.enabled", "是否启用", Boolean.class);
        criteriaMetadata.add("remark", "t1.remark", "备注", String.class);
        criteriaMetadata.add("parentUuid", "t1.parent_uuid", "上级模块UUID", String.class);
        ...
        criteriaMetadata.add("appProductUuid", "t2.app_product_uuid", "归属产品UUID", String.class);
        criteriaMetadata.add("appSystemUuid", "t2.app_system_uuid", "归属系统UUID", String.class);
    ...
    public List<QueryItem> query(QueryContext queryContext) {
        List<QueryItem> queryItems = queryContext.getNativeDao().namedQuery("appModuleManagerQuery",
                getQueryParams(queryContext), QueryItem.class, queryContext.getPagingInfo());
        // 上级集成信息UUID不存在的数据进行字段清空
        Map<String, QueryItem> piMap = Maps.newHashMap();
        for (QueryItem queryItem : queryItems) {
            piMap.put(queryItem.getString("appPiUuid"), queryItem);
        }
        for (QueryItem queryItem : queryItems) {
            if (!piMap.containsKey(queryItem.getString("parentAppPiUuid"))) {
                queryItem.put("parentAppPiUuid", null);
            }
        }
        return queryItems;
    }
    ...
    public long count(QueryContext queryContext) {
        return queryContext.getNativeDao().countByNamedQuery("appModuleManagerQuery", getQueryParams(queryContext));
    }
