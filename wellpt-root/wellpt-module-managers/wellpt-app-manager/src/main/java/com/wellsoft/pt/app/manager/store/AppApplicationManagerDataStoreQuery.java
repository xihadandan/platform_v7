 ...
 */
@Component
public class AppApplicationManagerDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public CriteriaMetadata initCriteriaMetadata(QueryContext queryContext) {
        ...
        criteriaMetadata.add("configurable", "t1.configurable", "标识该应用对于普通用户是否可配置", Boolean.class);
        criteriaMetadata.add("type", "t1.type", "应用类别 ", Integer.class);
        criteriaMetadata.add("correlativeFunction", "t1.correlative_function", "关联的功能ID", String.class);
    ...
    public List<QueryItem> query(QueryContext queryContext) {
        List<QueryItem> queryItems = queryContext.getNativeDao().namedQuery("appApplicationManagerQuery",
    ...
    public long count(QueryContext queryContext) {
        return queryContext.getNativeDao()
                .countByNamedQuery("appApplicationManagerQuery", getQueryParams(queryContext));
    }
