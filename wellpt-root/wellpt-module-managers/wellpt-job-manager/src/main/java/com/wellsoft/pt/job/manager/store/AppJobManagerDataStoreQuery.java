 ...
 */
@Component
public class AppJobManagerDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public CriteriaMetadata initCriteriaMetadata(QueryContext queryContext) {
        ...
        criteriaMetadata.add("jobClassName", "t1.job_class_name", "任务调度类", String.class);
        ...
        criteriaMetadata.add("nestFireTime", "next_fire_time", "下次执行时间", String.class);
        criteriaMetadata.add("schedulerInstanceId", "scheduler_instance_id", "最近执行实例",
                String.class);
    ...
    public long count(QueryContext queryContext) {
        ...
        return total != -1 ? total : queryContext.getNativeDao().countByNamedQuery(
                "appModuleJobManagerQuery", getQueryParams(queryContext));
    }
