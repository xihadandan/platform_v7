 ...
 */
@Service
public class DyformDesignTemplateCleanupProvider extends AbstractDataCleanupDataCleanupProvider {
    @Override
    public String getType() {
        return "dyformDesignTemplate";
    }
    ...
    public ExpectCleanupResult cleanup(Params params) {
        ...
        return ExpectCleanupResult.total(this.nativeDao.batchExecute("delete from app_user_widget_definition p" +
                        " where widget_id='dyform_user_template'" + (clearTypes.contains("clearUserDesign") ? " and p.user_id is not null" : "")
                , null));
    }
