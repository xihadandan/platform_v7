...
public class CustomConfiguration extends org.hibernate.cfg.Configuration implements Serializable {
    ...
    private static final MetadataSourceType[] DEFAULT_ARTEFACT_PROCESSING_ORDER = new MetadataSourceType[]{
    ...
    protected void reset() {
        ...
        specjProprietarySyntaxEnabled = System.getProperty(
        ...
        this.dialect = ((SessionFactoryImpl) sessionFactory).getDialect();
    }
    ...
    private XmlDocument add(InputSource inputSource, Origin origin) {
        XmlDocument metadataXml = MappingReader.INSTANCE.readMappingDocument(entityResolver,
                inputSource, origin);
        add(metadataXml);
        return metadataXml;
    }
    ...
    public void add(XmlDocument metadataXml) {
        if (inSecondPass || !isOrmXml(metadataXml)) {
            ...
            List<String> classNames = jpaMetadataProvider.getXMLContext().addDocument(
                    metadataXml.getDocumentTree());
            for (String className : classNames) {
                try {
                    metadataSourceQueue.add(
                            reflectionManager.classForName(className, this.getClass()));
                } catch (ClassNotFoundException e) {
                    throw new AnnotationException(
                            "Unable to load class defined in XML: " + className, e);
                }
            }
    ...
    public CustomConfiguration addCacheableFileStrictly(File xmlFile) throws SerializationException,
        ...
        Document document = (Document) SerializationHelper.deserialize(
    ...
    public CustomConfiguration addURL(URL url) throws MappingException {
        ...
        try {
            ...
            throw new InvalidMappingException("Unable to open url stream [" + urlExternalForm + "]",
                    "URL",
                    urlExternalForm, e);
        }
        return this;
    ...
    private XmlDocument add(InputStream inputStream, final String type, final String name) {
        ...
        try {
            ...
            try {
                ...
                LOG.trace("Was unable to close input stream", ignore);
            }
        }
    }
     ...
     *                          processing the contained mapping document.
     */
    public CustomConfiguration addResource(String resourceName,
    ...
    public CustomConfiguration addResource(String resourceName) throws MappingException {
        ...
        if (resourceInputStream == null) {
            resourceInputStream = Environment.class.getClassLoader().getResourceAsStream(
                    resourceName);
        }
        if (resourceInputStream == null) {
            throw new MappingNotFoundException("resource", resourceName);
        ...
        try {
            try {
                ...
                throw new InvalidMappingException(
                        "Could not read mapping documents from jar: " + jar.getName(), "jar",
                        jar.getName(), ioe);
            }
    ...
    @SuppressWarnings({"unchecked"})
    public Iterator<IdentifierGenerator> iterateGenerators(
    ...
    public String[] generateDropSchemaScript(Dialect dialect) throws HibernateException {
        ...
        {
            ListIterator itr = auxiliaryDatabaseObjects.listIterator(
                    auxiliaryDatabaseObjects.size());
    ...
    public String[] generateSchemaCreationScript(Dialect dialect) throws HibernateException {
        ...
        while (iter.hasNext()) {
            ...
            if (table.isPhysicalTable()) {
                ...
                Iterator<String> comments = table.sqlCommentStrings(dialect, defaultCatalog,
                        defaultSchema);
        ...
        while (iter.hasNext()) {
            Table table = (Table) iter.next();
            if (table.isPhysicalTable() && dialect.hasAlterTable()) {
        ...
        while (iter.hasNext()) {
            String[] lines = ((PersistentIdentifierGenerator) iter.next()).sqlCreateStrings(
                    dialect);
            script.addAll(Arrays.asList(lines));
        }
        ...
        for (AuxiliaryDatabaseObject auxiliaryDatabaseObject : auxiliaryDatabaseObjects) {
            if (auxiliaryDatabaseObject.appliesToDialect(dialect)) {
                script.add(auxiliaryDatabaseObject.sqlCreateString(dialect, mapping, defaultCatalog,
                        defaultSchema));
            }
        }
    ...
    public String[] generateSchemaUpdateScript(Dialect dialect, DatabaseMetadata databaseMetadata)
        ...
        List<SchemaUpdateScript> scripts = generateSchemaUpdateScriptList(dialect,
                databaseMetadata);
     ...
     * @see org.hibernate.tool.hbm2ddl.SchemaUpdate
     */
    public List<SchemaUpdateScript> generateSchemaUpdateScriptList(Dialect dialect,
        ...
        Iterator iter = getTableMappings();
        while (iter.hasNext()) {
            AbstractTable table = (AbstractTable) iter.next();
            ...
            if (table.isPhysicalTable()) {
                ...
                if (tableInfo == null) {
                    scripts.add(new SchemaUpdateScript(
                            table.sqlCreateString(dialect, mapping, tableCatalog,
                                    tableSchema), false));

                    //执行表相关备注信息
                    Iterator<String> comments = table.sqlCommentStrings(dialect, mapping,
                            defaultCatalog,
                            defaultSchema);
                    while (comments.hasNext()) {
                    ...
                    Iterator<String> subiter = table.sqlAlterStrings(dialect, mapping, tableInfo,
                            tableCatalog,
                            tableSchema);
                    while (subiter.hasNext()) {
        ...
        while (iter.hasNext()) {
            ...
            if (table.isPhysicalTable()) {
                ...
                if (!constraintMethod.equals(UniqueConstraintSchemaUpdateStrategy.SKIP)) {
                    ...
                    while (uniqueIter.hasNext()) {
                        ...
                        if (tableInfo != null && StringHelper.isNotEmpty(uniqueKey.getName())) {
                            final IndexMetadata meta = tableInfo.getIndexMetadata(
                                    uniqueKey.getName());
                            if (meta != null) {
                                continue;
                            }
                ...
                while (subIter.hasNext()) {
                    ...
                    scripts.add(new SchemaUpdateScript(
                            index.sqlCreateString(dialect, mapping, tableCatalog,
                                    tableSchema), false));
                }
            }
        }
            ...
            if (table.isPhysicalTable()) {
                ...
                if (dialect.hasAlterTable()) {
                    ...
                    while (subIter.hasNext()) {
                        ForeignKey fk = (ForeignKey) subIter.next();
                        if (fk.isPhysicalConstraint()) {
                            boolean create = tableInfo == null || (tableInfo.getForeignKeyMetadata(
                                    fk) == null && (
                                    // Icky workaround for MySQL bug:
                                    !(dialect instanceof MySQLDialect) || tableInfo.getIndexMetadata(
                                            fk.getName()) == null));
                            if (create) {
                                scripts.add(new SchemaUpdateScript(
                                        fk.sqlCreateString(dialect, mapping, tableCatalog,
                                                tableSchema), false));
                            }
                        }
                    }
    ...
    private List<MetadataSourceType> determineMetadataSourcePrecedence() {
        if (metadataSourcePrecedence.isEmpty()
                ...
                getProperties().getProperty(ARTEFACT_PROCESSING_ORDER))) {
            metadataSourcePrecedence = parsePrecedence(
                    getProperties().getProperty(ARTEFACT_PROCESSING_ORDER));
        }
        if (metadataSourcePrecedence.isEmpty()) {
    ...
    private List<MetadataSourceType> parsePrecedence(String s) {
        ...
        while (precedences.hasMoreElements()) {
            tmpPrecedences.add(
                    MetadataSourceType.parsePrecedence((String) precedences.nextElement()));
        }
        ...
        while (itr.hasNext()) {
            ...
            if (clazz == null) {
                throw new MappingException(
                        "property-ref to unmapped class: " + upr.referencedClass);
            }
    ...
    private int processExtendsQueue() {
        ...
        while (extendsQueueEntry != null) {
            metadataSourceQueue.processHbmXml(extendsQueueEntry.getMetadataXml(),
    ...
    protected ExtendsQueueEntry findPossibleExtends() {
        ...
        while (itr.hasNext()) {
            ...
            boolean found = getClassMapping(entry.getExplicitName()) != null
                    || getClassMapping(HbmBinder.getClassName(entry.getExplicitName(),
    ...
    protected void secondPassCompileForeignKeys(Table table,
                                                Set<ForeignKey> done) throws MappingException {
        ...
        while (iter.hasNext()) {
            ...
            if (!done.contains(fk)) {
                ...
                if (referencedEntityName == null) {
                    throw new MappingException(
                            "An association from the table " + fk.getTable().getName()
    ...
    public SessionFactory buildSessionFactory(
        ...
        return new SessionFactoryImpl(this, mapping, serviceRegistry, settings,
    ...
    private void buildTypeRegistrations(ServiceRegistry serviceRegistry) {
        ...
        ClassLoaderService classLoaderService = serviceRegistry.getService(
                ClassLoaderService.class);
        for (TypeContributor contributor : classLoaderService.loadJavaServices(
                TypeContributor.class)) {
     ...
     */
    protected CustomConfiguration doConfigure(InputStream stream,
                                              String resourceName) throws HibernateException {
        try {
            ErrorLogger errorLogger = new ErrorLogger(resourceName);
            Document document = xmlHelper.createSAXReader(errorLogger, entityResolver).read(
    ...
    private void parseSessionFactory(Element sfNode, String name) {
        ...
        while (elements.hasNext()) {
            ...
            if ("mapping".equals(subelementName)) {
                ...
                setCollectionCacheConcurrencyStrategy(role, subelement.attributeValue("usage"),
                        region);
            }
        }
    }
    ...
    private void parseMappingElement(Element mappingElement, String name) {
        ...
        if (resourceAttribute != null) {
            ...
            LOG.debugf("Session-factory config [%s] named resource [%s] for mapping", name,
                    resourceName);
            ...
            LOG.debugf("Session-factory config [%s] named jar file [%s] for mapping", name,
                    jarFileName);
            addJar(new File(jarFileName));
        } else if (packageAttribute != null) {
            final String packageName = packageAttribute.getValue();
            LOG.debugf("Session-factory config [%s] named package [%s] for mapping", name,
                    packageName);
        ...
        while (grantElements.hasNext()) {
            ...
            if ("grant".equals(elementName)) {
                jaccPermissionDeclarations.addPermissionDeclaration(
                        new GrantedPermission(grantElement
                                .attributeValue("role"), grantElement.attributeValue("entity-name"),
                                grantElement
    ...
    RootClass getRootClassMapping(String clazz) throws MappingException {
        try {
            ...
            throw new MappingException(
                    "You may only specify a cache for root <class> mappings.  Attempted on " + clazz);
        }
    }
     ...
     * @return this for method chaining
     */
    public CustomConfiguration setCacheConcurrencyStrategy(String entityName,
    ...
    public void setCacheConcurrencyStrategy(String entityName, String concurrencyStrategy,
                                            String region,
                                            boolean cacheLazyProperty) throws MappingException {
        caches.add(
                new CacheHolder(entityName, concurrencyStrategy, region, true, cacheLazyProperty));
    }
     ...
     * @return this for method chaining
     */
    public CustomConfiguration setCollectionCacheConcurrencyStrategy(String collectionRole,
                                                                     String concurrencyStrategy) {
    ...
    public Mapping buildMapping() {
        return new Mapping() {
            ...
            public Type getReferencedPropertyType(String entityName,
                ...
                if (prop == null) {
                    throw new MappingException(
                            "property not known: " + entityName + '.' + propertyName);
                }
                return prop.getType();
            }
        };
    ...
    public void addAttributeConverter(AttributeConverter attributeConverter) {
        ...
        Converter converterAnnotation = attributeConverter.getClass().getAnnotation(
                Converter.class);
    ...
    public void addAttributeConverter(AttributeConverterDefinition definition) {
        ...
        if (old != null) {
            throw new AssertionFailure(
                    String.format("AttributeConverter class [%s] registered multiple times",
                            definition.getAttributeConverter().getClass()));
        }
    }
    ...
    protected class MappingsImpl implements ExtendedMappings, Serializable {
        ...
        public Table addTable(String schema, String catalog, String name, String subselect,
            ...
            if (table == null) {
                table = TableFactory.build(dialect);
        ...
        public Table addDenormalizedTable(String schema, String catalog, String name,
                                          boolean isAbstract,
                                          String subselect,
        ...
        public void addQuery(String name,
                             NamedQueryDefinition query) throws DuplicateMappingException {
        ...
        public void addSQLQuery(String name,
                                NamedSQLQueryDefinition query) throws DuplicateMappingException {
            if (!defaultNamedNativeQueryNames.contains(name)) {
                applySQLQuery(name, query);
            }
        }

        private void applySQLQuery(String name,
                                   NamedSQLQueryDefinition query) throws DuplicateMappingException {
            checkQueryName(name);
        ...
        public void addNamedProcedureCallDefinition(NamedProcedureCallDefinition definition)
            ...
            if (!defaultNamedProcedure.contains(name)) {
                final NamedProcedureCallDefinition previous = namedProcedureCallMap.put(name,
                        definition);
                if (previous != null) {
        ...
        public void applyResultSetMapping(ResultSetMappingDefinition sqlResultSetMapping)
                throws DuplicateMappingException {
            Object old = sqlResultSetMappings.put(sqlResultSetMapping.getName(),
                    sqlResultSetMapping);
            if (old != null) {
        ...
        public String getLogicalTableName(Table table) throws MappingException {
            return getLogicalTableName(table.getQuotedSchema(), table.getQuotedCatalog(),
                    table.getQuotedName());
        }
        ...
        public void addTableBinding(String schema, String catalog, String logicalName,
                                    String physicalName,
        ...
        public void addColumnBinding(String logicalName, Column physicalColumn, Table table)
                throws DuplicateMappingException {
            TableColumnNameBinding binding = (TableColumnNameBinding) columnNameBindingPerTable.get(
                    table);
            if (binding == null) {
        ...
        public String getPhysicalColumnName(String logicalName,
                                            Table table) throws MappingException {
            ...
            if (finalName == null) {
                throw new MappingException(
                        "Unable to find column with logical name " + logicalName + " in table "
                                + table.getName());
            }
            return finalName;
        }
        ...
        public String getLogicalColumnName(String physicalName,
            ...
            if (logical == null) {
                throw new MappingException(
                        "Unable to find logical column name from physical name " + physicalName
        ...
        public void addPropertyAnnotatedWithMapsIdSpecj(XClass entityType, PropertyData property,
                                                        String mapsIdValue) {
            Map<String, PropertyData> map = propertiesAnnotatedWithMapsId.get(entityType);
            if (map == null) {
                map = new HashMap<String, PropertyData>();
        ...
        public PropertyData getPropertyAnnotatedWithIdAndToOne(XClass entityType,
                                                               String propertyName) {
        ...
        public void addUniqueConstraintHolders(Table table,
            ...
            List<UniqueConstraintHolder> holderList = getUniqueConstraintHoldersByTable().get(
                    table);
            if (holderList == null) {
        ...
        public void addAnyMetaDef(AnyMetaDef defAnn) throws AnnotationException {
            if (anyMetaDefs.containsKey(defAnn.name())) {
                throw new AnnotationException(
                        "Two @AnyMetaDef with the same name defined: " + defAnn.name());
            }
        ...
        private class TableColumnNameBinding implements Serializable {
            ...
            private void bindLogicalToPhysical(String logicalName, Column physicalColumn)
                ...
                if (existingPhysicalName != null) {
                    boolean areSamePhysicalColumn = physicalColumn.isQuoted() ? existingPhysicalName
                            .equals(physicalName) : existingPhysicalName.equalsIgnoreCase(
                            physicalName);
                    if (!areSamePhysicalColumn) {
                        throw new DuplicateMappingException(
                                " Table [" + tableName + "] contains logical column name ["
                                        + logicalName + "] referenced by multiple physical column names: ["
                                        + existingPhysicalName + "], [" + physicalName + "]",
                                "column-binding", tableName + "."
                                + logicalName);
                    }
                }
            }
            ...
            private void bindPhysicalToLogical(String logicalName, Column physicalColumn)
                ...
                if (existingLogicalName != null && !existingLogicalName.equals(logicalName)) {
                    throw new DuplicateMappingException(
                            " Table [" + tableName + "] contains physical column name ["
    ...
    protected class MetadataSourceQueue implements Serializable {
        ...
        private void writeObject(java.io.ObjectOutputStream out) throws IOException {
            out.defaultWriteObject();
            ArrayList<Class<?>> serializableAnnotatedClasses = new ArrayList<Class<?>>(
        ...
        private void processHbmXml(XmlDocument metadataXml, Set<String> entityNames) {
            try {
                HbmBinder.bindRoot(metadataXml, createMappings(), Collections.emptyMap(),
                        entityNames);
            } catch (MappingException me) {
                throw new InvalidMappingException(metadataXml.getOrigin().getType(),
            ...
            for (XClass clazz : original) {
                ...
                while (superClass != null && !reflectionManager.equals(superClass, Object.class)
                    ...
                    if (superClass.isAnnotationPresent(Entity.class)
                            || superClass.isAnnotationPresent(
        ...
        private void orderHierarchy(List<XClass> copy, List<XClass> newList, List<XClass> original,
                                    XClass clazz) {
