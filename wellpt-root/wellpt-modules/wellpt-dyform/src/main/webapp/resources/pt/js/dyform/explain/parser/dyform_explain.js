...
$(function () {
    ...
    $.extend($.dyform, {
        ...
        nss: function (ns, selector) { // namespaceSelector
            if ($.trim(ns).length) {
                return selector + "[data-ns=\"" + ns + "\"]";
            }
            return selector + ":not([data-ns])";
        },
        wns: function (ns, html) { // wrapNamespace
            // 添加命名空间
            ...
            $wrap.find(".layout[name]").each(function (idx, element) {
                var name = $(element).attr("name");
                $(element).attr("data-ns", ns);
        ...
        parseRequireModules: function (formDefinition) {
            ...
            var requireModules = [], formDefinitions = [formDefinition];
            var subformDefinitions = formDefinition["subformDefinitions"];
            if ($.isPlainObject(formDefinition.layouts)) {
                requireModules.push("wLayoutManager");
            }
        ...
        warn: (function (win) {
            ...
            return (false === $.isFunction(warn) ? $.noop : function (msg) {
        ...
        .extend(/**
             ...
             */
            {
                ...
                hasBlockAnchor: function (formDefinition) {
                    ...
                    if (anchors.length >= 2) {
                        ...
                        for (var i = 0; i < anchors.length; i++) {
                            if (i == 0) {
                                $ulHtml += '<li class="anchor_active" data-code="' + anchors[i].blockCode + '">' + anchors[i].blockTitle + '</li>';
                            } else {
                                $ulHtml += '<li data-code="' + anchors[i].blockCode + '">' + anchors[i].blockTitle + '</li>';
                            }
                        }
                ...
                parseForm: function (options) {
                    ...
                    if (this.isDisplayAsModel()) {// 判断是否用显示单据
                        ...
                        $("li", ".wellpt_form_anchor").on("click", function () {
                            ...
                            var scroll_top = $("td[blockcode='" + $(this).data("code") + "']").offset().top - 71;
                            isScroll = true
                        ...
                        $(window).on('scroll', function () {
                            ...
                            var scrollTop = $(window).scrollTop(),
                                tables = $(".wellpt_form_container table").find("td[blockanchor='true']");
                            if (scrollTop > 800) {
                            ...
                            for (var i = 0; i < tables.length; i++) {
                                var that = $(tables).eq(i).parents("table");
                                if (scrollTop < that.offset().top) {
                                    $(".wellpt_form_anchor li").removeClass('anchor_active');
                                    $(".wellpt_form_anchor li").eq(i).addClass('anchor_active');
                                    break;
                                }
                            }
                        })
                    ...
                    for (var i = 0; i < defaultEventNames.length; i++) {
                        if (!this.isEventExist(defaultEventNames[i])) {// 二开文件中绑定的事件优先级更高
                            if (typeof this.getOption()[defaultEventNames[i]] == "function") {// 初始化时，有初始化该事件
                    ...
                    function fillFormData() {
                        _this.fillFormData(data, {
                            ...
                            callback: function () {
                                ...
                                if (_this.isDisplayAsLabel()) { // 显示为标签
                                    _this.showAsLabel();
                                }
                ...
                afterParseForm: function () {
                    ...
                    if ($("#wf_form").length > 0 && $("#wf_form").siblings("#dyform").length > 0) {
                        //表单滚动条，侧边tab打开状态并且不分屏,滚动条不显示，其他情况都显示
                        var scollHeight = $(window).height();
                        $("#wf_form").siblings("#dyform").height(scollHeight - 80).niceScroll({
                        ...
                        setTimeout(function () {
                            $("#wf_form").siblings("#dyform").getNiceScroll().resize();
                        }, 1000)
                        $(document).on("filechange", function (e) {
                            setTimeout(function () {
                                $("#wf_form").siblings("#dyform").getNiceScroll().resize();
                            }, 100)
                        })
                            ...
                            if (113 == event.keyCode) {
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        if (true == data) {
                                            openWindowByPost(ctx + "/pt/dyform/data/previewMobileView.action", {
                ...
                formatModel: function (formDefinition) {
                    ...
                    $($.dyform.nss(ns, ".model_subform[fieldName]")).each(
                        function () {
                            var $this = $(this);
                            var name = $this.attr("fieldName");
                            if (name == undefined && $.trim(name).length == 0) {
                ...
                collectFormData: function (success, error) {
                    ...
                    this.getFormDatas(function (formDatas) {// 收集成功回调
                        if (_this.isEnableSignature()) {// 创建签名
                ...
                showValidateMessage: function (control) {
                    ...
                    if (control.getPos() == dyControlPos.subForm) {// 在从表中
                        control.setEditable();
                    }
                    var editableElem = control.get$InputElem();
                    if (editableElem != null && editableElem.size() > 0) {
                ...
                // _focusElement:function($wrapElement){
                //     this._unfocusAllElements();

                //     $wrapElement.addClass('focused');
                //     setTimeout(function () {
                //         $wrapElement.removeClass('focused')
                //     }, 4800);
                // },

                // _unfocusAllElements: function(){
                ...
                activeLayoutByControl: function (control) {
                    var $layouts = control.$placeHolder.parents(".layout");
                    if ($layouts.size() == 0) {// 没有放在布局里面
                        return;
                    }

                    for (var i = $layouts.size(); i >= 0; i--) {
                        var $layoutElem = $($layouts[i]);
                        if ($layoutElem.is(".tab")) {// 页签布局里面
                ...
                reFillFormData: function (newData, callback) {
                    var self = this;
                    self.collectFormData(function (formData) {
                ...
                fillFormDisplayData: function (formDatas, formUuid, callback) {
                    ...
                    var _this = this, dlg_selector = "excel_dlg_confirm";
                    ...
                    var json = {
                        ...
                        width: 400,
                        height: 180,
                        content: "<div id=\"popup_message_div\" style=\"height: 70px;\"><div id=\"popup_message_icon\"></div><div id=\"popup_message\" style=\"width: 310px; height: 45px;overflow: auto;\">追加&nbsp;:&nbsp;导入时,保留之前的单据。<br>覆盖&nbsp;:&nbsp;清空之前的单据,然后导入。</div></div>",
                ...
                setEditable: function () {
                    var _this = this;
                    var ns = _this.getOption().namespace;
                    this.$($.dyform.nss(ns, ".value[fieldName]")).each(
                        function () {
                            var fieldName = $(this).attr("fieldName");
                            var control = _this.getControl(fieldName);
                            if (typeof control == "undefined" || control == null
                                || control.getPos() == dyControlPos.subForm) {
                                return true; // continue;
                            }
                            control.setEditable();
                        });
                ...
                handleException: function (exceptionData, options) {
                    ...
                    if (subcode === "SerialNumberOccupy") {
                        ...
                        appModal.confirm(exceptionData.title, function (result) {
                            if (result) {
                                dyformDataOptions.putOptions("serialNumberConfirmed", true);
    ...
    $.dyform.extend({
        getFieldNameByApplyTo: function (applyTo, formUuid) {
            ...
            if (typeof fieldName == "undefined") {
                console.log("cann't get the fieldName for mappingName[" + JSON.cStringify(applyTo) + "][formID:"
                    + formDefinition.id + "]");
        ...
        getControl: function (fieldName, dataUuidOfSubform) {
            var self = this;
            var controlId = fieldName, options;
        ...
        getControlByControlId: function (controlId) {
            var self = this;
            var ctl = self.cache.get.call(self, cacheType.control, controlId) || ($.ControlManager[controlId] && $.ControlManager[controlId].control);
            return ctl;
        },
        ...
        autoWidth: function () {
            ...
            _this.resizeTid = setTimeout(function () {
                var ns = _this.getOption().namespace;
                _this.$($.dyform.nss(ns, ".value[fieldName][pos!='" + dyControlPos.subForm + "']")).each(function () {// 遍历主表的占位符
                    var fieldName = $(this).attr("fieldName");
                    var control = _this.getControl(fieldName);
                    if (typeof control == "undefined" || control == null) {
                        return true;
        ...
        getLayout: function (name) {
            return $.LayoutManager.getLayout(name);
        }
    ...
    $.dyform.extend({
        ...
        getFormUuidByRowId: function (rowId) {
            ...
            var formUuid = null;
            self.getFormDatas(function (formDatas) {
                for (var i in formDatas) {
    ...
    $.dyform.extend({
        ...
        hideBlock: function (blockCode) {
            var _this = this;
            var ns = _this.getOption().namespace;
            this.$("td[blockCode='" + blockCode + "']").attr("blockhide", true);
            this.$("td[blockCode='" + blockCode + "']").closest('table').hide();
        },
        /**
        ...
        showBlock: function (blockCode) {
            var _this = this;
            var ns = _this.getOption().namespace;
            this.$("td[blockCode='" + blockCode + "']").attr("blockhide", false);
            this.$("td[blockCode='" + blockCode + "']").closest('table').show();
        },
            ...
            for (var i in tips) {
                ...
                (function (i) {
                    ...
                    if (type == "1") {
                        ...
                        var tips_index = 0;
                        $("#" + i).on("mouseover", ".tool-content", function (e) {
                            e.stopPropagation();
                            e.preventDefault();
                            if (tips_index) {
                                return false
        ...
        parseTab: function (formDefinition) {
            ...
            for (var i in tabs) {
                ...
                if (typeof subtabs == "undefined") {// 没有subtabs属性，不处理
                    continue;
                }
        ...
        hideTab: function (tabName) {
            var _this = this;
            var ns = _this.getOption().namespace;
            this.$("div[name='" + tabName + "']").hide();
            this.$("a[href='#" + tabName + "']").parent().hide();
            this.$("li:visible:eq(0)").find("a[href*='sub']").tab('show');//子页签code sub开头
        ...
        showTab: function (tabName) {
            var _this = this;
            var ns = _this.getOption().namespace;
            this.$("div[name='" + tabName + "']").show();
            this.$("a[href='#" + tabName + "']").parent().show();
        },
        /**
         * 主表解析
         *
        ...
        parseMainForm: function (formDefinition, formData) {
            ...
            var ns = _this.getOption().namespace;
            // 解析布局
            _this.$($.dyform.nss(ns, ".layout[layoutName]")).each(function () {
                var name = $(this).attr("layoutName");
                if (!(name in formDefinition.layouts)) {
                    return true;// continue;
                }

                $.LayoutManager.createLayout($(this), name, formDefinition);
            });
            // 解析控件
            _this.$($.dyform.nss(ns, ".value[fieldName]")).each(function () {
        ...
        validateMainform: function (formUuid) {
            ...
            this.$($.dyform.nss(ns, ".value[fieldName]")).each(function () {
                var fieldName = $(this).attr("fieldName");
                if ($.dyform.isValidateIgnore(formId, fieldName)) {// 不需要验证的字段
                    return true;
                } else {
                    var control = _this.getControl(fieldName);
                    if (typeof control == "undefined" || control == null || control.getPos() == dyControlPos.subForm) {// 从表不需要验证
                        return true;
                    }
                }
        ...
        collectMainformData: function (success, error) {
            ...
            var promiseFuns = [];
            var ns = _this.getOption().namespace;
            this.$($.dyform.nss(ns, ".value[fieldName][pos!='" + dyControlPos.subForm + "']")).each(function () {// 遍历主表的占位符
                var fieldName = $(this).attr("fieldName");
                var control = _this.getControl(fieldName);
                if (typeof control == "undefined" || control == null) {
                    return true;
                ...
                var promiseFun = function () {
                    var def = $.Deferred();
                    if (control.isGetValueByAsyn && control.isGetValueByAsyn()) {// 需要异步取值
                        control.getValue(function (value) {
            ...
            var fnThen = function (index) {
                promiseFuns[index]().then(function (fieldName, value) {
                    formData[fieldName] = value;
                    if (index == promiseFuns.length - 1) {// 最后一个元素
                        // console.log("数据收集完成");
                ...
                }, function (fieldName, errorInfo) {// 附件上传失败
                    console.log(fieldName + "字段取值， 失败回调");
                    error.call(_this, errorInfo);
                });

            };
            if (promiseFuns.length == 0) {// 主表没有控件
                var dataUuid = _this.getDataUuid();// 先从缓存中获取
                if (typeof dataUuid == "undefined") {
                    dataUuid = _this.createUuid();
                    _this.setDataUuid(dataUuid);
        ...
        collectMainformDisplayData: function (formUuid) {
            ...
            var ns = _this.getOption().namespace;
            var formData = {};
            this.$($.dyform.nss(ns, ".value[fieldName]")).each(function () {// 遍历主表的占位符
                var fieldName = $(this).attr("fieldName");
                var control = _this.getControl(fieldName);
                if (typeof control == "undefined" || control == null) {
                    return true;
        ...
        getFormDatas: function (success, error) {
            ...
            for (var i in subforms) {
                (function (j) {
                    ...
                    promiseFuns.push(function () {
                        var def = $.Deferred();
                        subformctl.collectSubformData(function (formData) {
            ...
            promiseFuns.push(function () {
                // 收集主表数据
                var def = $.Deferred();
                var mainformData = _this.collectMainformData(function (mainformData) {// 收集成功
                    var mainformDatas = [];
                    mainformDatas.push(mainformData);
                    def.resolve(formUuid, mainformDatas);
                }, function (errorInfo) {// 收集失败
                    def.reject(errorInfo);
                });
                return def.promise();
            });
                ...
                promiseFuns[index]().then(
                    function (formUuid, formData) {
                        for (var i in formData) {
                            ...
                            _this.compareNewAndOldData(_this.getOldFormData(formUuid, formDataOfOneForm.uuid),
                                formDataOfOneForm);// 新旧数据对比
                        }

                        formDatas[formUuid] = formData;
                        if (index == promiseFuns.length - 1) {// 最后一个
                            success.call(_this, formDatas);
                        } else {
                            fnThen(index + 1);
                        }
                    }, function (errorInfo) {// 附件上传失败
                        error.call(_this, errorInfo);
                    });
            };
            fnThen(0);
        ...
        getOldFormData: function (formUuid, dataUuid) {
            ...
            if (oldFormDatasOfOneForm == undefined || oldFormDatasOfOneForm == null
                || this.objectLength(oldFormDatasOfOneForm) == 0) {
                return {};
            }
        ...
        compareNewAndOldData: function (oldFormData, formData) {
            ...
            if (oldFormData == undefined || this.objectLength(oldFormData) == 0) {// 新数据,标记为新增
                this.markAsAddedFormData(formUuid, dataUuid);
            } else {// 旧数据
                if (this.isMarkedAsNewFormData(formUuid, dataUuid)) {// 已被标记为新增
                    return;
                } else {
                    ...
                    for (var field in formData) {
                        ...
                        var newV = formData[field];
                        var oldV = oldFormData[field];
                        if (typeof newV == "object" && typeof oldV == "object" && newV instanceof Array) {// 附件
        ...
        isMarkedAsNewFormData: function (formUuid, dataUuid) {
            ...
            if (addedFormDatasOfOneForm == null || addedFormDatasOfOneForm == undefined
                || addedFormDatasOfOneForm.length == 0) {
                return false;
            }
        ...
        fillSubformData: function (formDatas, subformUuids, index, optional) {
            ...
            if (index == subformUuids.length - 1) {
                subformctl.fillFormData(formData, {
                    ...
                    callback: function () {
                        var timey = (new Date()).getTime();
                        console.log("填充从表[" + subformctl.getDisplayName() + "][" + formData.length + "]的数据所用时间:"
                            + (timey - timex) / 1000.0 + "s");
                        optional.callback();
                    }
                });
            } else {
