 ...
 */
@Component
public class WorkFlowDataSource extends AbstractDataSourceProvider {
    ...
    public Collection<DataSourceColumn> getAllDataSourceColumns() {
        ...
        taskId.setFieldName("taskId");
        taskId.setColumnName("taskId");
        taskId.setColumnAliase("taskId");
        taskId.setTitleName("办理环节ID");
        dataSourceColumns.add(taskId);

        DataSourceColumn taskNameBL = new DataSourceColumn();
        taskNameBL.setFieldName("taskNameBL");
        taskNameBL.setColumnName("taskNameBL");
        taskNameBL.setColumnAliase("taskNameBL");
        taskNameBL.setTitleName("办理环节名称");
        dataSourceColumns.add(taskNameBL);

        DataSourceColumn createTimeBL = new DataSourceColumn();
        createTimeBL.setFieldName("createTimeBL");
        createTimeBL.setColumnName("createTimeBL");
        createTimeBL.setColumnAliase("createTimeBL");
        createTimeBL.setTitleName("办理时间");
        dataSourceColumns.add(createTimeBL);
     ...
     * @return
     */
    private String getAssignee(String taskInstUuid) {
        StringBuilder sb = new StringBuilder();
        List<AclSid> aclSids = aclServiceWrapper.getSid(TaskInstance.class, taskInstUuid, AclPermission.TODO);
    ...
    public List<QueryItem> query(Set<DataSourceColumn> dataSourceColumn, String whereHql,
        ...
        for (DataSourceColumn dc : dataSourceColumn) {
            if (dc.getFieldName().equals("taskId")) {
                ...
                continue;
            } else if (dc.getFieldName().equals("createTimeBL")) {
                continue;
        ...
        whereHql = StringUtils.replace(whereHql, " dueTime ", " o.flowInstance.dueTime ");
        whereHql = StringUtils.replace(whereHql, " createTimeBL ", " o.flowInstance.createTime ");
        ...
        whereHql = StringUtils.replace(whereHql, " taskNameBL ", " o.name ");
        ...
        List<QueryItem> queryItems = aclService.queryForItem(TaskInstance.class, aclQueryInfo, AclPermission.DONE,
        ...
        if (!queryItems.isEmpty()) {
            ...
            values.put("assignee", SpringSecurityUtils.getCurrentUserId());
            ...
            Map<String, QueryItem> doneMap = new HashMap<String, QueryItem>();
            for (QueryItem doneItem : doneItems) {
                String flowInstUuid = doneItem.getString(QueryItem.getKey("flowInstUuid"));
            ...
            for (QueryItem queryItem : queryItems) {
                ...
                if (doneMap.containsKey(flowInstUuid)) {
                    QueryItem doneItem = doneMap.get(flowInstUuid);
                    queryItem.put("taskId", doneItem.getString(QueryItem.getKey("办理环节id")));
                    queryItem.put("taskNameBL", doneItem.getString(QueryItem.getKey("环节名称")));
                    queryItem.put("createTimeBL", doneItem.getDate(QueryItem.getKey("办理时间")));
                }
            }
        }

        for (QueryItem queryItem : queryItems) {
            if (queryItem.get("taskUuid") != null) {
                queryItem.put("assignee", getAssignee(queryItem.get("taskUuid").toString()));
            }
    ...
    private void decorateGzWorkData(List<QueryItem> queryItems) {
        ...
        for (UfWfGzWorkData ufWfGzWorkData : ufWfGzWorkDatas) {
            ...
            for (QueryItem queryItem : items) {
                ...
                if (StringUtils.isNotBlank(ufWfGzWorkData.getDoneTaskName())) {
                    queryItem.put("taskNameBL", ufWfGzWorkData.getDoneTaskName());
                } else {
                    queryItem.put("taskNameBL", ufWfGzWorkData.getCurrentTaskName());
                }
            }
        }
    }
