...
public final class Util {
    ...
    public static boolean areRegionsEqual(CellRangeAddress region1, CellRangeAddress region2) {
        ...
        return (region1.getFirstColumn() == region2.getFirstColumn()
                && region1.getLastColumn() == region2.getLastColumn() && region1.getFirstRow() == region2.getFirstRow() && region2
            ...
            for (int j = oldRow.getFirstCellNum(), c = oldRow.getLastCellNum(); j <= c; j++) {
                ...
                if (oldCell != null) {
                    ...
                    if (mergedRegion != null) {
                        CellRangeAddress newMergedRegion = new CellRangeAddress(newRow.getRowNum(), newRow.getRowNum()
    ...
    public static void copyCell(org.apache.poi.ss.usermodel.Cell oldCell, org.apache.poi.ss.usermodel.Cell newCell,
        ...
        switch (oldCell.getCellType()) {
            case STRING:
                ...
                newCell.setCellValue(newCell.getSheet().getWorkbook().getCreationHelper()
                        .createRichTextString(oldValue.replaceAll(expressionToReplace, expressionReplacement)));
                break;
            case NUMERIC:
                newCell.setCellValue(oldCell.getNumericCellValue());
                break;
