 ...
 * @date 2017年10月12日
 */
public class HqlDataFactory extends BaseDataFactory implements DataFactory {
    ...
    public Object createData(Object dest, Class<?> destClass, String targetDataId) {
        if (targetDataId == null) {
            throw new MapperException("HqlDataFactory targetDataId is not null");
        }
        targetDataId = targetDataId.toLowerCase();
        if (targetDataId.startsWith("from ") || targetDataId.startsWith("select ")) {
            UniversalDao dao = ApplicationContextHolder.getBean("universalDao", UniversalDao.class);
            Query query = dao.getSession().createQuery(targetDataId);
