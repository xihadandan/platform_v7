 ...
 */
@Component
public class WorkFlowMonitorDataSource extends AbstractDataSourceProvider {
    ...
    public List<QueryItem> query(Set<DataSourceColumn> dataSourceColumns, String whereSql, Map<String, Object> queryParams, String orderBy,
        ...
        List<QueryItem> queryItems = nativeDao.namedQuery("monitorTaskInstanceQuery", values, QueryItem.class, pagingInfo);

        // 查询总数
        if (autoCount) {
            List<QueryItem> count = nativeDao.namedQuery("countMonitorTaskInstanceQuery", values, QueryItem.class);
