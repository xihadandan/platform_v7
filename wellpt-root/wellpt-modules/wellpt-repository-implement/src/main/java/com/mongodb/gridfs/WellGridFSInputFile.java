...
import com.wellsoft.pt.repository.support.CheckedBufferInputStream;
...
/**
 * Description: 保存定制的MD5,计算前8M的数据流
 *
 * @author zhongzh
 * @version 1.0
 *
 ...
 * @date 2016年7月5日
 */
public class WellGridFSInputFile extends com.mongodb.gridfs.GridFSInputFile {

    private final CheckedBufferInputStream _in;
     ...
     */
    protected WellGridFSInputFile(GridFS fs, InputStream in, String filename, boolean closeStreamOnPersist) {
        super(fs, in, filename, closeStreamOnPersist);
        this.fs = fs;
        try {
            _in = new CheckedBufferInputStream(in);
        } catch (NoSuchAlgorithmException e1) {
            throw new RuntimeException("No MD5!", e1);
        } catch (IOException e1) {
            throw new RuntimeException("fileName[" + filename + "] Digest Error", e1);
        }
		 ...
		 */
        _buffer = new byte[(int) this.chunkSize];
    }
...
//			_outputStream = new WellMyOutputStream();
//		}
...
//	}
     ...
     * @param filename Name of the file to be created.
     */
    protected WellGridFSInputFile(GridFS fs, InputStream in, String filename) {
     ...
     * @param filename Name of the file to be created.
     */
    protected WellGridFSInputFile(GridFS fs, String filename) {
     ...
     * @param fs The GridFS connection handle.
     */
    protected WellGridFSInputFile(GridFS fs) {
        this(fs, null, null);
    }
    ...
    public void save(long chunkSize) {
        ...
        // note that chunkSize only changes this.chunkSize in case we actually save chunks
    ...
    public int saveChunks(long chunkSize) throws IOException {
        ...
        if (chunkSize <= 0 || chunkSize > GridFS.MAX_CHUNKSIZE) {
            throw new MongoException(
                    "chunkSize must be greater than zero and less than or equal to GridFS.MAXthis.chunkSize");
        }
    ...
    private void _dumpBuffer(boolean writePartial) {
        if ((_currentBufferPosition < this.chunkSize) && !writePartial) {
        ...
        if (_currentBufferPosition != this.chunkSize) {
            writeBuffer = new byte[_currentBufferPosition];
            System.arraycopy(_buffer, 0, writeBuffer, 0, _currentBufferPosition);
        }

        DBObject chunk = createChunk(this.id, _currentChunkNumber, writeBuffer);
    ...
    private int _readStream2Buffer() throws IOException {
        int bytesRead = 0;
        while (_currentBufferPosition < this.chunkSize && bytesRead >= 0) {
            bytesRead = _in.read(_buffer, _currentBufferPosition, (int) this.chunkSize - _currentBufferPosition);
            if (bytesRead > 0) {
    ...
    private void _finishData() {
        if (!_savedChunks) {
            // override md5
            this.md5 = _in == null ? null : _in.getMd5();// Util.toHex(_messageDigester.digest());

            // _messageDigester = null;
            this.length = _totalBytes;
            _savedChunks = true;
            try {
     ...
     * @author Guy K. Kloss
     */
    /*class WellMyOutputStream extends MyOutputStream {
     ...
     *//*
		...
		public void close() {
			...
			_finishData();
			// save file obj
			WellGridFSInputFile.super.save();
		}
	}*/
