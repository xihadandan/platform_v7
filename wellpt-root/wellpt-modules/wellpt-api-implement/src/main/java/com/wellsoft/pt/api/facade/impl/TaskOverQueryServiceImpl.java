 ...
 * @date 2014-8-12
 */
@Service(ApiServiceName.TASK_OVER_QUERY)
public class TaskOverQueryServiceImpl extends TaskQueryServiceImpl {
    ...
    protected TaskQuery preTaskQuery(WellptQueryRequest<?> queryRequest) {
        ...
        whereSql.append("(t2.is_active = 0 and t2.end_time is not null)");
     ...
     * (non-Javadoc)
     *
     * @see com.wellsoft.pt.api.facade.impl.TaskQueryServiceImpl#order(com.wellsoft.pt.bpm.engine.query.api.TaskQuery)
     */
    @Override
    protected void order(TaskQuery taskQuery) {
        taskQuery.order("order by t2.end_time desc");
    }
