...
/**
 * Description: Entity dao 只用于与平台实体类规范无关的应用服务上
 *
 * @param <Entity>
 * @param <UUID>
 * @author chenqiong
...
public abstract class EntityDaoImpl<Entity extends Serializable> implements EntityDao<Entity> {

    protected static final Logger LOGGER = LoggerFactory.getLogger(EntityDaoImpl.class);

    private static final ConcurrentHashMap<String, EntityDao> DAO_MAP = new ConcurrentHashMap<String, EntityDao>();
     ...
     * @return
     */
    protected static String generateDynamicNamedQueryString(SessionFactory sessionFactory, String queryName,
        ...
        if (namedSqlReloadable || Context.isDebug()) {
            try {
                ...
                if (resource != null) {
                    ...
                    cfg.buildMappings();
                    NamedSQLQueryDefinition define = (NamedSQLQueryDefinition) cfg.getNamedSQLQueries().get(queryName);
                    hql = define.getQueryString();
                } else {
                    hql = ((SessionFactoryImpl) sessionFactory).getNamedSQLQuery(queryName).getQueryString();
                }
            } catch (Exception e) {
        ...
        try {
            // 去掉/* */的注释
            hql = TemplateEngineFactory.getDefaultTemplateEngine().process(hql.replaceAll("/\\*.*\\*/", ""), values);
        } catch (Exception e) {
            LOGGER.error(ExceptionUtils.getStackTrace(e));
    ...
    protected void initSessionFactory() {
        String sessionFactoryBeanName = Config.MULTI_TENANCY
                && this.entityClass.getAnnotation(CommonEntity.class) != null ? Config.COMMON_SESSION_FACTORY_BEAN_NAME
                : Config.TENANT_SESSION_FACTORY_BEAN_NAME;
        try {
            bindSessionFactory(sessionFactoryBeanName, Config.getValue(Config.KEY_MULTI_TENANCY_STRATEGY));
        } catch (Exception e) {
     ...
     */
    @Override
    public Entity getOne(Serializable uuid) {
        return (Entity) getSession().get(entityClass, uuid);
    }

    /**
     * 按属性查找唯一对象, 匹配方式为相等.
     */
    public Entity getUniqueBy(final String propertyName, final Object value) {
        List<Entity> results = (List<Entity>) listByFieldEqValue(propertyName, value);
     ...
     */
    @Override
    public Entity getOneByNameHQLQuery(String queryName, Map<String, Object> values) {
        String hql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
        List<Entity> results = (List<Entity>) this.listQuery(hql, values, entityClass, null, true);
        return CollectionUtils.isNotEmpty(results) ? results.get(0) : null;
    }
     ...
     */
    @Override
    public Entity getOneByNameSQLQuery(String queryName, Map<String, Object> values) {
        String sql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
        List<Entity> results = (List<Entity>) this.listQuery(sql, values, entityClass, null, false);
        return CollectionUtils.isNotEmpty(results) ? results.get(0) : null;
    }
     ...
     */
    @Override
    public List<Entity> listByNameHQLQuery(String queryName, Map<String, Object> values) {
        return this.listByNameHQLQuery(queryName, values, null);
    }

    @Override
    public List<Entity> listByNameSQLQuery(String queryName, Map<String, Object> values) {
        return this.listByNameSQLQuery(queryName, values, null);
    }

    @Override
    public List<Entity> listByNameSQLQuery(String queryName, Map<String, Object> values, PagingInfo pagingInfo) {
        String sql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
        return (List<Entity>) this.listQuery(sql, values, entityClass, pagingInfo, false);
    }
     ...
     * @return
     */
    public List<Entity> listByNameHQLQuery(String queryName, final Map<String, Object> values, PagingInfo pagingInfo) {
        String hql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
        return (List<Entity>) this.listQuery(hql, values, entityClass, pagingInfo, true);
    }
     ...
     * @return
     */
    public List<QueryItem> listQueryItemByNameHQLQuery(String queryName, final Map<String, Object> values,
    ...
    public <ITEM extends BaseQueryItem> List<ITEM> listItemByNameSQLQuery(String queryName, Class<ITEM> itemClass,
    ...
    public <ITEM extends BaseQueryItem> List<ITEM> listItemByNameHQLQuery(String queryName, Class<ITEM> itemClass,
                                                                          Map<String, Object> values, PagingInfo pagingInfo) {
        // 条件与排序信息在HQL中
        String hql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
     ...
     * (non-Javadoc)
     *
     * @see JpaDao#listQueryItemBySQL(String, Map, PagingInfo)
     */
    @Override
     ...
     * @return
     */
    public List<QueryItem> listQueryItemByNameSQLQuery(String queryName, final Map<String, Object> values,
                                                       PagingInfo pagingInfo) {
        // 条件与排序信息在HQL中
        String hql = generateDynamicNamedQueryString(sessionFactory, queryName, values);
    ...
    private <T> List<T> listQuery(String queryLang, final Map<String, Object> values, Class<T> itemClass,
        ...
        if (pagingInfo != null) {
            if (pagingInfo.isAutoCount()) {
                long totalCount = isHQL ? countByHQL(queryLang, values) : countBySQL(queryLang, values);
    ...
    public long countBySQL(final String sql, final Map<String, Object> values) {
        ...
        try {
            Long count = ((BigDecimal) createSQLQuery(countHql, values).uniqueResult()).longValue();
            return count;
        } catch (Exception e) {
    ...
    @Override
    public long countByEntity(Entity entity) {
        Criteria criteria = getSession().createCriteria(ClassUtils.getUserClass(entity)).add(Example.create(entity));
     ...
     * @param uuids
     */
    public void deleteByUuids(Collection<? extends Serializable> uuids) {
    ...
    public List<Entity> listByHQLAndPage(String hql, Map<String, Object> params, PagingInfo page) {
        return (List<Entity>) this.listQuery(hql, params, entityClass, page, true);
    }

    public List<Entity> listBySQLAndPage(String sql, Map<String, Object> params, PagingInfo page) {
        return (List<Entity>) this.listQuery(sql, params, entityClass, page, false);
    }

    public Entity getOneByHQL(String hql, Map<String, Object> params) {
        Query query = getSession().createQuery(hql);
        if (MapUtils.isNotEmpty(params)) {
            query.setProperties(params);
        }
    ...
    public Entity getOneBySQL(String sql, Map<String, Object> params) {
        return (Entity) createSQLQuery(sql, params).setResultTransformer(getItemTransformerMap(entityClass))
                .uniqueResult();
    }
    ...
    public List<Entity> listBySQL(String sql, Map<String, Object> params) {
        return createSQLQuery(sql, params).setResultTransformer(getItemTransformerMap(entityClass)).list();
    }

    private List<Entity> listByFieldMatchValue(String field, Object value, final MatchType matchType) {
     ...
     * @return
     */
    public List<Entity> listByFieldEqValue(String field, Object value) {
     ...
     * @return
     */
    public List<Entity> listByFieldNotEqValue(String field, Object value) {
        Criteria criteria = getSession().createCriteria(entityClass);
     ...
     * @return
     */
    public List<Entity> listByFieldNotInValues(String field, List<Object> value) {
     ...
     * @return
     */
    public List<Entity> listByFieldInValues(String field, List<Object> value) {
        if (CollectionUtils.isNotEmpty(value) && value.size() > 1000) {
            throw new ServiceException("in查询数量不能超过1000");
        }
        Criteria criteria = getSession().createCriteria(entityClass);
    ...
    public int deleteByNamedSQL(String queryName, Map<String, Object> params) {
        return executeBySQL(generateDynamicNamedQueryString(sessionFactory, queryName, params), params);
    }
    ...
    public int deleteByNamedHQL(String queryName, Map<String, Object> params) {
        return executeByHQL(generateDynamicNamedQueryString(sessionFactory, queryName, params), params);
    }
     ...
     * @return
     */
    public List<Entity> listByEntity(Entity entityInst, List<PropertyFilter> propertyFilters, String orderBy,
                                     PagingInfo pagingInfo) {
    ...
    @Override
    public List<Entity> listAllByOrderPage(PagingInfo pagingInfo, String orderBy) {
        Criteria criteria = getSession().createCriteria(entityClass);
        // 排序
    ...
    private Criterion[] buildCriterionByPropertyFilter(final List<PropertyFilter> filters, boolean queryOr) {
        if (!queryOr) {
     ...
     * @return
     */
    public List<Entity> listByEntity(Entity entity) {
     ...
     * @return
     */
    public List<Entity> listByEntityAndPage(Entity entity, PagingInfo pagingInfo, String orderBy) {
    ...
    protected void bindSessionFactory(String sessionFactoryName, String multiTenancyStrategy) {
        ...
        if (sessionFactory == null) {
            sessionFactory = ApplicationContextHolder.getBean(sessionFactoryName, SessionFactory.class);
        }
     ...
     * @return
     */
    public EntityDao getDaoBySessionFactory(String sessionFactoryId) {
        ...
        if (!DAO_MAP.containsKey(sessionFactoryId)) {
            ...
            try {
                EntityDaoImpl jpaDao = this.getClass().newInstance();
