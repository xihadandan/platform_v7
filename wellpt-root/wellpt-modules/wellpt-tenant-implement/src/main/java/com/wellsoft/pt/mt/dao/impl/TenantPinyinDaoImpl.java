/*
 * @(#)2014-1-2 V1.0
 *
 * Copyright 2014 WELL-SOFT, Inc. All rights reserved.
 */
package com.wellsoft.pt.mt.dao.impl;

import com.wellsoft.pt.jpa.dao.impl.AbstractJpaDaoImpl;
import com.wellsoft.pt.mt.dao.TenantPinyinDao;
import com.wellsoft.pt.mt.entity.TenantPinyin;
import org.springframework.stereotype.Repository;

import java.util.HashMap;
import java.util.Map;

/**
 * Description: 如何描述该类
 *
 * @author zhulh
 * @version 1.0
 *
 * <pre>
 * 修改记录:
 * 修改后版本	修改人		修改日期			修改内容
 * 2014-1-2.1	zhulh		2014-1-2		Create
 * </pre>
 * @date 2014-1-2
 */
@Repository
public class TenantPinyinDaoImpl extends AbstractJpaDaoImpl<TenantPinyin, String> implements TenantPinyinDao {
    private static final String DELETE_BY_USER = "delete from TenantPinyin pinyin where pinyin.entityUuid = :entityUuid";

    /**
     * 如何描述该方法
     *
     * @param entityUuid
     */
    public void deleteByEntityUuid(String entityUuid) {
        Map<String, Object> values = new HashMap<String, Object>();
        values.put("entityUuid", entityUuid);
        this.deleteByHQL(DELETE_BY_USER, values);
    }

}
