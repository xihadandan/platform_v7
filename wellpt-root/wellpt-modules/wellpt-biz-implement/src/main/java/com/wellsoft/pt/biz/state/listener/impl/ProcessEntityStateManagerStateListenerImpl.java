 ...
 */
@Component
public class ProcessEntityStateManagerStateListenerImpl extends AbstractStateManagerStateListener {
    ...
    protected void doStateChanged(Event event, EnumStateTriggerType stateTriggerType, StateConfigMatcher configMatcher) {
        ...
        // 业务主体状态管理——业务事项、过程节点、业务流程状态变更
        ProcessDefinitionJson.ProcessEntityConfig entityConfig = parser.getProcessEntityConfig();
        if (entityConfig == null) {
            return;
        }
        List<StateDefinition.StateConfig> stateConfigs = StateManagerHelper.getStateConfigs(entityConfig.getStates(), stateTriggerType, configMatcher);
