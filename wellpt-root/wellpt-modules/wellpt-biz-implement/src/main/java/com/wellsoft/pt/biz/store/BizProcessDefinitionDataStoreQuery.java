 ...
 */
@Component
public class BizProcessDefinitionDataStoreQuery extends AbstractDataStoreQueryInterface {
    ...
    public CriteriaMetadata initCriteriaMetadata(QueryContext context) {
        ...
        criteriaMetadata.add("version", "t.version", "版本", Double.class);
        criteriaMetadata.add("enabled", "t.enabled", "是否启用", Boolean.class);
        criteriaMetadata.add("businessId", "t.business_id", "业务ID", String.class);
        criteriaMetadata.add("tagId", "t.tag_id", "业务标签", String.class);
    ...
    public List<QueryItem> query(QueryContext queryContext) {
        List<QueryItem> queryItems = queryContext.getNativeDao().namedQuery("bizProcessDefinitionQuery",
                getQueryParams(queryContext), QueryItem.class, queryContext.getPagingInfo());
        // 名称显示版本号
        queryItems.forEach(item -> {
            item.put("name", item.getString("name") + "(" + item.getDouble("version") + ")");
        });
        return queryItems;
    }
