...
public class ServletUtils {
    ...
    /**
     * 返回服务端地址。集群环境时ServletUtils.getServerRequestURLConfig
     *
     * @param request
     * @return ps：http://192.168.40.10:8080
     */
    ...
    public static String getServerRequestURLConfig(HttpServletRequest request) {
        // server.requestURL适用集群配置
        String requestURL = Config.getValue("server.requestURL");
        if (StringUtils.isNotBlank(requestURL)) {
            return requestURL;
        }
        return getServerRequestURL(request);
    }
