...
public class WellJBarcode {
    ...
    private WellTextPainter textpainter;
     ...
     * @param barHeight
     */
    public WellJBarcode(BarcodeEncoder encoder, BarcodePainter painter, WellTextPainter textpainter, double xdimension,
                        int barHeight, double wideRatio) {
     ...
     * @param textpainter
     */
    public WellJBarcode(BarcodeEncoder encoder, BarcodePainter painter, WellTextPainter textpainter) {
     ...
     */
    public BufferedImage createBarcode(String text) throws InvalidAtributeException {
        String tmp = new String(text);
        if (isCheckDigit()) {
            text = text + bcencoder.computeCheckSum(text);
            if (isShowCheckDigit()) {
                tmp = text;
            }
        }
        BarSet[] encoded = bcencoder.encode(text);
        BufferedImage img = bcpainter.paint(encoded, 1, barHeight, wideToNarrow);
        if (isShowText()) {
            textpainter.paintText(img, tmp, 1);
        }

        BufferedImage result = new BufferedImage((int) (img.getWidth() * (module / (MIN_XDIMENSION))) + 1,
                (int) getBarHeight(), BufferedImage.TYPE_INT_RGB);
        AffineTransform at = AffineTransform.getScaleInstance((module / (MIN_XDIMENSION)),
     ...
     * 如何描述该方法
     *
     * @param projectIdText   项目编号
     * @param projectNameText 项目名称
     * @param text            办件单号
     * @param itemNameText    事项名称
     * @return
     * @throws InvalidAtributeException
     */
    public BufferedImage createBarcode(String projectIdText, String projectNameText, String text, String itemNameText)
            throws InvalidAtributeException {
        String tmp = new String(text);
        if (isCheckDigit()) {
            text = text + bcencoder.computeCheckSum(text);
        ...
        BufferedImage img = bcpainter.paint(encoded, 1, barHeight, wideToNarrow);
        if (isShowText()) {
            textpainter.paintText(img, projectIdText, projectNameText, text, itemNameText, 1);
        }
        BufferedImage result = new BufferedImage((int) (img.getWidth() * (module / (MIN_XDIMENSION))) + 1,
                (int) getBarHeight(), BufferedImage.TYPE_INT_RGB);
        AffineTransform at = AffineTransform.getScaleInstance((module / (MIN_XDIMENSION)),
     ...
     */
    public WellTextPainter getWellTextPainter() {
        return textpainter;
    }
     ...
     */
    public void setWellTextPainter(WellTextPainter textpainter) {
        this.textpainter = textpainter;
    }
