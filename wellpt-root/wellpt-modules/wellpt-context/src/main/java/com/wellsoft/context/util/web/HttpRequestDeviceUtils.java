...
public class HttpRequestDeviceUtils {
    ...
    private static LiteDeviceResolver liteDeviceResolver = new LiteDeviceResolver();
    ...
    private static String[] tabletUserAgents = new String[]{"13G36"};
    ...
    public static boolean isTablet(HttpServletRequest request) {
        ...
        String userAgent = request.getHeader("User-Agent");
        if (null != userAgent && device.isMobile()) {
            for (String tabletUserAgent : tabletUserAgents) {
                if (userAgent.contains(tabletUserAgent)) {
                    return true;
                }
            }
        }
        return null != device && device.isTablet();
    }
}
