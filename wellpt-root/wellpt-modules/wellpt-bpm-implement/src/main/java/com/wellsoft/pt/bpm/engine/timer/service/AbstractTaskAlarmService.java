 ...
 * @date 2013-10-22
 */
public abstract class AbstractTaskAlarmService extends AbstractTaskTimerService {
    /**
     * @param dueUsers
    ...
    protected void hanlderAndSendMessage(Collection<TaskTimerUser> taskTimerUsers, TaskInstance taskInstance) {
        // 预警提醒——人员
        List<TaskTimerUser> alarmUsers = filterTaskTimerUser(taskTimerUsers, TimerUser.ALARM_OBJECT);

        if (alarmUsers.isEmpty()) {
            return;
        }
...
//        OrgApiFacade orgApiFacade = ApplicationContextHolder.getBean(OrgApiFacade.class);
        ...
        for (TaskTimerUser taskTimerUser : alarmUsers) {
            String value = taskTimerUser.getValue();
            WorkFlowTimerUser alarmUser = Enum.valueOf(WorkFlowTimerUser.class, value.trim());
            ...
            switch (alarmUser) {
                case Doing: // 在办人员
                    ...
                    messageTemplates = flowDelegate.getMessageTemplateMap().get(
                            WorkFlowMessageTemplate.WF_WORK_ALARM_DOING.getType());
                    MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_ALARM_DOING, messageTemplates,
                            taskInstance, taskInstance.getFlowInstance(), userIds, ParticipantType.TodoUser);
                    break;
                case Monitor: // 督办人员
                    userIds = getMonitorUserIds(taskInstance);
                    messageTemplates = flowDelegate.getMessageTemplateMap().get(
                            WorkFlowMessageTemplate.WF_WORK_ALARM_SUPERVISE.getType());
                    MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_ALARM_SUPERVISE, messageTemplates,
                            taskInstance, taskInstance.getFlowInstance(), userIds, ParticipantType.SuperviseUser);
                    break;
                case Tracer: // 跟踪人员
                    userIds = getTracerUserIds(taskInstance);
                    messageTemplates = flowDelegate.getMessageTemplateMap().get(
                            WorkFlowMessageTemplate.WF_WORK_ALARM_TRACER.getType());
                    MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_ALARM_TRACER, messageTemplates,
                            taskInstance, taskInstance.getFlowInstance(), userIds, ParticipantType.MonitorUser);
                    break;
                case Admin: // 流程管理人员
                    userIds = getAdminUserIds(taskInstance);
                    messageTemplates = flowDelegate.getMessageTemplateMap().get(
                            WorkFlowMessageTemplate.WF_WORK_ALARM_ADMIN.getType());
                    MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_ALARM_ADMIN, messageTemplates,
                            taskInstance, taskInstance.getFlowInstance(), userIds, ParticipantType.MonitorUser);
                    break;
                case Other: // 其他人员
                    // 预警提醒——其他人员
                    List<TaskTimerUser> otherUsers = filterTaskTimerUser(taskTimerUsers, TimerUser.ALARM_USER);
                    userIds = getOtherUserIds(otherUsers, taskInstance, taskData);
                    messageTemplates = flowDelegate.getMessageTemplateMap().get(
                            WorkFlowMessageTemplate.WF_WORK_ALARM_OTHER.getType());
                    MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_ALARM_OTHER, messageTemplates,
...
//                            Set<String> ids = orgApiFacade.queryUserMainJobSuperiorLeaderList(u);
                    ...
                    if (!superiros.isEmpty()) {
                        ...
                        MessageClientUtils.send(taskData, WorkFlowMessageTemplate.WF_WORK_DUE_DOING_SUPERIOR, messageTemplates,
