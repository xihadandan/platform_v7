...
public class GroupAppFunctionSourceLoader extends AbstractAppFunctionSourceLoader {
    ...
    public List<AppFunctionSource> getAppFunctionSources() {
        ...
        List<MultiOrgGroup> groups = this.dao.getAll(MultiOrgGroup.class);
        for (MultiOrgGroup group : groups) {
            appFunctionSources.add(convert2AppFunctionSource(group));
        }
    ...
    public <ITEM extends Serializable> AppFunctionSource convert2AppFunctionSource(ITEM item) {
        MultiOrgGroup group = (MultiOrgGroup) item;
        String uuid = group.getUuid();
        String fullName = group.getName() + "_" + group.getId();
        String name = "组织_群组_" + group.getName();
