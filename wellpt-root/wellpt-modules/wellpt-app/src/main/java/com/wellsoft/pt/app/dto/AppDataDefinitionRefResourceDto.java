/*
 * @(#)8/18/23 V1.0
 *
 * Copyright 2023 WELL-SOFT, Inc. All rights reserved.
 */
package com.wellsoft.pt.app.dto;

import com.google.common.collect.Lists;
import com.wellsoft.pt.app.entity.AppDataDefinitionRefResourceEntity;
import com.wellsoft.pt.app.ui.client.widget.configuration.FunctionElement;

import java.util.List;

/**
 * Description: 如何描述该类
 *
 * @author zhulh
 * @version 1.0
 *
 * <pre>
 * 修改记录:
 * 修改后版本	修改人		修改日期			修改内容
 * 8/18/23.1	zhulh		8/18/23		Create
 * </pre>
 * @date 8/18/23
 */
public class AppDataDefinitionRefResourceDto extends AppDataDefinitionRefResourceEntity {

    /**
     * 引用的功能元素
     */
    private List<FunctionElement> functionElements = Lists.newArrayList();

    /**
     * @return the functionElements
     */
    public List<FunctionElement> getFunctionElements() {
        return functionElements;
    }

    /**
     * @param functionElements 要设置的functionElements
     */
    public void setFunctionElements(List<FunctionElement> functionElements) {
        this.functionElements = functionElements;
    }
}
