 ...
 * 修改记录:
 * 修改后版本	修改人		修改日期			修改内容
 * 2016年11月14日.1	zhulh		2016年11月14日		Create
 * </pre>
 * @date 2016年11月14日
 */
@Service
@Transactional(readOnly = true)
public class FlowCategoryAppFunctionSourceLoader extends AbstractAppFunctionSourceLoader {
    ...
    public String getAppFunctionType() {
        return AppFunctionType.FlowCategory;
    }
    ...
    public List<AppFunctionSource> getAppFunctionSources() {
        ...
        List<FlowCategory> flowCategories = this.dao.getAll(FlowCategory.class);
        for (FlowCategory flowCategory : flowCategories) {
            appFunctionSources.add(convert2AppFunctionSource(flowCategory));
        }
    ...
    public <ITEM extends Serializable> AppFunctionSource convert2AppFunctionSource(ITEM item) {
        FlowCategory flowCategory = (FlowCategory) item;
        String uuid = flowCategory.getUuid();
        String fullName = flowCategory.getName() + "_" + flowCategory.getCode();
        String name = "流程分类_" + flowCategory.getName();
        String id = flowCategory.getCode();
        String code = flowCategory.getCode();
