...
public class ResourceMenuAppFunctionSourceLoader extends AbstractAppFunctionSourceLoader {
    ...
    private String getResourcePath(Resource resource) {
        ...
        while (parent != null) {
            ...
            if (resourceSet.contains(parent)) {
                break;
            }
            resourceSet.add(parent);
        }
    ...
    public <ITEM extends Serializable> AppFunctionSource convert2AppFunctionSource(ITEM item) {
        ...
        String name = "菜单/URL资源_" + resourcePath;
        ...
        extras.put("url", resource.getUrl());
    ...
    public Collection<Object> getObjectIdentities(AppFunctionSource appFunctionSource) {
        String menuCode = appFunctionSource.getCode();
        String url = ObjectUtils.toString(appFunctionSource.getExtras().get("url"),
                StringUtils.EMPTY);
        List<Object> objectIdentities = new ArrayList<Object>();
        if (StringUtils.isBlank(url)) {
            if (StringUtils.isNotBlank(menuCode)) {
                objectIdentities.add(menuCode);
            } else {
