...
public class PrintTemplateAppFunctionSourceLoader extends AbstractAppFunctionSourceLoader {
    ...
    public String getAppFunctionType() {
        return AppFunctionType.PrintTemplate;
    }
    ...
    public List<AppFunctionSource> getAppFunctionSources() {
        ...
        List<PrintTemplate> printTemplates = this.dao.getAll(PrintTemplate.class);
        for (PrintTemplate printTemplate : printTemplates) {
            appFunctionSources.add(convert2AppFunctionSource(printTemplate));
        }
    ...
    public <ITEM extends Serializable> AppFunctionSource convert2AppFunctionSource(ITEM item) {
        PrintTemplate printTemplate = (PrintTemplate) item;
        String uuid = printTemplate.getUuid();
        String fullName = printTemplate.getName() + "_" + printTemplate.getId();
        String name = "打印模板_" + printTemplate.getName() + "_v" + printTemplate.getVersion();
        String id = printTemplate.getId() + "_v" + printTemplate.getVersion();
        String code = printTemplate.getCode();
