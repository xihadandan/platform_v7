 ...
 * 修改记录:
 * 修改后版本	修改人		修改日期			修改内容
 * 2014-1-6.1	zhulh		2014-1-6		Create
 * </pre>
 * @date 2014-1-6
 */
public class ZipFileConvertUtils {
    private static Log logger = LogFactory.getLog(ZipFileConvertUtils.class.getName());

    public static File convertRar(File file) throws Exception {
        // 文件类型判断
        ...
        if (!"rar".equalsIgnoreCase(extension)) {
            throw new RuntimeException("请上传rar/zip格式的文件!");
        }
        ...
        File rarFolder = new File(baseFolder, folderName + "_" + UUID.randomUUID());
        if (!rarFolder.exists()) {
            rarFolder.mkdirs();
        }
        ...
        try {
            // 解压RAR文件
            extractArchive(path, rarFolder.getAbsolutePath());
        } catch (Exception e) {
            logger.info(e.getMessage());
            throw new RuntimeException("压缩文件格式未知或者数据已经被损坏!");
        }

        // RAR文件转为ZIP文件
        File zipFile = new File(baseFolder, rarFolder.getName() + ".zip");

        ZipUtils.zipFolder(rarFolder.getAbsolutePath(), zipFile.getAbsolutePath(), false);
