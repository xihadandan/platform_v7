...
@Service
@Transactional
public class SynOfflineEventListener extends WellptEventListener<SynOfflineEvent> {
     ...
     */
    @Override
    public synchronized void onApplicationEvent(SynOfflineEvent event) {
        Object direct = event.getSource();
        if (direct != null && direct instanceof Integer) {
            long currTime = System.currentTimeMillis();
            if (((Integer) direct) == 2 && currTime - lastSendtime > 1000 * 60 * 60 * 2) { // 外网发送短信(2小时安全期,防止链路抖动)
