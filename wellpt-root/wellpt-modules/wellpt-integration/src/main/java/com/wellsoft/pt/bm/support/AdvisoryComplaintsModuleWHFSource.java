...
@Service
@Transactional
public class AdvisoryComplaintsModuleWHFSource extends AbstractViewDataSource {
    ...
    public Collection<ViewColumn> getAllViewColumns() {
        ...
        ViewColumn subject = new ViewColumn();
        subject.setAttributeName("body");
        subject.setColumnAlias("body");
        subject.setColumnName("提交内容");
        subject.setColumnType(ViewColumnType.STRING);
        viewColumns.add(subject);
        ...
        ViewColumn type = new ViewColumn();
        type.setAttributeName("type");
        type.setColumnAlias("type");
        type.setColumnName("类型");
        type.setColumnType(ViewColumnType.STRING);
        viewColumns.add(type);
        ...
        replyTime.setColumnName("回复时间");
        replyTime.setColumnType(ViewColumnType.STRING);
        viewColumns.add(replyTime);

        ViewColumn submitUserName = new ViewColumn();
        submitUserName.setAttributeName("submitUserName");
        submitUserName.setColumnAlias("submitUserName");
        submitUserName.setColumnName("提交人姓名");
        submitUserName.setColumnType(ViewColumnType.STRING);
        viewColumns.add(submitUserName);

        ViewColumn isReply = new ViewColumn();
        isReply.setAttributeName("isReply");
        isReply.setColumnAlias("isReply");
        isReply.setColumnName("是否已回复");
        isReply.setColumnType(ViewColumnType.STRING);
        viewColumns.add(isReply);

        ViewColumn replyId = new ViewColumn();
        replyId.setAttributeName("replyId");
        replyId.setColumnAlias("replyId");
        replyId.setColumnName("回复单位");
        replyId.setColumnType(ViewColumnType.STRING);
        viewColumns.add(replyId);

        ViewColumn replyMsg = new ViewColumn();
        replyMsg.setAttributeName("replyMsg");
        replyMsg.setColumnAlias("replyMsg");
        replyMsg.setColumnName("回复内容");
        replyMsg.setColumnType(ViewColumnType.STRING);
        viewColumns.add(replyMsg);
    ...
    public List<QueryItem> query(Collection<ViewColumn> viewColumns, String whereHql, Map<String, Object> queryParams,
        ...
        StringBuffer hql = new StringBuffer("select " + sb + " from AdvisoryComplaints o where 1=1 ");
        ...
        if (units == null || units.size() == 0) {
            ...
            unit = units.get(0);
            hql.append(" and o.replyId =:replyId ");
            values.put("replyId", unit.getId());
        }
        ...
        List<CdDataDictionaryItemDto> dataDictionaries = basicDataApiFacade.getDataDictionariesByType("ADVISORY_COMPLAINTS");
        List<QueryItem> queryItems = commercialBusinessService.queryQueryItem(hql.toString(), values, pagingInfo);
        for (QueryItem queryItem : queryItems) {
            String type = (String) queryItem.get("type");
            for (CdDataDictionaryItemDto d : dataDictionaries) {
                if (type.equals(d.getValue())) {
                    queryItem.put("type", d.getLabel());
                    break;
                }
            }
        }
    ...
    public Long count(Collection<ViewColumn> viewColumns, String whereHql, Map<String, Object> queryParams) {
        ...
        StringBuffer hql = new StringBuffer("select count(*) from AdvisoryComplaints o where 1=1 ");
        Map<String, Object> values = new HashMap<String, Object>();
