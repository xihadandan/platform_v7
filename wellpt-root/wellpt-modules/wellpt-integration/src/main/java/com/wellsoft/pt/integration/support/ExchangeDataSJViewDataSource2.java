 ...
 */
@Component
public class ExchangeDataSJViewDataSource2 extends AbstractDataExchangeViewDataSource {
    ...
    public List<QueryItem> query(Collection<ViewColumn> viewColumns, String whereHql, Map<String, Object> queryParams,
        ...
        StringBuffer hql = new StringBuffer(
                "select "
                        + sbStr
                        + " from ExchangeDataMonitor o left join o.exchangeDataSendMonitor s left join s.exchangeData d where o.unitId = '"
    ...
    public Long count(Collection<ViewColumn> viewColumns, String whereHql, Map<String, Object> queryParams) {
        ...
        // }
        StringBuffer hql = new StringBuffer(
                "select count(*) from ExchangeDataMonitor o left join o.exchangeDataSendMonitor s left join s.exchangeData d where o.unitId = '"
                        + commonUnits.get(0).getId() + "'");
        if (!StringUtils.isBlank(whereHql)) {
            hql.append(" and " + whereHql);
        }
