 ...
 */
@Component
public class CurrentUserDepartmentIdDataOwnerProviderImpl extends AbstractCurrentUserDataOwnerProvider {
    ...
    public List<String> getOwnerIds() {
        return getCurrentUserDepartmentIds();
    }

    /**
     * @return
     */
    private List<String> getCurrentUserDepartmentIds() {
        UserDetails userDetails = SpringSecurityUtils.getCurrentUser();
        List<String> departmentIds = Lists.newArrayList();
        OrgTreeNodeDto mainJob = userDetails.getMainJob();
        if (mainJob != null) {
            if (StringUtils.isNotBlank(mainJob.getDeptId())) {
                departmentIds.add(mainJob.getDeptId());
            }
        }
        ...
        if (CollectionUtils.isNotEmpty(otherJobs)) {
            for (OrgTreeNodeDto otherJob : otherJobs) {
                if (StringUtils.isNotBlank(otherJob.getDeptId())) {
                    departmentIds.add(otherJob.getDeptId());
                }
            }
        }
