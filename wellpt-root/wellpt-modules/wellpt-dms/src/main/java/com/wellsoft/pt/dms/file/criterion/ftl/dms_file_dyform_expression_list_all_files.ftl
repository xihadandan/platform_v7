...
<if isNotBlank(readFileField) || isNotBlank(editFileField)>
    select distinct a_.uuid as data_uuid
    from ${tableName} a_
...
and t.deny = 'Y') c2
on fc.ref_object_identity_uuid =
c2.object_identity_uuid)) f2
where f1.folder_uuid = f2.folder_uuid and f1.status <> '0'
)