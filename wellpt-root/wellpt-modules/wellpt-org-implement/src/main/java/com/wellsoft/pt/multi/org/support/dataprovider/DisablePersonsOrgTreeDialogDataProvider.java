 ...
 */
@Component
public class DisablePersonsOrgTreeDialogDataProvider extends AbstractOrgTreeDialogServiceImpl
        implements OrgTreeDialogProvider, OrgTreeAllUserProvider {

    public static final String TYPE = "DisablePersons";
    /**
    ...
    public List<OrgNode> children(OrgTreeDialLogAsynParms parms) {
        ...
        this.addDisTreeNodes(parms.getOrgVersionId(), parms.getIsNeedUser(), treeNodes, elements,
                parms.getCheckedIds());
        // 职位节点 并且 需要用户数据
        if (parms.getTreeNodeId().startsWith(IdPrefix.JOB.getValue()) && parms.getIsNeedUser() == 1) {
            treeNodes = this.getDisJobUserTreeNode(parms.getOrgVersionId(), parms.getTreeNodeId(),
                    parms.getCheckedIds());
        }
        return treeNodes;
    }
    ...
    private Map<String, OrgNode> orgNodeMap(OrgNode orgNode, OrgTreeDialLogAsynParms parms, Set<String> jobIdSet) {
        return this.getStringOrgNodeMap(orgNode, parms.getCheckedIds(), jobIdSet, parms.getOrgVersionId(),
                parms.getOrgVersionId());
    }
    ...
    public List<OrgNode> full(OrgTreeDialLogAsynParms parms) {
        ...
        if (!MultiOrgUserAccount.PT_ACCOUNT_ID.equals(userId) && StringUtils.isNotBlank(eleIdPath)) {
            ...
            for (String path : eleIdPathSet) {
                ...
                List<OrgNodeDto> nodeDtoList = multiOrgTreeNodeService.listItemHqlQuery(hqlSb.toString(),
                        OrgNodeDto.class, query);
        ...
        if (parms.getIsNeedUser() == 1) {
            jobNodeAddDisUsers(parms.getOrgVersionId(), parms.getCheckedIds(), orgNodeMap, jobIdSet);
        }
    ...
    public List<OrgNode> search(OrgTreeDialLogAsynParms parms) {
        ...
        if (parms.getIsNeedUser() == 1) {
            ...
            hqlSb.append(
                    " where a.id = b.userId and a.isForbidden = 1 and exists(select 1 from MultiOrgUserTreeNode c where a.id = c.userId and c.orgVersionId=:orgVersionId )");
            /**
             *  子查询MultiOrgTreeNode 指定 orgVersionId 获取 禁用用户isForbidden=1
             *  全模糊查询
             *  按匹配度 =**》**%》%**》%**% 排序
             */
            hqlSb.append(
                    "and (a.loginName like :key_like or a.loginNameLowerCase like :key_like or a.userName like :key_like or a.userNamePy like :key_like or a.userNameJp like :key_like) "
                            + "order by ("
        ...
        String hql = "from MultiOrgElement a where exists (select 1 from MultiOrgTreeNode b where b.orgVersionId=:orgVersionId and b.eleId=a.id and b.eleIdPath like :eleIdPath) "
                + "and (a.name like :key_like or a.shortName like :key_like) " + "order by ("
    ...
    private List<OrgNode> searchEleIdPath(OrgTreeDialLogAsynParms parms, String eleIdPath) {
        ...
        if (parms.getIsNeedUser() == 1 && jobIdSet.size() > 0) {
            ...
            StringBuilder hqlSb = new StringBuilder(
                    "select a.id as id,a.userName as name,a.userNamePy as namePy,b.sex as iconSkin,a.code as code from MultiOrgUserAccount a,MultiOrgUserInfo b ");
            hqlSb.append(
                    " where a.id = b.userId and a.isForbidden = 1 and exists(select 1 from MultiOrgUserTreeNode c where a.id = c.userId and c.orgVersionId=:orgVersionId and ");
    ...
    private Set<String> getJobIdSet(String orgVersionId, String eleIdPath) {
        ...
        for (String path : eleIdPathSet) {
            ...
            hqlSb.append(
                    " where a.orgVersionId=:orgVersionId and a.eleId like :eleId and a.eleIdPath like :eleIdPath ");
    ...
    public UserNodePy allUserSearch(OrgTreeDialLogAsynParms parms) {
        ...
        if (StringUtils.isNotBlank(parms.getTreeNodeId()) && !parms.getOrgVersionId().equals(parms.getTreeNodeId())) {
            MultiOrgTreeNode treeNode = multiOrgTreeNodeService.queryByVerIdEleId(parms.getOrgVersionId(),
                    parms.getTreeNodeId());
            query.put("eleIdPath", treeNode.getEleIdPath() + "%");
            hqlSb.append(
                    ",MultiOrgTreeNode d where  d.orgVersionId=:orgVersionId and d.eleId = c.eleId and a.id = b.userId and a.id = c.userId and a.isForbidden = 1 and c.orgVersionId=:orgVersionId and d.eleIdPath like :eleIdPath ");
        } else {
            String eleIdPath = parms.getOtherParams().get("eleIdPath");
            if (StringUtils.isNotBlank(eleIdPath)) {
                ...
                hqlSb.append(
                        " where a.id = b.userId and a.id = c.userId and a.isForbidden = 1 and c.orgVersionId=:orgVersionId ");
            }
        }
        if (StringUtils.isNotBlank(parms.getKeyword())) {
            ...
            query.put("key_like", "%" + parms.getKeyword() + "%");
            hqlSb.append(
                    " and (a.loginName like :key_like or a.userName like :key_like or a.userNamePy like :key_like or a.userNameJp like :key_like) ");
        }
