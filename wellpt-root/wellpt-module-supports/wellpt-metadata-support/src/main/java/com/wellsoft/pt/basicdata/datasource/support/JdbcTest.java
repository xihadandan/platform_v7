 ...
 * 2016-8-8.2	zhongzh		2016-8-8		Deprecated (使用JdbcSupports,该类连接未关闭,迁移平稳后删除)
 * </pre>
 * @date 2014-8-14
 */
@Deprecated
public class JdbcTest {
    ...
    public List getColumnByOut(String databaseType, String owner, String userName, String passWord, String url,
        ...
        if (databaseType.equals("1")) {
            ...
            try {
                Connection connection = jc.createConnection(databaseType, userName, passWord, url);
                PreparedStatement pState = connection.prepareStatement(psSql.toString());
                ResultSet rs = pState.executeQuery();
                while (rs.next()) {
                    Map<String, String> map = new HashMap<String, String>();
                ...
                if (rs != null) {
                    jc.releaseConnection(connection);
                }
            } catch (SQLException e) {
                logger.error(e.getMessage(), e);
            }
