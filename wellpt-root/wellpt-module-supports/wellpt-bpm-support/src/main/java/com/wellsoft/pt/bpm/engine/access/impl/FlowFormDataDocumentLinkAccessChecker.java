...
/**
 * Description: 流程表单数据文档链接关系，流程权限检查
 *
 * @author zhulh
 * @version 1.0
 *
 ...
 */
@Component
public class FlowFormDataDocumentLinkAccessChecker extends AbstractDocumentLinkAccessChecker {
    ...
    public String getName() {
        return "flowFormDataDocumentLinkAccessChecker";
    }
    ...
    public boolean check(String dataUuid, DocumentLinkInfo documentLinkInfo) {
        String flowInstUuid = flowInstanceService.getUuidByDataUuid(dataUuid);
        if (StringUtils.isBlank(flowInstUuid)) {
            return false;
        }

        // 源数据是草稿流程的表单数据
        if (StringUtils.equals(dataUuid, documentLinkInfo.getSourceDataUuid())) {
