...
import com.wellsoft.pt.rule.engine.executor.DefineExecutor;
 ...
 * @date 2015-6-24
 */
public class DefineCommandNode extends Node {
    ...
    public void parse(Context context) throws ParseException {
        ...
        executor = new DefineExecutor(); // context.createExecutor(name);
    }
    ...
    public Result execute(Param param) throws ExecuteException {
        ...
        logger.debug("start execute DefineCommondNode at level " + level);
        if (executor == null) {
            throw new ExecuteException(name + ": is not defined");
        } else {
            executor.execute(new Param(context, level, expression, msg));
        }
        logger.debug("over execute DefineCommondNode at level " + level);
    ...
    public String toString() {
        return "[define " + expression.toString() + " end]";
    }
