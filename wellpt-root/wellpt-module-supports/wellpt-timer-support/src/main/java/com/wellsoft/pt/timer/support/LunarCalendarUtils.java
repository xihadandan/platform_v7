 ...
 * 修改记录:
 * 修改后版本	修改人		修改日期			修改内容
 * 2021年4月30日.1	zhulh		2021年4月30日		Create
 * </pre>
 * @date 2021年4月30日
 */
public class LunarCalendarUtils {
	...
	public static void main(String[] args) {
		//计算录入年份是否有闰月
		System.out.println(leapMonth(2021));
		//传回农历 year年month月的总天数，总共有13个月包括闰月
		System.out.println(daysInLunarMonth(2017, 6));
		//传回农历year年month月的总天数
		System.out.println(daysInMonth(2017, 6, false));
		//阳历转阴历  9.25  10.23 有问题
		System.out.println(Arrays.toString(solarToLunar(2022, 1, 19)));
		//阴历转阳历
		System.out.println(Arrays.toString(lunarToSolar(2021, 4, 16, false)));
		System.out.println(getSolar(2021, 4, 16));
	}*/
    ...
    public static String getLunar(int year, int month, int day) {
        int[] lunars = solarToLunar(year, month, day);
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.YEAR, lunars[0]);
        calendar.set(Calendar.MONTH, lunars[1] - 1);
        calendar.set(Calendar.DAY_OF_MONTH, lunars[2]);
     ...
     * @return
     */
    public static String getSolar(int year, int month, int day) {
        int[] solars = lunarToSolar(year, month, day, leapMonth(year) != 0);
        Calendar calendar = Calendar.getInstance();
        calendar.set(Calendar.YEAR, solars[0]);
        calendar.set(Calendar.MONTH, solars[1] - 1);
        calendar.set(Calendar.DAY_OF_MONTH, solars[2]);
        return DateUtils.formatDate(calendar.getTime());
    }
